---
task_id: aff4b7e3-4b53-4a42-861f-0cd9aa36e7f6
...
created_at: '2026-06-12T19:21:51.547427+00:00'
updated_at: '2026-06-12T19:38:16.504220+00:00'
target_files:
- backend/app/notification_pipeline.py
---
...
The `NOTIFICATION_CRITICALITY` dictionary (line 39) is explicitly stated to be a 'Mirror of NOTIFICATION_CRITICALITY in src/lib/telegram.js — keep the two in sync.' Similarly, the logic for `build_daily_summary_text` (line 80) mirrors `src/lib/notificationRules.js`. This reliance on manual synchronization of critical business logic and data structures across different language ecosystems (Python a

📁 file: backend/app/notification_pipeline.py (line 39)
...
- `backend/app/notification_pipeline.py:39`
...
- `backend/app/handlers/scan_failure_handler.py` — این فایل `notification_pipeline.py` را import می‌کند (caller)
- `backend/app/tasks/scan_tasks.py` — این فایل `notification_pipeline.py` را import می‌کند (caller)
- `tests/integration/test_notification_pipeline.py` — این فایل `notification_pipeline.py` را import می‌کند (caller)
...
## 🧪 دستورات اعتبارسنجی
- `python -m py_compile backend/app/notification_pipeline.py`
- `ruff check backend/app/notification_pipeline.py`
- `pytest -x`
