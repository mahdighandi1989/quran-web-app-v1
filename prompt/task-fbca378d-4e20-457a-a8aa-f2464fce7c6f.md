---
task_id: fbca378d-4e20-457a-a8aa-f2464fce7c6f
title: ⬆️ aiClient.js ممکن است نیاز به update داشته باشد به‌خاطر تغییرات در AIWidgets.jsx
...
# ⬆️ aiClient.js ممکن است نیاز به update داشته باشد به‌خاطر تغییرات در AIWidgets.jsx
...
## 🔍 جزئیات
- 📂 فایل تغییریافته (consumer): `src/components/AIWidgets.jsx`
- 📂 فایل upstream (producer — نیازمند update): `src/lib/aiClient.js`
- 🔥 risk: **high**
- 📝 reason: The `AIAssistButton` component in `src/components/AIWidgets.jsx` now passes an `AbortSignal` via the `signal` option in the configuration object to the `chatComplete` function. If `src/lib/aiClient.js`'s `chatComplete` function did not previously support this option, it would need to be updated to correctly handle abort signals for ongoing AI requests. Failure to do so could lead to un-cancellable
...
---
[scan #2 at 2026-06-12T19:23:09.512206+00:00]
## 📋 شرح
یک فایل تغییر کرده و حالا از API/behavior جدیدی استفاده می‌کند که upstream فعلی ممکن است پشتیبانی نکند.

## 🔍 جزئیات
- 📂 فایل تغییریافته (consumer): `src/components/AIExamGenerator.jsx`
- 📂 فایل upstream (producer — نیازمند update): `src/lib/analytics.js`
- 🔥 risk: **medium**
- 📝 reason: فا
...
- `src/components/AIWidgets.jsx` — consumer که تغییر کرده
...
## 🌐 نقشهٔ وابستگی‌ها
`src/components/AIWidgets.jsx` به `src/lib/aiClient.js` وابسته است.
...
- [ ] بررسی شد `src/components/AIWidgets.jsx` چه API ای از `src/lib/aiClient.js` می‌خواهد
- [ ] اگر API در `src/lib/aiClient.js` نیست، اضافه شد
...
1. گام ۱: `src/components/AIWidgets.jsx` را بخوان و usages جدید از `src/lib/aiClient.js` را پیدا کن.
گام ۲: API contract در `src/lib/aiClient.js` چک کن.
گام ۳: اگر contract ناسازگار است، `src/lib/aiClient.js` را extend کن.
...
تغییر `src/lib/aiClient.js` ممکن است بقیه consumers را تحت تأثیر قرار دهد — قبل از تغییر، همهٔ مصرف‌کنندگانش را check کن.
