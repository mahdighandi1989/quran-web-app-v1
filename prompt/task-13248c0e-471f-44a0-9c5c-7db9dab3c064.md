---
task_id: 13248c0e-471f-44a0-9c5c-7db9dab3c064
title: 'Inconsistency: AIChat component documented but not visibly integrated'
type: bug
priority: high
...
created_at: '2026-06-12T19:12:38.630009+00:00'
updated_at: '2026-06-12T19:29:54.194353+00:00'
target_files:
- src/components/AIWidgets.jsx
- docs/ai-chat-use-cases.md
---

# Inconsistency: AIChat component documented but not visibly integrated

## Raw Idea

The `AIChat` component is defined in `src/components/AIWidgets.jsx` (lines 63-107) and explicitly listed in `docs/ai-chat-use-cases.md` as the UI surface for the `QA` (Question & Answer) use case. However, a previous task (`task-06f17cca`, marked 'merged') suggested it was unused and should be removed. Furthermore, the provided `App.jsx` snippet does not show `AIChat` being imported or rendered. This creates a critical inconsistency: either the component is intended to be used and its integration is missing/incomplete, or the documentation is stale and the component is indeed dead code. Given the project's focus on AI features, it's highly probable that the `QA` functionality is intended.
...
- `src/components/AIWidgets.jsx:63-107` — `AIChat`
  ```jsx
  export function AIChat({ getConfig, buildPrompt }) { ... }
  ```
- `docs/ai-chat-use-cases.md:15`
  ```markdown
...
React, Vite, AI integration.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/App.jsx` — Main application component, likely where routing or tab content is managed.
- `src/routes.jsx` — Defines application routes, a potential integration point for AIChat.
- `src/lib/aiTasks.js` — Contains `qaPrompt` which is needed for AIChat's `buildPrompt` prop.
- `prompt/archive/task-06f17cca-a305-40b3-b38a-06ae9cde3ee6.md` — Previous task that incorrectly marked AIChat as unused.
- `src/lib/aiClient.js` — `AIWidgets.jsx` این فایل را import می‌کند
- `src/lib/analytics.js` — `AIWidgets.jsx` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
This is a frontend consistency issue. If integrated, it will depend on `aiClient.js`, `aiTasks.js`, and `analytics.js`. No direct backend changes are expected, but the AI proxy (`VITE_AI_PROXY_URL`) will be used.
...
- [ ] The `AIChat` component is rendered in the application at an appropriate, accessible location (e.g., a new 'Q&A' tab or route).
- [ ] Users can interact with the `AIChat` (send questions, receive AI-generated answers).
- [ ] `trackAIInteraction(AI_USE_CASE.QA)` is correctly triggered on successful AI responses.
- [ ] The `docs/ai-chat-use-cases.md` remains accurate regarding `AIChat`'s role.
- [ ] The status of `task-06f17cca` is re-evaluated to reflect that `AIChat` is not unused.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Integrate the `AIChat` component into the application's UI, likely by adding a new route in `src/routes.jsx` or a dedicated tab/section in `src/App.jsx` that renders it. Ensure the `buildPrompt` prop is correctly passed to enable Q&A functionality. If, after thorough investigation, it's confirmed to be truly unused and not intended for future use, then remove the component and update `docs/ai-chat-use-cases.md` accordingly.

## 💡 نمونه‌های قبل/بعد
**Current state (AIChat defined but not rendered)**

_قبل:_
```
// AIChat component is defined in AIWidgets.jsx but not visibly imported or rendered in App.jsx or routes.jsx.
```

**Desired state (AIChat integrated into a route/tab)**

_بعد:_
```
import { AIChat } from './components/AIWidgets.jsx';
import { qaPrompt } from './lib/aiTasks.js';
// ... inside App.jsx or routes.jsx rendering logic:
// <Route path="/qa" element={<AIChat getConfig={getAiConfig} buildPrompt={qaPrompt} />} />
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run dev`
- `npm test src/test/aiClient.test.js`

## ⚠️ ریسک‌ها و موارد احتیاط
Requires understanding the intended UI flow for Q&A and potentially modifying core routing/layout components.
...
1. The `AIChat` component is rendered in the application at an appropriate, accessible location (e.g., a new 'Q&A' tab or route). _(verify: ui_interaction)_
2. Users can interact with the `AIChat` (send questions, receive AI-generated answers). _(verify: ui_interaction)_
3. `trackAIInteraction(AI_USE_CASE.QA)` is correctly triggered on successful AI responses. _(verify: ui_interaction)_
4. The `docs/ai-chat-use-cases.md` remains accurate regarding `AIChat`'s role. _(verify: static)_
5. The status of `task-06f17cca` is re-evaluated to reflect that `AIChat` is not unused. _(verify: manual_only)_

## Task Steps

### Step 1: بررسی وضعیت فعلی و کاربرد کامپوننت AIChat
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق codebase برای تعیین اینکه آیا کامپوننت `AIChat` در حال حاضر در جایی import یا render می‌شود یا خیر، و همچنین تأیید اینکه آیا واقعاً کد مرده است یا صرفاً ادغام آن ناقص مانده است. این بررسی باید شامل فایل `src/components/AIWidgets.jsx` برای تعریف کامپوننت و جستجو در فایل‌های اصلی UI (مانند `App.jsx`) برای یافتن import یا render آن باشد. هدف این است که وضعیت واقعی کامپوننت قبل از هرگونه تغییر مشخص شود.
**Excerpt:**
```
The `AIChat` component is defined in `src/components/AIWidgets.jsx` (lines 63-107) and explicitly listed in `docs/ai-chat-use-cases.md` as the UI surface for the `QA` (Question & Answer) use case. However, a previous task (`task-06f17cca`, marked 'merged') suggested it was unused and should be removed. Furthermore, the provided `App.jsx` snippet does not show `AIChat` being imported or rendered. This creates a critical inconsistency: either the component is intended to be used and its integration is missing/incomplete, or the documentation is stale and the component is indeed dead code.
```

### Step 2: تأیید قصد پیاده‌سازی قابلیت QA از طریق AIChat
**Status:** `pending` (0%)
**Scope:** با توجه به تمرکز پروژه بر ویژگی‌های هوش مصنوعی، این مرحله شامل تأیید صریح تصمیم برای ادغام و فعال‌سازی قابلیت 'Question & Answer' (QA) با استفاده از کامپوننت `AIChat` است. این تصمیم به طور رسمی پیشنهاد حذف کامپوننت در `task-06f17cca` را نادیده می‌گیرد و مسیر را برای ادغام آن هموار می‌کند. این مرحله یک تصمیم استراتژیک است که بر اساس اولویت‌های پروژه گرفته می‌شود.
**Excerpt:**
```
Given the project's focus on AI features, it's highly probable that the `QA` functionality is intended.
```

### Step 3: ادغام کامپوننت AIChat در رابط کاربری برنامه
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `App.jsx` (یا هر کامپوننت والد مناسب دیگری که مسئول render کردن بخش‌های اصلی UI است) برای import کردن کامپوننت `AIChat` از `src/components/AIWidgets.jsx` و سپس render کردن آن در مکان مناسبی در ساختار UI برنامه است. هدف این است که کامپوننت `AIChat` به طور فعال در برنامه نمایش داده شود و برای کاربران قابل دسترسی باشد تا قابلیت QA را ارائه دهد.
**Excerpt:**
```
Furthermore, the provided `App.jsx` snippet does not show `AIChat` being imported or rendered. This creates a critical inconsistency: either the component is intended to be used and its integration is missing/incomplete...
```

### Step 4: به‌روزرسانی مستندات docs/ai-chat-use-cases.md
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازبینی و به‌روزرسانی فایل `docs/ai-chat-use-cases.md` است تا اطمینان حاصل شود که به طور دقیق وضعیت فعلی کامپوننت `AIChat` را به عنوان یک سطح UI فعال برای مورد استفاده 'Question & Answer' (QA) منعکس می‌کند. هرگونه اشاره به اینکه کامپوننت 'unused' یا 'dead code' است باید حذف شود و توضیحات باید با ادغام واقعی آن در برنامه همخوانی داشته باشد.
**Excerpt:**
```
The `AIChat` component is defined in `src/components/AIWidgets.jsx` (lines 63-107) and explicitly listed in `docs/ai-chat-use-cases.md` as the UI surface for the `QA` (Question & Answer) use case. ... or the documentation is stale and the component is indeed dead code.
```

### Step 5: مستندسازی حل ناسازگاری task-06f17cca
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اضافه کردن یک یادداشت یا کامنت در مکان‌های مرتبط (مانند نزدیکی تعریف کامپوننت `AIChat` در `src/components/AIWidgets.jsx`، یا در یک فایل `README.md` پروژه، یا یک فایل تصمیم‌گیری معماری) است که توضیح می‌دهد چرا پیشنهاد قبلی `task-06f17cca` مبنی بر حذف `AIChat` نادیده گرفته شده و تصمیم به ادغام آن گرفته شده است. هدف این است که سابقه تاریخی این تصمیم و حل ناسازگاری به وضوح مستند شود.
**Excerpt:**
```
However, a previous task (`task-06f17cca`, marked 'merged') suggested it was unused and should be removed. ... This creates a critical inconsistency: either the component is intended to be used and its integration is missing/incomplete, or the documentation is stale and the component is indeed dead code.
```
