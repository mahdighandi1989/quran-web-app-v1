---
task_id: 1aa5faf9-41b2-4cdc-83cf-fc63ba8e4416
title: وابستگی دایره‌ای بین `src/lib/firebase.js` و `src/lib/telegramStore.js`.
type: bug
priority: high
...
# وابستگی دایره‌ای بین `src/lib/firebase.js` و `src/lib/telegramStore.js`.

## Raw Idea

نقشهٔ Importهای داخلی نشان می‌دهد که `src/lib/firebase.js`، `src/lib/telegramStore.js` را import می‌کند، و در عین حال `src/lib/telegramStore.js` نیز `src/lib/firebase.js` را import می‌کند. این یک وابستگی دایره‌ای است که می‌تواند منجر به مشکلات زمان اجرا (مانند `undefined` شدن ماژول‌ها در زمان بارگذاری)، دشواری در تست‌نویسی، و پیچیدگی در درک جریان داده شود. این الگو معمولاً نشان‌دهندهٔ طراحی نامناسب ماژول‌ها است که مسئولیت‌هایشان به درستی تفکیک نشده‌اند.
...
- `src/lib/firebase.js:نامشخص (فایل ارائه نشده است)` — `import of telegramStore`
  ```jsx
  import { telegramStore } from './telegramStore.js';
  ```
- `src/lib/telegramStore.js:نامشخص (فایل ارائه نشده است)` — `import of firebase.js`
  ```jsx
...
JavaScript module system, Firebase, React application state.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/App.jsx` — احتمالاً هر دو ماژول را مقداردهی اولیه کرده و استفاده می‌کند.
- `src/components/TelegramSettings.jsx` — با منطق ذخیره‌سازی تلگرام تعامل دارد.
- `src/App.test.jsx` — تست‌های مربوط به منطق Firebase را شامل می‌شود.
- `src/test/telegramStore.test.js` — تست‌های مربوط به منطق ذخیره‌سازی تلگرام را شامل می‌شود.
- `src/lib/analytics.js` — `firebase.js` این فایل را import می‌کند
- `src/lib/aiStore.js` — این فایل `firebase.js` را import می‌کند (caller)
- `src/lib/appStateStore.js` — این فایل `firebase.js` را import می‌کند (caller)
- `src/lib/telegramStore.js` — این فایل `firebase.js` را import می‌کند (caller)
- `src/lib/firebase.js` — `telegramStore.js` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این یک مشکل معماری اصلی است که بر نحوه مدیریت state و persistence مربوط به Firebase و Telegram تأثیر می‌گذارد. حل آن نیاز به بازسازی دقیق برای اطمینان از مرزهای صحیح ماژول و جریان داده دارد.
...
- [ ] وابستگی دایره‌ای بین `src/lib/firebase.js` و `src/lib/telegramStore.js` برطرف می‌شود.
- [ ] تمام تست‌های مربوط به این ماژول‌ها (مانند `src/test/telegramStore.test.js`، `src/App.test.jsx`) با موفقیت اجرا می‌شوند.
- [ ] یکپارچه‌سازی Firebase و Telegram در برنامه به درستی کار می‌کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. وابستگی دایره‌ای را با بازطراحی مسئولیت‌های این دو ماژول برطرف کنید. به عنوان مثال، اگر `telegramStore.js` برای ذخیره‌سازی/بازیابی داده‌ها از Firebase نیاز به `firebase.js` دارد، باید `firebase.js` را به عنوان یک dependency به آن تزریق کرد یا یک ماژول واسط برای مدیریت تعاملات Firebase ایجاد کرد. یا اگر `firebase.js` نیاز به `telegramStore.js` دارد، باید دلیل آن بررسی شود و وابستگی معکوس حذف شود.
...
**بعد (مفهومی - telegramStore Firebase را به عنوان dependency دریافت می‌کند)**

_بعد:_
```
// src/lib/firebase.js
// No import of telegramStore
// ...
// src/lib/telegramStore.js
// import { getFirestore } from 'firebase/firestore'; // یا دریافت از طریق پارامتر
```
...
## 🧪 دستورات اعتبارسنجی
- `npm test`
- `npm run dev (و بررسی دستی تنظیمات تلگرام و عملکرد برنامه)`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال ایجاد رگرسیون در یکپارچه‌سازی Firebase یا Telegram در صورت عدم بازسازی دقیق.
