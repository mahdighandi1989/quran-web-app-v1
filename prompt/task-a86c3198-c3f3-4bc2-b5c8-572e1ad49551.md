---
task_id: a86c3198-c3f3-4bc2-b5c8-572e1ad49551
title: 'Incomplete Feature: Web Speech API integration missing from UI'
...
created_at: '2026-06-12T19:13:40.394555+00:00'
updated_at: '2026-06-12T19:30:31.959517+00:00'
target_files:
- src/App.jsx
- src/components/AIWidgets.jsx
---

# Incomplete Feature: Web Speech API integration missing from UI

## Raw Idea

The `README.md` (line 26) mentions 'برای تشخیص گفتار (🎙️) مرورگر باید Web Speech API را پشتیبانی کند,' implying speech recognition is a supported feature. Furthermore, `task-38df6aee` (marked 'merged') was titled 'پیاده‌سازی تشخیص گفتار و لود داینامیک XLSX' and should have addressed this. However, no UI component in the provided files (`AIExamGenerator.jsx`, `AIWidgets.jsx`, `TelegramSettings.jsx`) or `index.html` shows any visible integration or usage of the Web Speech API (e.g., a microphone button, speech input field). This indicates that the speech recognition feature is either incomplete, not integrated into the user interface, or the previous task was incorrectly marked as fully merged for this specific part.
...
- `src/App.jsx` — Main application component, a central place to add a global speech input toggle or integrate into existing input fields.
- `src/components/AIWidgets.jsx` — The `AIChat` component (lines 101-107) has an input field where speech input would be highly beneficial.
...
- `prompt/archive/task-38df6aee-852b-40d1-a5a9-4c3b7e19c71d.md` — Previous task that was supposed to implement this feature.
- `src/lib/aiClient.js` — Speech input could be used to generate prompts for AI interactions.
...
## 🌐 نقشهٔ وابستگی‌ها
This is a frontend feature implementation. It relies on the browser's Web Speech API. No direct backend changes are immediately required, but speech input could enhance existing AI interactions.
...
- [ ] A visible UI element (e.g., a microphone icon) is added to a relevant part of the application (e.g., next to a text input field in `AIChat`).
- [ ] Clicking the UI element initiates Web Speech API recognition.
- [ ] Recognized speech is converted to text and populates the associated input field or triggers a relevant action.
- [ ] The speech recognition feature works correctly in browsers that support the Web Speech API.
- [ ] The status of `task-38df6aee` is re-evaluated to reflect the completion of the speech recognition part.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Implement a UI element (e.g., a microphone button) in a relevant component (e.g., `src/App.jsx` or a new component) that utilizes the Web Speech API for speech recognition. This would allow users to input text via voice, fulfilling the documented feature. Consider integrating it with existing text input fields, such as the AI chat input in `AIChat`.

## 💡 نمونه‌های قبل/بعد
**Current UI (no speech input)**

_قبل:_
```
// No visible microphone icon or speech input field in the application's UI.
```

**UI with speech input (example for AIChat)**
...
<input type="text" value={input} placeholder="پرسش خود دربارهٔ قرآن را بنویسید و Enter بزنید…" aria-label="متن سوال شما"
        ...
        <button className="btn-icon" title="ورودی صوتی" onClick={startSpeechRecognition}>🎙️</button>
...
Browser compatibility for Web Speech API can vary, requiring graceful degradation or polyfills.
...
1. A visible UI element (e.g., a microphone icon) is added to a relevant part of the application (e.g., next to a text input field in `AIChat`). _(verify: ui_interaction)_
2. Clicking the UI element initiates Web Speech API recognition. _(verify: ui_interaction)_
3. Recognized speech is converted to text and populates the associated input field or triggers a relevant action. _(verify: ui_interaction)_
4. The speech recognition feature works correctly in browsers that support the Web Speech API. _(verify: ui_interaction)_
5. The status of `task-38df6aee` is re-evaluated to reflect the completion of the speech recognition part. _(verify: manual_only)_

## Task Steps

### Step 1: بررسی و تأیید پیاده‌سازی موجود Web Speech API
**Status:** `pending` (0%)
**Scope:** این مرحله شامل جستجوی دقیق در فایل‌های ذکر شده و فایل‌های مرتبط دیگر برای تعیین اینکه آیا پیاده‌سازی جزئی یا کامل از Web Speech API از قبل وجود دارد یا خیر، می‌شود. هدف شناسایی توابع، کلاس‌ها یا شنونده‌های رویداد موجود مرتبط با تشخیص گفتار است، حتی اگر در حال حاضر به UI متصل نباشند. این مرحله برای جلوگیری از کار تکراری و استفاده از هر کد موجود از `task-38df6aee` حیاتی است و شامل نوشتن کد جدید نیست، بلکه صرفاً فاز تحقیقاتی است.
**Excerpt:**
```
The `README.md` (line 26) mentions 'برای تشخیص گفتار (🎙️) مرورگر باید Web Speech API را پشتیبانی کند,' implying speech recognition is a supported feature. Furthermore, `task-38df6aee` (marked 'merged') was titled 'پیاده‌سازی تشخیص گفتار و لود داینامیک XLSX' and should have addressed this. However, no UI component in the provided files (`AIExamGenerator.jsx`, `AIWidgets.jsx`, `TelegramSettings.jsx`) or `index.html`) shows any visible integration or usage of the Web Speech API (e.g., a microphone button, speech input field).
```

### Step 2: پیاده‌سازی کامپوننت UI دکمه میکروفون
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد یک کامپوننت UI جدید و قابل مشاهده، به طور خاص یک دکمه میکروفون، در یکی از فایل‌های اصلی UI تمرکز دارد. این دکمه به عنوان نقطه ورود کاربر برای شروع تشخیص گفتار عمل خواهد کرد. کامپوننت باید به درستی استایل‌دهی شده و آماده دریافت کنترل‌کننده‌های رویداد برای شروع و توقف فرآیند تشخیص گفتار باشد. مکان دقیق بر اساس زمینه برنامه، احتمالاً در `AIExamGenerator.jsx` یا `AIWidgets.jsx` که انتظار ورودی کاربر می‌رود، تعیین خواهد شد.
**Excerpt:**
```
no UI component in the provided files (`AIExamGenerator.jsx`, `AIWidgets.jsx`, `TelegramSettings.jsx`) or `index.html`) shows any visible integration or usage of the Web Speech API (e.g., a microphone button, speech input field).
```

### Step 3: یکپارچه‌سازی Web Speech API با کامپوننت UI
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اتصال دکمه میکروفون تازه ایجاد شده (از مرحله ۲) به Web Speech API مرورگر است. این شامل مقداردهی اولیه شیء `SpeechRecognition`، اتصال شنونده‌های رویداد به دکمه میکروفون (به عنوان مثال، `onClick` برای شروع تشخیص) و مدیریت رویداد `onresult` برای گرفتن گفتار رونویسی شده است. این مرحله پیوند عملکردی اصلی بین عمل کاربر و قابلیت‌های تشخیص گفتار مرورگر را برقرار می‌کند.
**Excerpt:**
```
no UI component in the provided files (`AIExamGenerator.jsx`, `AIWidgets.jsx`, `TelegramSettings.jsx`) or `index.html`) shows any visible integration or usage of the Web Speech API (e.g., a microphone button, speech input field). This indicates that the speech recognition feature is either incomplete, not integrated into the user interface...
```

### Step 4: نمایش گفتار رونویسی شده در فیلدهای ورودی
**Status:** `pending` (0%)
**Scope:** این مرحله بر گرفتن متن رونویسی شده از Web Speech API (از مرحله ۳) و پر کردن آن در فیلدهای ورودی متن مربوطه در UI برنامه تمرکز دارد. این شامل شناسایی فیلد(های) ورودی هدف است که انتظار ورودی گفتار در آن می‌رود (به عنوان مثال، یک ناحیه متنی برای سوالات امتحان یا ورودی ویجت) و به روز رسانی برنامه‌نویسی مقدار آنها با گفتار تشخیص داده شده. هدف این است که ورودی گفتار مستقیماً توسط برنامه قابل استفاده باشد.
**Excerpt:**
```
no UI component in the provided files... shows any visible integration or usage of the Web Speech API (e.g., a microphone button, speech input field).
```

### Step 5: پیاده‌سازی بازخورد کاربر و مدیریت خطا برای تشخیص گفتار
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بهبود تجربه کاربری با ارائه بازخورد بصری و متنی واضح در طول فرآیند تشخیص گفتار و مدیریت قوی خطاهای احتمالی است. این شامل نمایش پیام‌هایی مانند 'در حال گوش دادن...', 'در حال پردازش...', 'تشخیص گفتار پشتیبانی نمی‌شود' یا 'مجوز میکروفون رد شد' است. همچنین شامل مدیریت صحیح رویدادهای `onerror` از API `SpeechRecognition` و اطلاع‌رسانی به کاربر در مورد مشکلات است.
**Excerpt:**
```
This indicates that the speech recognition feature is either incomplete, not integrated into the user interface...
```

### Step 6: به‌روزرسانی README.md و مستندات مرتبط
**Status:** `pending` (0%)
**Scope:** این مرحله تضمین می‌کند که تمام مستندات پروژه، به ویژه `README.md` (خط ۲۶)، به طور دقیق ویژگی تشخیص گفتار کاملاً یکپارچه و کاربردی را منعکس می‌کند. این شامل تأیید صحت اشاره به پشتیبانی از Web Speech API، افزودن دستورالعمل‌هایی در مورد نحوه استفاده از دکمه میکروفون جدید و جزئیات هرگونه الزامات سازگاری مرورگر یا محدودیت‌های شناخته شده است. اگر `task-38df6aee` مستنداتی داشت، باید برای سازگاری نیز بررسی شود.
**Excerpt:**
```
The `README.md` (line 26) mentions 'برای تشخیص گفتار (🎙️) مرورگر باید Web Speech API را پشتیبانی کند,' implying speech recognition is a supported feature.
```
