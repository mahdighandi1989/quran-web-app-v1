...
updated_at: '2026-06-06T06:26:20.901155+00:00'
tags:
- consolidated
- post_verify_merge
---
...
📌 دلیل تلفیق (rationale توسط AI): این خوشه شامل تمامی تسک‌های مرتبط با مشکلات پیکربندی پروژه، سیستم بیلد، و زیرساخت تست است. از جمله عدم وجود اسکریپت تست، نیاز به افزودن تست برای فایل‌های بدون پوشش، به‌روزرسانی تست‌ها، عدم تطابق نام فایل‌های تست، ناسازگاری نسخه‌های Node.js و پکیج‌ها، حذف وابستگی‌های تست بلااستفاده، و رسیدگی به فایل‌های orphan (مانند vitest.config.js و main.jsx).
🎯 theme: رفع مشکلات زیرساخت بیلد و تست پروژه
💎 estimated_difficulty: large

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 1 از 9
  id: 1de7a5cf-5174-4678-9e4b-4dcd7e3cc963
  عنوان اصلی: عدم وجود اسکریپت تست در package.json ریشه
  اولویت اصلی: high
  وضعیت verify قبلی: error
  فایل‌های دخیل: package.json

📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - linter بدون warning عبور می‌کند [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - type-check موفق است [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
عدم وجود اسکریپت تست در package.json ریشه

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `package.json:5-8` — `scripts` — بخش scripts فقط start دارد و فاقد test است
  ```json
  "scripts": {
...
- `vitest.config.js` (سطر 1) — پیکربندی Vitest موجود است
- `README.md` (سطر 1) — مستندات به npm test اشاره می‌کند

## 🌐 نقشهٔ وابستگی‌ها
vitest و وابستگی‌های آن در

## 🔍 Context و وضعیت فعلی
package.json ریشه هیچ اسکریپت test یا test:watch ندارد، در حالی که README.md دستورات `npm test` و `npm run test:watch` را مستند کرده است. همچنین vitest.config.js و فایل‌های تست متعدد (بیش از 20 فایل در src/test/) وجود دارند. نبود اسکریپت تست باعث می‌شود کاربر نتواند تست‌ها را اجرا کند و CI pipeline نیز کار نمی‌کند. شواهد: README.md خطوط مربوط به تست، وجود vitest.config.js و فایل‌های src/test/*.test.*
...
1. افزودن اسکریپت‌های "test": "vitest run" و "test:watch": "vitest" به بخش scripts در package.json ریشه.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 9
  id: 781d5344-4778-4eb8-ad73-1de87dfc5a01
  عنوان اصلی: افزودن تست برای فایل‌های source بدون پوشش
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/lib/aiClient.js, src/lib/aiProviders.js, src/lib/aiStore.js, src/lib/appStateStore.js, src/lib/telegramStore.js

📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "tests/", "timeout_seconds": 300}]
  - linter بدون warning عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "lint", "timeout_seconds": 120}]
  - type-check موفق است [verify_method=backend_test] [verify_plan={"test_node": "type_check", "timeout_seconds": 120}]
...
## 🎯 هدف (خلاصه ساختاریافته)
فایل‌های source بدون تست متناظر: aiClient.js, aiProviders.js, aiStore.js, appStateStore.js, telegramStore.js

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/lib/aiClient.js:1-200` — `chatComplete, isAIReady, resolveActiveAI` — هیچ فایل تستی برای این ماژول وجود ندارد
  ```jsx
  export async function chatComplete(provider, key, model, messages, opts = {}) {
    // ...
  }
  ```
- `src/lib/aiProviders.js:1-150` — `allProviders, getProviderById, validateProviderKey, validateModel` — هیچ فایل تستی برای این ماژول وجود ندارد
  ```jsx
  export function allProviders(ai) {
    return [...(ai.customProviders || []), ...BUILTIN_PROVIDERS];
  }
  ```
- `src/lib/aiStore.js:1-80` — `loadAIConfig, saveAIConfig` — هیچ فایل تستی برای این ماژول وجود ندارد
  ```jsx
  export async function loadAIConfig(uid) {
    // ...
  }
  ```
- `src/lib/appStateStore.js:1-60` — `mirrorAppState, getAppState` — هیچ فایل تستی برای این ماژول وجود ندارد
  ```jsx
  export async function mirrorAppState(uid, state) {
    // ...
  }
  ```
- `src/lib/telegramStore.js:1-90` — `loadTelegramConfig, saveTelegramConfig` — هیچ فایل تستی برای این ماژول وجود ندارد
  ```jsx
...
- `src/lib/aiClient.js` (سطر 1) — hub با ۵ مصرف‌کننده: AIExamGenerator, AIWidgets, App, telegramCommands, aiClient.test
- `src/lib/aiProviders.js` (سطر 1) — hub با ۵ مصرف‌کننده: App, aiClient, aiStore, AISettings.test, aiClient.test
- `src/lib/aiStore.js` (سطر 1) — hub با ۲ مصرف‌کننده: App, aiStore.test
- `src/lib/appStateStore.js` (سطر 1) — hub با ۲ مصرف‌کننده: App, appStateStore.test
- `src/lib/telegramStore.js` (سطر 1) — hub با ۲ مصرف‌کننده: App, telegramStore.test
...
## 🌐 نقشهٔ وابستگی‌ها
این ۵ ماژول hub هستند و توسط ۲ تا ۵ فایل دیگر مصرف می‌شوند. aiClient.js و aiProviders.js بحرانی‌ترین‌اند چون AI flow کل برنامه به آن‌ها وابسته است.

## 🔍 Context و وضعیت فعلی
پنج فایل lib کلیدی (hub) که توسط ۲ تا ۵ فایل دیگر import می‌شوند، فایل تست اختصاصی ندارند. aiClient.js (hub با ۵ مصرف‌کننده) و aiProviders.js (hub با ۵ مصرف‌کننده) بحرانی‌ترین‌اند. aiStore.js و appStateStore.js و telegramStore.js نیز hub هستند و تست ندارند. این شکاف باعث می‌شود تغییرات در این ماژول‌ها بدون پوشش تست باقی بماند و رگرسیون در AI flow، state management و Telegram integration تشخیص داده نشود.
...
## 🪜 مراحل اجرایی پیشنهادی
1. برای هر ۵ فایل، یک فایل تست جدید در src/test/ ایجاد شود که توابع عمومی هر ماژول را با mock کردن وابستگی‌های خارجی (Firebase, fetch) پوشش دهد.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 9
  id: 686d4918-6b4a-4526-9712-e0f3d43da152
  عنوان اصلی: 🔄 App.test.jsx نیازمند بررسی به‌خاطر تغییر در App.jsx
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/App.test.jsx

📋 acceptance_criteria کامل:
  - diff `src/App.jsx` بررسی شد و تأثیر بر `src/App.test.jsx` مستند شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - کد dependent با contract جدید align شد [verify_method=backend_test] [verify_plan={"test_node": "src/App.test.jsx", "timeout_seconds": 60}]
  - integration test که هر دو فایل را پوشش می‌دهد عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "src/App.test.jsx", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
🔄 App.test.jsx نیازمند بررسی به‌خاطر تغییر در App.jsx
...
- `src/App.jsx` — فایل upstream که تغییر کرده
- `src/lib/arabic.js` — `App.test.jsx` این فایل را import می‌کند
- `src/lib/format.js` — `App.test.jsx` این فایل را import می‌کند
- `src/lib/firebase.js` — `App.test.jsx` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
فایل `src/App.test.jsx` یکی از مصرف‌کنندگان `src/App.jsx` است.
...
## 🔍 جزئیات
- 📂 فایل تغییریافته: `src/App.jsx`
- 📂 فایل وابسته (نیازمند بررسی): `src/App.test.jsx`
- 🔥 risk level: **high**
- 📝 reason: This file directly tests the `App` component. A change in the component's signature (props) will likely break existing tests that rely on the old prop structure or expect specific props.
...
- [ ] diff `src/App.jsx` بررسی شد و تأثیر بر `src/App.test.jsx` مستند شد
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: diff آخر در `src/App.jsx` را بخوان — چی عوض شده.
گام ۲: کد در `src/App.test.jsx` که این تغییر را مصرف می‌کند پیدا کن.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 4 از 9
  id: 4f215fc7-b482-4b4f-a651-fde4ebf00a1e
  عنوان اصلی: عدم تطابق نام پکیج در package.json ریشه و package.json سرور
  اولویت اصلی: medium
  وضعیت verify قبلی: error
  فایل‌های دخیل: package.json, server/package.json

📋 acceptance_criteria کامل:
  - package.json ریشه name="quran-web-app" داشته باشد [verify_method=static] [verify_plan={"grep_patterns": ["\"name\": \"quran-web-app\""], "files_hint": ["package.json"]}]
  - server/package.json name="quran-telegram-bot" باقی بماند [verify_method=static] [verify_plan={"grep_patterns": ["\"name\": \"quran-telegram-bot\""], "files_hint": ["server/package.json"]}]
  - npm install در ریشه فقط وابستگی‌های فرانت‌اند را نصب کند [verify_method=static] [verify_plan={"grep_patterns": ["\"firebase-admin\""], "files_hint": ["package.json"], "negate_match": true}]
...
## 🎯 هدف (خلاصه ساختاریافته)
عدم تطابق نام پکیج در package.json ریشه و package.json سرور

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `package.json:2` — `name` — نام پکیج ریشه باید quran-web-app باشد نه quran-telegram-bot
  ```json
  "name": "quran-telegram-bot"
  ```
- `server/package.json:2` — `name` — نام پکیج سرور درست است
  ```json
  "name": "quran-telegram-bot"
  ```
...
Node.js + npm monorepo با دو سرویس مجزا

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `package.json` (سطر 2) — فایل اصلی package.json که name اشتباه دارد
- `server/package.json` (سطر 2) — فایل package.json سرور که name درست دارد

## 🌐 نقشهٔ وابستگی‌ها
این دو package.json مستقل هستند و هیچ وابستگی متقابلی ندارند. تغییر name در ریشه روی سرور تأثیر نمی‌گذارد.

## 🔍 Context و وضعیت فعلی
فایل package.json در ریشه پروژه دارای name="quran-web-app" است، در حالی که فایل package.json در پوشه server/ دارای name="quran-telegram-bot" است. این دو package.json متعلق به دو سرویس متفاوت (فرانت‌اند و سرور بات تلگرام) هستند و نباید با یک package.json مدیریت شوند. این باعث سردرگمی در نصب وابستگی‌ها و اجرای دستورات npm می‌شود. همچنین در package.json ریشه، وابستگی firebase-admin وجود ندارد در حالی که در server/package.json وجود دارد.

## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
verify می‌تواند پیاده‌سازی متفاوت ولی هم‌ارز را قبول کند.

- [ ] package.json ریشه name="quran-web-app" داشته باشد
- [ ] server/package.json name="quran-telegram-bot" باقی بماند
- [ ] npm install در ریشه فقط وابستگی‌های فرانت‌اند را نصب کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. فایل package.json ریشه را به نام مناسب (quran-web-app) نگه دارید و مطمئن شوید که وابستگی‌های سرور (firebase-admin) در server/package.json باقی بمانند. همچنین دستورات npm در ریشه نباید به سرور مربوط باشند.

## 💡 نمونه‌های قبل/بعد
**اصلاح name در package.json ریشه**
...
## ⚠️ ریسک‌ها و موارد احتیاط
بدون ریسک - فقط تغییر نام metadata
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 5 از 9
  id: 92e89378-33b7-4a32-ade8-d4ac2fb6a7d8
  عنوان اصلی: نسخه Node.js ناسازگار بین package.json ریشه و .node-version
  اولویت اصلی: medium
  وضعیت verify قبلی: error
  فایل‌های دخیل: .node-version

📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "tests/", "timeout_seconds": 180}]
  - linter بدون warning عبور می‌کند [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - type-check موفق است [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
## 🎯 هدف (خلاصه ساختاریافته)
نسخه Node.js ناسازگار بین package.json ریشه و .node-version

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `.node-version:1` — `node_version`
  ```
  20
  ```
...
## 🔍 Context و وضعیت فعلی
فایل package.json ریشه در بخش engines نیاز به node >=20.0.0 دارد، در حالی که فایل .node-version (که توسط ابزارهایی مانند nvm, fnm, asdf استفاده می‌شود) مشخص نیست چه نسخه‌ای را تعیین می‌کند. همچنین server/package.json نیاز به node >=18 دارد. این ناسازگاری می‌تواند باعث شود توسعه‌دهندگان از نسخه‌های مختلف Node.js استفاده کنند و رفتارهای متفاوتی ببینند.
...
## 🪜 مراحل اجرایی پیشنهادی
1. فایل .node-version را با نسخه‌ای که با engines در package.json ریشه هماهنگ است (>=20.0.0) به‌روز کنید. همچنین مطمئن شوید که server/package.json نیز با نسخه مناسب هماهنگ است.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 6 از 9
  id: f1e5bc0f-9e12-4b32-8aeb-66a3ac35e9c5
  عنوان اصلی: حذف @testing-library/user-event بلااستفاده
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - `@testing-library/user-event` در هیچ direct import مصرف نمی‌شود + هیچ transitive dep ای آن را require نمی‌کند [verify_method=static] [verify_plan={"grep_patterns": ["import\\s+.*from\\s+['\"]@testing-library/user-event['\"]", "require\\(['\"]@testing-library/user-event['\"]\\)"], "files_hint": ["frontend/**/*.js", "frontend/**/*.ts", "frontend/]
  - package از manifest حذف شد + lockfile به‌روز [verify_method=static] [verify_plan={"grep_patterns": ["\"@testing-library/user-event\""], "files_hint": ["package.json", "package-lock.json", "yarn.lock"], "expected_absence": true}]
  - build/test/CI همچنان عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/frontend/test_all.py::test_ci_passes_after_dep_removal", "timeout_seconds": 300}]
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
package `@testing-library/user-event` در `` declare شده ولی در هیچ import کدبیس مصرف نمی‌شود.

## 🤔 چرا مهم است
dependency بلااستفاده باعث: (۱) bundle size بزرگ، (۲) attack surface بیشتر برای vulnerabilities، (۳) supply chain risk.

## 🔍 جزئیات
- نام package: `@testing-library/user-event`
- زبان: `node`
- علت: in package.json but no import for '@testing-library/user-event' found

## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
verify می‌تواند پیاده‌سازی متفاوت ولی هم‌ارز را قبول کند.

- [ ] `@testing-library/user-event` در هیچ direct import مصرف نمی‌شود + هیچ transitive dep ای آن را require نمی‌کند
...
## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: `npm/pip list --depth=0` و grep import روی `@testing-library/user-event`.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 7 از 9
  id: e51ad6a2-440a-4ff3-a20f-add81484bbcd
  عنوان اصلی: 🆕 فایل جدید `vitest.config.js` به هیچ‌جا متصل نیست (orphan)
  اولویت اصلی: medium
  وضعیت verify قبلی: error
  فایل‌های دخیل: vitest.config.js

📋 acceptance_criteria کامل:
  - بررسی شد چه فایلی باید این فایل را import کند [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - wiring اضافه شد یا فایل حذف شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - scan بعدی این فایل را orphan گزارش نمی‌کند [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
## 🎯 هدف (خلاصه ساختاریافته)
🆕 فایل جدید `vitest.config.js` به هیچ‌جا متصل نیست (orphan)
...
## 🌐 نقشهٔ وابستگی‌ها
فایل جدید `vitest.config.js` بدون companion.
...
## 🪜 مراحل اجرایی پیشنهادی
1. بررسی کن فایل `vitest.config.js` به چه companion ای نیاز دارد و آن را ایجاد یا متصل کن.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 8 از 9
  id: a8cf86b8-37dc-43a7-b284-cb08fe81076f
  عنوان اصلی: 🆕 فایل جدید `main.jsx` به هیچ‌جا متصل نیست (orphan)
  اولویت اصلی: medium
  وضعیت verify قبلی: error
  فایل‌های دخیل: src/main.jsx

📋 acceptance_criteria کامل:
  - بررسی شد چه فایلی باید این فایل را import کند [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - wiring اضافه شد یا فایل حذف شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - scan بعدی این فایل را orphan گزارش نمی‌کند [verify_method=static] [verify_plan={"grep_patterns": ["import\\s+.*\\s+from\\s+['\"](?:\\.{1,2}/)*main\\.jsx['\"]", "import\\s+['\"](?:\\.{1,2}/)*main\\.jsx['\"]"], "files_hint": ["src/**/*.{js,jsx,ts,tsx}"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
🆕 فایل جدید `main.jsx` به هیچ‌جا متصل نیست (orphan)
...
- `src/routes.jsx` — `main.jsx` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
فایل جدید `src/main.jsx` بدون companion.
...
## 🪜 مراحل اجرایی پیشنهادی
1. بررسی کن فایل `src/main.jsx` به چه companion ای نیاز دارد و آن را ایجاد یا متصل کن.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 9 از 9
  id: e9060c31-615d-4531-839c-695d3ebb4cd2
  عنوان اصلی: اتصال یا حذف `appStateStore.test.js`
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/test/appStateStore.test.js
...
## 🎯 هدف (خلاصه ساختاریافته)
🆕 فایل جدید `appStateStore.test.js` به هیچ‌جا متصل نیست (orphan)

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/test/appStateStore.test.js`
...
- `src/lib/appStateStore.js` — `appStateStore.test.js` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
فایل جدید `src/test/appStateStore.test.js` بدون companion.
...
## 🔍 جزئیات
- 📂 added: `src/test/appStateStore.test.js`
- 🔥 risk: **medium**
- 📝 reason: این فایل جدید اضافه شده ولی هیچ فایل دیگری آن را import نمی‌کند — احتمالاً wiring فراموش شده یا dead-on-arrival است.
...
## 🪜 مراحل اجرایی پیشنهادی
1. بررسی کن فایل `src/test/appStateStore.test.js` به چه companion ای نیاز دارد و آن را ایجاد یا متصل کن.
...
- در commit message: `merged-from: 1de7a5cf-5174-4678-9e4b-4dcd7e3cc963, 781d5344-4778-4eb8-ad73-1de87dfc5a01, 686d4918-6b4a-4526-9712-e0f3d43da152, 4f215fc7-b482-4b4f-a651-fde4ebf00a1e, 92e89378-33b7-4a32-ade8-d4ac2fb6a7d8, f1e5bc0f-9e12-4b32-8aeb-66a3ac35e9c5, e51ad6a2-440a-4ff3-a20f-add81484bbcd, a8cf86b8-37dc-43a7-b284-cb08fe81076f, e9060c31-615d-4531-839c-695d3ebb4cd2`
...
1. اعمال تغییر بدون شکستن تست‌های موجود _(verify: manual_only)_
2. linter بدون warning عبور می‌کند _(verify: manual_only)_
3. type-check موفق است _(verify: manual_only)_
4. diff `src/App.jsx` بررسی شد و تأثیر بر `src/App.test.jsx` مستند شد _(verify: manual_only)_
...
7. package.json ریشه name="quran-web-app" داشته باشد _(verify: static)_
8. server/package.json name="quran-telegram-bot" باقی بماند _(verify: static)_
9. npm install در ریشه فقط وابستگی‌های فرانت‌اند را نصب کند _(verify: static)_
10. `@testing-library/user-event` در هیچ direct import مصرف نمی‌شود + هیچ transitive dep ای آن را require نمی‌کند _(verify: static)_
11. package از manifest حذف شد + lockfile به‌روز _(verify: static)_
12. build/test/CI همچنان عبور می‌کند _(verify: backend_test)_
13. بررسی شد چه فایلی باید این فایل را import کند _(verify: manual_only)_
14. wiring اضافه شد یا فایل حذف شد _(verify: manual_only)_
15. scan بعدی این فایل را orphan گزارش نمی‌کند _(verify: manual_only)_
