---
task_id: 25432bf2-37d5-4650-a14a-7c1a620f7ca1
title: ابهام در نقش و نقطه ورود بک‌اند پایتون (`backend/`)
...
created_at: '2026-06-12T19:19:31.695774+00:00'
updated_at: '2026-06-12T19:35:40.780697+00:00'
target_files:
- backend/
- pyproject.toml
- README.md
...
# ابهام در نقش و نقطه ورود بک‌اند پایتون (`backend/`)

## Raw Idea

دایرکتوری `backend/` شامل فایل‌های پایتون است (`__init__.py`, `app/handlers/scan_failure_handler.py`, `app/notification_pipeline.py`, `app/tasks/scan_tasks.py`). با این حال، هیچ نقطه ورود واضحی (مانند `main.py`, `app.py` با یک فریم‌ورک وب مانند Flask/FastAPI) یا `requirements.txt` (فقط `pyproject.toml` برای pytest) برای نشان دادن یک بک‌اند وب پایتون کاملاً کاربردی وجود ندارد. این ابهام در مورد نقش بک‌اند پایتون، استقرار و عملکرد صحیح آن را دشوار می‌کند. ممکن است یک نگهدارنده مکان یا یک پیاده‌سازی ناقص باشد.
...
- `backend/:نامشخص` — `New entry point file`
  ```
  # Create backend/main.py or similar
  ```
- `pyproject.toml:نامشخص` — `tool.poetry.dependencies`
  ```
  # Add runtime dependencies here
  ```
- `README.md:نامشخص` — `Backend setup section`
  ```markdown
  # Update documentation for Python backend setup and deployment
  ```
...
بک‌اند پایتون، فریم‌ورک‌های وب (FastAPI/Flask)، وظایف پس‌زمینه، استقرار.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `render.yaml` — پیکربندی استقرار، ممکن است برای بک‌اند پایتون نیاز به به‌روزرسانی داشته باشد.
- `server/telegram-bot.mjs` — اگر بات Node.js نیاز به تعامل با بک‌اند پایتون دارد.
- `tests/backend/` — تست‌های پایتون موجود.

## 🌐 نقشهٔ وابستگی‌ها
وضعیت فعلی بک‌اند پایتون مبهم است و به طور بالقوه منجر به مشکلات استقرار یا عدم تحقق عملکرد می‌شود. روشن کردن نقش آن و ارائه یک نقطه ورود واضح برای ادغام آن در سیستم کلی ضروری است.
...
- [ ] یک نقطه ورود واضح برای بک‌اند پایتون ایجاد شده است (مثلاً `backend/main.py` برای یک سرویس وب، یا یک اسکریپت worker).
- [ ] وابستگی‌های زمان اجرا در `requirements.txt` یا `pyproject.toml` لیست شده‌اند.
- [ ] مستندات (README یا docs/architecture) نحوه اجرای و استقرار بک‌اند پایتون را توضیح می‌دهند.
- [ ] تست‌های پایتون موجود می‌توانند در برابر ساختار بک‌اند روشن شده اجرا شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. نقش و نقطه ورود بک‌اند پایتون را روشن کنید. 1. اگر قرار است یک سرویس وب باشد، یک فایل برنامه اصلی (مثلاً `backend/main.py`) با استفاده از یک فریم‌ورک مانند FastAPI یا Flask اضافه کنید و endpointهای API آن را تعریف کنید. 2. اگر صرفاً برای وظایف پس‌زمینه است، اطمینان حاصل کنید که یک راه واضح برای اجرای این وظایف وجود دارد (مثلاً تنظیمات Celery worker، یا فراخوانی مستقیم از سرور Node.js در صورت لزوم). 3. یک `requirements.txt` اضافه کنید یا `pyproject.toml` را برای لیست کردن وابستگی‌های زمان اجرا فراتر از `pytest` به‌روز کنید. 4. `README.md` یا `docs/architecture/` را برای توضیح معماری و استقرار بک‌اند پایتون به‌روز کنید.

## 💡 نمونه‌های قبل/بعد
**قبل: ساختار فعلی بک‌اند پایتون**
...
**بعد: بک‌اند پایتون با نقطه ورود وب**
...
│   ├── notification_pipeline.py
│   └── tasks/
├── main.py # New FastAPI/Flask app entry point
├── requirements.txt # New file for runtime dependencies
└── __init__.py
```
...
معرفی یک فریم‌ورک وب یا task runner جدید پیچیدگی را اضافه می‌کند.
...
1. یک نقطه ورود واضح برای بک‌اند پایتون ایجاد شده است (مثلاً `backend/main.py` برای یک سرویس وب، یا یک اسکریپت worker). _(verify: static)_
2. وابستگی‌های زمان اجرا در `requirements.txt` یا `pyproject.toml` لیست شده‌اند. _(verify: static)_
3. مستندات (README یا docs/architecture) نحوه اجرای و استقرار بک‌اند پایتون را توضیح می‌دهند. _(verify: manual_only)_
4. تست‌های پایتون موجود می‌توانند در برابر ساختار بک‌اند روشن شده اجرا شوند. _(verify: backend_test)_

## Task Steps

### Step 1: ایجاد نقطه ورود اصلی بک‌اند و ادغام فریم‌ورک وب
**Status:** `pending` (0%)
**Scope:** این مرحله بر ایجاد یک نقطه ورود واضح برای بک‌اند پایتون تمرکز دارد. شامل ساخت فایل `app.py` (یا `main.py`) در دایرکتوری `backend/` و راه‌اندازی یک فریم‌ورک وب سبک (مانند FastAPI) برای مدیریت درخواست‌های HTTP است. هدف این است که یک ساختار پایه برای بک‌اند وب فراهم شود که بتواند به عنوان یک سرویس مستقل اجرا شود. این مرحله شامل پیاده‌سازی منطق کسب‌وکار از فایل‌های موجود مانند `scan_failure_handler.py` یا `notification_pipeline.py` نیست، بلکه فقط زیرساخت وب را ایجاد می‌کند.
**Excerpt:**
```
دایرکتوری `backend/` شامل فایل‌های پایتون است (`__init__.py`, `app/handlers/scan_failure_handler.py`, `app/notification_pipeline.py`, `app/tasks/scan_tasks.py`). با این حال، هیچ نقطه ورود واضحی (مانند `main.py`, `app.py` با یک فریم‌ورک وب مانند Flask/FastAPI) یا `requirements.txt` (فقط `pyproject.toml` برای pytest) برای نشان دادن یک بک‌اند وب پایتون کاملاً کاربردی وجود ندارد. این ابهام در مورد نقش بک‌اند پایتون، استقرار و عملکرد صحیح آن را دشوار می‌کند. ممکن است یک نگهدارنده مکان یا یک پیاده‌سازی ناقص باشد.
```

### Step 2: پیاده‌سازی مدیریت وابستگی‌های پروژه
**Status:** `pending` (0%)
**Scope:** این مرحله به ایجاد یا به‌روزرسانی یک فایل مدیریت وابستگی (مانند `requirements.txt` یا `pyproject.toml`) برای بک‌اند پایتون می‌پردازد. هدف این است که تمام کتابخانه‌های مورد نیاز برای اجرای بک‌اند وب، از جمله فریم‌ورک وب انتخاب‌شده و هر وابستگی زمان اجرا دیگر، به وضوح تعریف و لیست شوند. این کار تضمین می‌کند که محیط‌های توسعه و استقرار سازگار و قابل بازتولید باشند. این مرحله شامل نصب واقعی وابستگی‌ها یا راه‌اندازی محیط مجازی نیست، بلکه فقط تعریف لیست وابستگی‌ها را پوشش می‌دهد.
**Excerpt:**
```
دایرکتوری `backend/` شامل فایل‌های پایتون است (`__init__.py`, `app/handlers/scan_failure_handler.py`, `app/notification_pipeline.py`, `app/tasks/scan_tasks.py`). با این حال، هیچ نقطه ورود واضحی (مانند `main.py`, `app.py` با یک فریم‌ورک وب مانند Flask/FastAPI) یا `requirements.txt` (فقط `pyproject.toml` برای pytest) برای نشان دادن یک بک‌اند وب پایتون کاملاً کاربردی وجود ندارد. این ابهام در مورد نقش بک‌اند پایتون، استقرار و عملکرد صحیح آن را دشوار می‌کند. ممکن است یک نگهدارنده مکان یا یک پیاده‌سازی ناقص باشد.
```
