...
updated_at: '2026-06-12T19:19:31.695776+00:00'
target_files:
- backend/
- pyproject.toml
- README.md
...
1. یک نقطه ورود واضح برای بک‌اند پایتون ایجاد شده است (مثلاً `backend/main.py` برای یک سرویس وب، یا یک اسکریپت worker).
2. وابستگی‌های زمان اجرا در `requirements.txt` یا `pyproject.toml` لیست شده‌اند.
3. مستندات (README یا docs/architecture) نحوه اجرای و استقرار بک‌اند پایتون را توضیح می‌دهند.
4. تست‌های پایتون موجود می‌توانند در برابر ساختار بک‌اند روشن شده اجرا شوند.
