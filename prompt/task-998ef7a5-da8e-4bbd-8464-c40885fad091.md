---
task_id: 998ef7a5-da8e-4bbd-8464-c40885fad091
title: TelegramSettings.jsx uses non-collision-safe ID generation for reminders/devices
type: bug
priority: high
...
created_at: '2026-06-12T19:16:05.294725+00:00'
updated_at: '2026-06-12T19:33:08.389071+00:00'
target_files:
- src/components/TelegramSettings.jsx
---

# TelegramSettings.jsx uses non-collision-safe ID generation for reminders/devices

## Raw Idea

The `uid()` function in `src/components/TelegramSettings.jsx` (line 33) uses `Math.random().toString(36).slice(2, 10)` to generate unique IDs for new Telegram devices and reminders. This method is not cryptographically secure and has a high probability of collisions, especially as the number of user-added items increases. In contrast, `src/components/AISettings.jsx` (line 33) correctly uses `crypto.randomUUID()` with a robust fallback. This inconsistency and lack of collision safety can lead to data corruption or unexpected behavior when adding multiple devices or reminders, as new items might clobber existing ones with the same generated ID.
...
- `src/components/TelegramSettings.jsx:33-33` — `uid` — The uid function needs to be updated to use a collision-safe method.
  ```jsx
...
React, JavaScript, client-side ID generation.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/components/AISettings.jsx` (سطر 33) — Contains the correct, collision-safe uid implementation to be mirrored.
- `src/lib/telegram.js` — The telegram module might interact with these IDs; ensuring consistency is important.
...
## 🌐 نقشهٔ وابستگی‌ها
This change affects how new Telegram devices and reminders are identified and stored. It's a local change within the component but impacts data integrity.
...
- [ ] New devices and reminders added in `TelegramSettings.jsx` use UUIDs generated by `crypto.randomUUID()` if available.
- [ ] The fallback ID generation is still unique enough for practical purposes (time + random).
- [ ] No existing devices/reminders are broken by this change (new IDs are generated only for new items).
- [ ] A unit test is added to `src/test/TelegramSettings.test.jsx` to verify the new uid generation logic.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Update the `uid` function in `src/components/TelegramSettings.jsx` to use `crypto.randomUUID()` for generating collision-safe IDs, with a robust fallback for environments where `crypto.randomUUID()` is not available, mirroring the implementation in `src/components/AISettings.jsx` for consistency and security.

## 💡 نمونه‌های قبل/بعد
**Non-collision-safe uid vs. collision-safe uid**
...
_بعد:_
```
const uid = () => { try { if (typeof crypto !== 'undefined' && typeof crypto.randomUUID === 'function') { return 'p_' + crypto.randomUUID(); } } catch { /* crypto unavailable — fall through */ } return 'p_' + Date.now().toString(36) + '_' + Math.random().toString(36).slice(2, 10); };
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run test -- src/test/TelegramSettings.test.jsx`
- `Manual test: Add multiple devices/reminders rapidly and verify unique IDs in browser dev tools/Firestore.`

## ⚠️ ریسک‌ها و موارد احتیاط
None, as it only affects new IDs and uses a robust fallback.
...
1. New devices and reminders added in `TelegramSettings.jsx` use UUIDs generated by `crypto.randomUUID()` if available. _(verify: ui_interaction)_
2. The fallback ID generation is still unique enough for practical purposes (time + random). _(verify: backend_test)_
3. No existing devices/reminders are broken by this change (new IDs are generated only for new items). _(verify: ui_interaction)_
4. A unit test is added to `src/test/TelegramSettings.test.jsx` to verify the new uid generation logic. _(verify: backend_test)_

## Task Steps

### Step 1: بررسی و تحلیل تابع `uid()` فعلی در TelegramSettings.jsx
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق تابع `uid()` موجود در فایل `src/components/TelegramSettings.jsx` و شناسایی تمام نقاطی است که این تابع برای تولید شناسه‌های منحصر به فرد (UID) برای دستگاه‌های تلگرام و یادآورها فراخوانی می‌شود. هدف این است که درک کاملی از نحوه عملکرد فعلی، وابستگی‌های آن، و تأثیرات احتمالی تغییر آن به دست آید. این مرحله صرفاً تحلیلی است و شامل هیچ‌گونه تغییر کد نمی‌شود.
**Excerpt:**
```
The `uid()` function in `src/components/TelegramSettings.jsx` (line 33) uses `Math.random().toString(36).slice(2, 10)` to generate unique IDs for new Telegram devices and reminders. This method is not cryptographically secure and has a high probability of collisions, especially as the number of user-added items increases.
```

### Step 2: پیاده‌سازی `crypto.randomUUID()` با مکانیزم fallback در TelegramSettings.jsx
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح فایل `src/components/TelegramSettings.jsx` تمرکز دارد. تابع `uid()` (یا منطق معادل آن) که مسئول تولید شناسه‌ها است، باید به‌روزرسانی شود تا از `crypto.randomUUID()` برای تولید شناسه‌های امن و بدون برخورد استفاده کند. یک مکانیزم fallback قوی، مشابه آنچه در `src/components/AISettings.jsx` پیاده‌سازی شده، باید اضافه شود تا از عملکرد صحیح در محیط‌هایی که `crypto.randomUUID()` در دسترس نیست، اطمینان حاصل شود. هدف اصلی، حذف آسیب‌پذیری برخورد شناسه‌ها برای دستگاه‌های تلگرام و یادآورهای جدید است.
**Excerpt:**
```
The `uid()` function in `src/components/TelegramSettings.jsx` (line 33) uses `Math.random().toString(36).slice(2, 10)` to generate unique IDs for new Telegram devices and reminders. This method is not cryptographically secure and has a high probability of collisions... In contrast, `src/components/AISettings.jsx` (line 33) correctly uses `crypto.randomUUID()` with a robust fallback. This inconsistency and lack of collision safety can lead to data corruption or unexpected behavior when adding multiple devices or reminders, as new items might clobber existing ones with the same generated ID.
```

### Step 3: تأیید عملکرد تولید ID جدید در رابط کاربری (UI)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تست دستی و end-to-end در رابط کاربری برنامه است تا اطمینان حاصل شود که افزودن دستگاه‌های تلگرام و یادآورهای جدید با منطق تولید ID به‌روز شده، به درستی کار می‌کند. تمرکز بر مشاهده این است که آیتم‌ها بدون خطا اضافه می‌شوند و شناسه‌های داخلی آن‌ها با استفاده از روش امن جدید تولید می‌شوند، که از مسائل برخورد شناسه‌های قبلی جلوگیری می‌کند. این مرحله صحت عملکردی را از دیدگاه کاربر تأیید می‌کند.
**Excerpt:**
```
This inconsistency and lack of collision safety can lead to data corruption or unexpected behavior when adding multiple devices or reminders, as new items might clobber existing ones with the same generated ID.
```

### Step 4: افزودن تست‌های واحد/یکپارچه‌سازی برای منطق تولید ID جدید
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد (unit tests) و/یا یکپارچه‌سازی (integration tests) اختصاصی برای اعتبارسنجی تابع `uid()` (یا ابزار جدید تولید ID) در `src/components/TelegramSettings.jsx` است. تست‌ها باید هم مسیر اصلی `crypto.randomUUID()` و هم مکانیزم fallback قوی را پوشش دهند و اطمینان حاصل کنند که شناسه‌ها همیشه به صورت امن و منحصر به فرد تحت شرایط مختلف تولید می‌شوند. هدف این مرحله جلوگیری از رگرسیون‌ها و تضمین قابلیت اطمینان طولانی‌مدت تولید ID است.
**Excerpt:**
```
This inconsistency and lack of collision safety can lead to data corruption or unexpected behavior when adding multiple devices or reminders, as new items might clobber existing ones with the same generated ID.
```
