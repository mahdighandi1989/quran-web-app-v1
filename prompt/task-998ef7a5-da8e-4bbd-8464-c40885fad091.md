...
updated_at: '2026-06-12T19:16:05.294727+00:00'
target_files:
...
1. New devices and reminders added in `TelegramSettings.jsx` use UUIDs generated by `crypto.randomUUID()` if available.
2. The fallback ID generation is still unique enough for practical purposes (time + random).
3. No existing devices/reminders are broken by this change (new IDs are generated only for new items).
4. A unit test is added to `src/test/TelegramSettings.test.jsx` to verify the new uid generation logic.
