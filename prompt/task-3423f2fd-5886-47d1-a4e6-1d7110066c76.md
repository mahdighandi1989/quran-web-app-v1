---
task_id: 3423f2fd-5886-47d1-a4e6-1d7110066c76
title: افزودن تست‌های واحد برای وظایف پردازش اسکن قرآن
type: bug
priority: high
...
created_at: '2026-06-12T19:14:39.716374+00:00'
updated_at: '2026-06-12T19:32:20.820520+00:00'
target_files:
- backend/app/tasks/scan_tasks.py
---

# افزودن تست‌های واحد برای وظایف پردازش اسکن قرآن

## Raw Idea

فایل `backend/app/tasks/scan_tasks.py` حاوی منطق برای وظایف اسکن (احتمالاً پردازش فایل‌های داده قرآن) است. این فایل هیچ تست واحدی ندارد. از آنجایی که شکست در این وظایف به عنوان یک رویداد بحرانی (scan_failed) در نظر گرفته می‌شود، عدم وجود تست‌ها می‌تواند منجر به خطاهای پردازش داده‌های اساسی و مشکلات پایداری شود که بدون اطلاع کاربر باقی می‌مانند.
...
- `backend/app/tasks/scan_tasks.py:1-end` — `scan_tasks.py` — فایل اصلی وظایف اسکن که نیاز به تست دارد.
  ```python
  from backend.app.handlers.scan_failure_handler import handle_scan_failure
  # ... (truncated)
  ```
...
Python, Pytest, Mocking (برای وابستگی‌های خارجی مانند دسترسی به فایل یا دیتابیس).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/handlers/scan_failure_handler.py` — handler شکست که در صورت بروز خطا توسط این وظایف فراخوانی می‌شود.
- `docs/architecture/data-pipeline.md` — سند معماری که pipeline داده و اهمیت پردازش داده‌های قرآن را توضیح می‌دهد.
- `backend/app/notification_pipeline.py` — `scan_tasks.py` این فایل را import می‌کند
- `tests/integration/test_notification_pipeline.py` — این فایل `scan_tasks.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این وظایف مسئول پردازش داده‌های ورودی قرآن هستند و در صورت شکست، `scan_failure_handler` را فراخوانی می‌کنند. تست‌ها باید اطمینان حاصل کنند که منطق پردازش داده قوی است و مسیرهای خطا به درستی فعال می‌شوند.
...
- [ ] فایل `tests/backend/app/tasks/test_scan_tasks.py` ایجاد شود.
- [ ] تست‌هایی برای سناریوهای موفقیت‌آمیز پردازش اسکن اضافه شود.
- [ ] تست‌هایی برای سناریوهای شکست (مثلاً فایل ورودی نامعتبر) اضافه شود و تأیید شود که `handle_scan_failure` فراخوانی می‌شود.
- [ ] تست‌هایی برای اطمینان از خروجی صحیح داده‌های پردازش‌شده اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست واحد جدید (`tests/backend/app/tasks/test_scan_tasks.py`) برای پوشش دادن منطق وظایف اسکن، شامل سناریوهای موفقیت و شکست.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `pytest tests/backend/app/tasks/test_scan_tasks.py`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به Mock کردن سیستم فایل یا هر منبع داده خارجی که وظایف اسکن به آن وابسته هستند.
...
1. فایل `tests/backend/app/tasks/test_scan_tasks.py` ایجاد شود. _(verify: static)_
2. تست‌هایی برای سناریوهای موفقیت‌آمیز پردازش اسکن اضافه شود. _(verify: backend_test)_
3. تست‌هایی برای سناریوهای شکست (مثلاً فایل ورودی نامعتبر) اضافه شود و تأیید شود که `handle_scan_failure` فراخوانی می‌شود. _(verify: backend_test)_
4. تست‌هایی برای اطمینان از خروجی صحیح داده‌های پردازش‌شده اضافه شود. _(verify: backend_test)_
