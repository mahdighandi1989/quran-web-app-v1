---
task_id: c2932506-9520-4adc-b99d-bc7ae8fdb10b
title: '`parseJsonArray` in `aiTasks.js` is vulnerable to malformed AI output'
type: bug
priority: medium
...
created_at: '2026-06-12T19:21:38.237724+00:00'
updated_at: '2026-06-12T19:36:50.341566+00:00'
target_files:
- src/lib/aiTasks.js
---

# `parseJsonArray` in `aiTasks.js` is vulnerable to malformed AI output

## Raw Idea

The `AIExamGenerator.jsx` component (line 45) relies on `parseJsonArray` (from `src/lib/aiTasks.js`) to process the raw output from the AI model. If the AI model, due to prompt engineering issues or inherent unpredictability, returns a string that is not valid JSON, or is not a JSON array, `JSON.parse` could throw an error. While `AIExamGenerator.jsx` has a general `try/catch`, `parseJsonArray` itself should be robust against malformed input, either returning a safe default (e.g., an empty array) or throwing a more specific, user-friendly error. This prevents crashes or unexpected behavior in the quiz rendering.
...
- `src/lib/aiTasks.js:5-5` — `parseJsonArray` — The implementation of parseJsonArray needs to be robust against malformed JSON.
  ```jsx
...
JavaScript, JSON parsing, AI model output.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/components/AIExamGenerator.jsx` (سطر 45) — Calls parseJsonArray on AI output.
- `src/test/aiTasks.test.js` — New tests should be added to cover malformed JSON input.
- `src/App.jsx` — این فایل `aiTasks.js` را import می‌کند (caller)
- `src/lib/telegramCommands.js` — این فایل `aiTasks.js` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This is a local change within the `aiTasks` utility but directly impacts the stability of the `AIExamGenerator` feature.
...
- [ ] `parseJsonArray` handles invalid JSON strings without crashing.
- [ ] If the AI output is not a valid JSON array, `parseJsonArray` returns an empty array or throws a specific error.
- [ ] `AIExamGenerator` gracefully handles the output of `parseJsonArray` even if it's empty or an error.
- [ ] New unit tests are added to `src/test/aiTasks.test.js` for malformed JSON inputs.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Implement a `try/catch` block within `parseJsonArray` in `src/lib/aiTasks.js` to gracefully handle `JSON.parse` errors. If parsing fails, it should return an empty array or throw a specific error indicating malformed AI output. Additionally, ensure the parsed result is an array before returning.

## 💡 نمونه‌های قبل/بعد
**Direct JSON.parse vs. safe JSON.parse (conceptual)**

_قبل:_
```
export function parseJsonArray(text) { return JSON.parse(text); }
```

_بعد:_
```
export function parseJsonArray(text) { try { const parsed = JSON.parse(text); if (!Array.isArray(parsed)) { console.warn('AI output was not an array, returning empty:', parsed); return []; } return parsed; } catch (e) { console.error('Failed to parse AI JSON output:', e); throw new Error('AI returned malformed data. Please try again.'); } }
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run test -- src/test/aiTasks.test.js`
- `Manually simulate malformed AI responses (e.g., by mocking `chatComplete` in `AIExamGenerator`) and verify UI behavior.`
...
1. `parseJsonArray` handles invalid JSON strings without crashing. _(verify: backend_test)_
2. If the AI output is not a valid JSON array, `parseJsonArray` returns an empty array or throws a specific error. _(verify: backend_test)_
3. `AIExamGenerator` gracefully handles the output of `parseJsonArray` even if it's empty or an error. _(verify: ui_interaction)_
4. New unit tests are added to `src/test/aiTasks.test.js` for malformed JSON inputs. _(verify: backend_test)_

## Task Steps

### Step 1: افزایش پایداری تابع `parseJsonArray` در برابر ورودی JSON نامعتبر
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح تابع `parseJsonArray` واقع در فایل `src/lib/aiTasks.js` تمرکز دارد. هدف اصلی، افزایش قابلیت مدیریت خطا در این تابع هنگام پردازش خروجی خام رشته‌ای از مدل هوش مصنوعی است. به طور خاص، این تابع باید بتواند به طور صحیح و بدون خطا، ورودی‌هایی را که JSON معتبر نیستند یا یک آرایه JSON نیستند، مدیریت کند. پیاده‌سازی باید به گونه‌ای باشد که یا یک آرایه خالی به عنوان مقدار پیش‌فرض امن بازگرداند، یا یک خطای مشخص و کاربرپسندتر را پرتاب کند. این مرحله شامل هیچ تغییری در کامپوننت `AIExamGenerator.jsx` یا سایر بخش‌هایی که از `parseJsonArray` استفاده می‌کنند، نمی‌شود، زیرا مشکل به منطق تجزیه و تحلیل خود تابع محدود است.
**Excerpt:**
```
The `AIExamGenerator.jsx` component (line 45) relies on `parseJsonArray` (from `src/lib/aiTasks.js`) to process the raw output from the AI model. If the AI model, due to prompt engineering issues or inherent unpredictability, returns a string that is not valid JSON, or is not a JSON array, `JSON.parse` could throw an error. While `AIExamGenerator.jsx` has a general `try/catch`, `parseJsonArray` itself should be robust against malformed input, either returning a safe default (e.g., an empty array) or throwing a more specific, user-friendly error. This prevents crashes or unexpected behavior in the quiz rendering.
```
