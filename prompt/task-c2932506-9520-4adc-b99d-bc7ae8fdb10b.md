...
updated_at: '2026-06-12T19:21:38.237726+00:00'
target_files:
- src/lib/aiTasks.js
---
...
1. `parseJsonArray` handles invalid JSON strings without crashing.
2. If the AI output is not a valid JSON array, `parseJsonArray` returns an empty array or throws a specific error.
3. `AIExamGenerator` gracefully handles the output of `parseJsonArray` even if it's empty or an error.
4. New unit tests are added to `src/test/aiTasks.test.js` for malformed JSON inputs.
