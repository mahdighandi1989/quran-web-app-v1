---
task_id: 760e15a8-8018-4900-aeda-fadc04c8a8da
title: Clarify usage of Firebase service account env vars in archived task
type: docs
priority: medium
...
created_at: '2026-06-12T19:19:53.428241+00:00'
updated_at: '2026-06-12T19:36:01.797290+00:00'
target_files:
- prompt/archive/task-1acc7ded-7ccc-4b4f-ae81-5df6f0b9aad2.md
---

# Clarify usage of Firebase service account env vars in archived task

## Raw Idea

The archived task `task-1acc7ded` (marked 'merged') incorrectly stated that `FIREBASE_SERVICE_ACCOUNT` and `GOOGLE_APPLICATION_CREDENTIALS` environment variables were unused and should be removed. However, the `.env.example` file clearly documents these variables as being essential for 'Firestore access for the Admin SDK' by the 'Telegram bot server (server/telegram-bot.mjs)'. The `server/package.json` also lists `firebase-admin` as a dependency, confirming their backend usage. This indicates the previous task's analysis was incomplete (likely frontend-focused), leading to a misleading 'merged' status for a non-existent problem. The current code and `.env.example` are correct.
...
- `prompt/archive/task-1acc7ded-7ccc-4b4f-ae81-5df6f0b9aad2.md:20-82`
  ```markdown
  ## 📋 شرح
...
Firebase Admin SDK, Node.js backend, environment variables.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.env.example` — Documents the purpose of these variables for the Telegram bot server.
- `server/package.json` — Lists `firebase-admin` dependency, which relies on these credentials.
- `server/telegram-bot.mjs` — The backend bot that consumes these credentials.

## 🌐 نقشهٔ وابستگی‌ها
This is a documentation/historical task correction. It clarifies cross-stack usage of environment variables. No functional code changes are required in the application or backend.
...
- [ ] The archived task `prompt/archive/task-1acc7ded-7ccc-4b4f-ae81-5df6f0b9aad2.md` is updated with a clear note explaining the backend usage of the environment variables.
- [ ] No changes are made to `.env.example` or `server/telegram-bot.mjs`.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Update the description within the archived task `prompt/archive/task-1acc7ded-7ccc-4b4f-ae81-5df6f0b9aad2.md` to clarify that these variables *are* used by the backend bot and that the original premise of the task was incorrect. No code changes are needed in `.env.example` or `server/telegram-bot.mjs` as their current state is correct.

## 💡 نمونه‌های قبل/بعد
**Original description in archived task**
...
**Proposed correction for archived task description**

_بعد:_
```
[Correction: `FIREBASE_SERVICE_ACCOUNT` and `GOOGLE_APPLICATION_CREDENTIALS` are used by the Node.js backend (`server/telegram-bot.mjs`) for Firebase Admin SDK access, as explicitly documented in `.env.example`. The previous scan was likely frontend-focused and missed this backend usage. No code changes are needed for these variables.]
```
...
- `git diff server/telegram-bot.mjs`
...
1. The archived task `prompt/archive/task-1acc7ded-7ccc-4b4f-ae81-5df6f0b9aad2.md` is updated with a clear note explaining the backend usage of the environment variables. _(verify: static)_
2. No changes are made to `.env.example` or `server/telegram-bot.mjs`. _(verify: static)_
