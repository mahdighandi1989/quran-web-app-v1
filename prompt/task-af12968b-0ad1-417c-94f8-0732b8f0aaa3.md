---
task_id: af12968b-0ad1-417c-94f8-0732b8f0aaa3
...
created_at: '2026-06-12T19:15:02.591093+00:00'
updated_at: '2026-06-12T19:32:42.784495+00:00'
target_files:
...
کامپوننت `AIChat` (خطوط 63-107) در فایل `src/components/AIWidgets.jsx` وجود دارد، اما یک تسک قبلی که به عنوان `merged` علامت‌گذاری شده (`task-06f17cca-a305-40b3-b38a-06ae9cde3ee6.md`) صراحتاً اعلام کرده بود که این کامپوننت استفاده‌نشده است و باید حذف شود. علاوه بر این، `docs/ai-chat-use-cases.md` کامپوننت `AIChat` را به عنوان رابط کاربری برای مورد استفاده `QA` لیست می‌کند که این خود یک تناقض است. اگر طبق تسک ادغام‌شده واقعاً استفاده‌نشده باشد، کد مرده است. اگر قرار است استفاده شود، یک کامپوننت UI یتیم است که در `App.jsx` یا `routes.jsx` وارد نشده است. با توجه به وضعیت `merged` تسک حذف، باید حذف شود.
...
- `src/components/AIWidgets.jsx:63-107` — `AIChat` — کامپوننت AIChat که باید حذف شود.
  ```jsx
...
- `prompt/archive/task-06f17cca-a305-40b3-b38a-06ae9cde3ee6.md` — تسک قبلی که صراحتاً این کامپوننت را استفاده‌نشده اعلام کرده و خواستار حذف آن شده بود.
- `docs/ai-chat-use-cases.md` — این سند AIChat را به عنوان رابط کاربری برای مورد استفاده QA لیست می‌کند که با وضعیت استفاده‌نشده آن در تضاد است.
- `src/App.jsx` — فایل اصلی برنامه که در صورت استفاده باید AIChat را وارد کند.
- `src/routes.jsx` — مکان بالقوه دیگری که در صورت استفاده AIChat باید وارد شود.
- `src/lib/aiClient.js` — `AIWidgets.jsx` این فایل را import می‌کند
- `src/lib/analytics.js` — `AIWidgets.jsx` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این کامپوننت در حال حاضر توسط هیچ فایل دیگری در زمینه ارائه شده وارد نشده است، بنابراین کد مرده است. حذف آن هیچ عملکرد موجودی را مختل نمی‌کند.
...
- [ ] کامپوننت `AIChat` از `src/components/AIWidgets.jsx` حذف شده است.
- [ ] `src/components/AIWidgets.jsx` دیگر `AIChat` را صادر نمی‌کند.
- [ ] `docs/ai-chat-use-cases.md` به‌روزرسانی شده است تا ارجاع به `AIChat` برای مورد استفاده `QA` حذف شود یا وضعیت آن روشن شود.
- [ ] هیچ ارجاع شکسته یا خطای ساخت (build error) ایجاد نشده است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. کامپوننت `AIChat` را از `src/components/AIWidgets.jsx` حذف کنید. سپس `docs/ai-chat-use-cases.md` را به‌روزرسانی کنید تا ارجاع به `AIChat` برای مورد استفاده `QA` حذف شود یا وضعیت آن روشن شود.
...
_بعد:_
```
// کامپوننت AIChat به دلیل استفاده‌نشدن حذف شد.
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run build`
- `grep -r "AIChat" src/components/AIWidgets.jsx`
- `grep -r "AIChat" src/App.jsx`
- `grep -r "AIChat" src/routes.jsx`

## ⚠️ ریسک‌ها و موارد احتیاط
هیچ، زیرا کد استفاده‌نشده است.
...
1. کامپوننت `AIChat` از `src/components/AIWidgets.jsx` حذف شده است. _(verify: static)_
2. `src/components/AIWidgets.jsx` دیگر `AIChat` را صادر نمی‌کند. _(verify: static)_
3. `docs/ai-chat-use-cases.md` به‌روزرسانی شده است تا ارجاع به `AIChat` برای مورد استفاده `QA` حذف شود یا وضعیت آن روشن شود. _(verify: static)_
4. هیچ ارجاع شکسته یا خطای ساخت (build error) ایجاد نشده است. _(verify: ui_interaction)_

## Task Steps

### Step 1: حذف کامپوننت `AIChat` از `src/components/AIWidgets.jsx`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل حذف کامل تعریف کامپوننت `AIChat` (خطوط 63-107) از فایل `src/components/AIWidgets.jsx` می‌شود. هدف این است که کد مرده‌ای که توسط `task-06f17cca-a305-40b3-b38a-06ae9cde3ee6.md` به عنوان استفاده‌نشده علامت‌گذاری شده بود، از codebase حذف شود. این مرحله شامل بررسی وابستگی‌های دیگر یا به‌روزرسانی فایل‌های `App.jsx` یا `routes.jsx` نمی‌شود، زیرا فرض بر این است که کامپوننت واقعاً استفاده‌نشده است و تصمیم حذف بر اساس وضعیت `merged` تسک قبلی گرفته شده است.
**Excerpt:**
```
کامپوننت `AIChat` (خطوط 63-107) در فایل `src/components/AIWidgets.jsx` وجود دارد، اما یک تسک قبلی که به عنوان `merged` علامت‌گذاری شده (`task-06f17cca-a305-40b3-b38a-06ae9cde3ee6.md`) صراحتاً اعلام کرده بود که این کامپوننت استفاده‌نشده است و باید حذف شود. ... با توجه به وضعیت `merged` تسک حذف، باید حذف شود.
```

### Step 2: به‌روزرسانی مستندات `docs/ai-chat-use-cases.md`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `docs/ai-chat-use-cases.md` برای حذف هرگونه ارجاع به کامپوننت `AIChat` به عنوان رابط کاربری برای مورد استفاده `QA` است. هدف این است که مستندات با وضعیت فعلی codebase (پس از حذف کامپوننت `AIChat`) همگام‌سازی شوند و تناقض موجود بین کد و مستندات برطرف گردد. این کار از سردرگمی توسعه‌دهندگان و کاربران در آینده جلوگیری می‌کند.
**Excerpt:**
```
علاوه بر این، `docs/ai-chat-use-cases.md` کامپوننت `AIChat` را به عنوان رابط کاربری برای مورد استفاده `QA` لیست می‌کند که این خود یک تناقض است.
```
