...
updated_at: '2026-06-12T19:15:02.591095+00:00'
target_files:
...
1. کامپوننت `AIChat` از `src/components/AIWidgets.jsx` حذف شده است.
2. `src/components/AIWidgets.jsx` دیگر `AIChat` را صادر نمی‌کند.
3. `docs/ai-chat-use-cases.md` به‌روزرسانی شده است تا ارجاع به `AIChat` برای مورد استفاده `QA` حذف شود یا وضعیت آن روشن شود.
4. هیچ ارجاع شکسته یا خطای ساخت (build error) ایجاد نشده است.
