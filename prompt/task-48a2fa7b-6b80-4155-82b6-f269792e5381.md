...
updated_at: '2026-06-12T19:08:52.313807+00:00'
target_files:
...
1. The `TELEGRAM_BOT_TOKEN` is never present in the browser's developer tools or network requests initiated from the client.
2. All Telegram API interactions from the client are successfully routed through a new backend proxy.
3. The Telegram bot's functionality (token validation, chat ID detection, test messages, menu setup) remains fully operational.
4. The `TelegramSettings.jsx` UI correctly reflects the connection status without exposing the token.
