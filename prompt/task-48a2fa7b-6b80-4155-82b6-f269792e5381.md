---
task_id: 48a2fa7b-6b80-4155-82b6-f269792e5381
title: Telegram Bot Token is Exposed Client-Side in UI State
type: security
priority: critical
...
created_at: '2026-06-12T19:08:52.313803+00:00'
updated_at: '2026-06-12T19:27:24.529070+00:00'
target_files:
- src/components/TelegramSettings.jsx
---

# Telegram Bot Token is Exposed Client-Side in UI State

## Raw Idea

The `TelegramSettings.jsx` component directly binds the `tg.botToken` from its props to a password input field. This means the sensitive bot token is present in the client-side React state and browser memory. This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets. An attacker with XSS capabilities or access to the user's browser could extract this token.
...
- `src/components/TelegramSettings.jsx:183` — `TelegramSettings` — The input field where the bot token is displayed and bound to client-side state.
  ```jsx
...
React, Firebase (Firestore), Telegram Bot API, Node.js backend proxy.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/lib/telegram.js` (سطر -1) — Contains client-side Telegram API calls that currently use the token directly.
- `src/App.jsx` (سطر -1) — Passes the Telegram configuration (including the token) to the TelegramSettings component.
- `server/telegram-bot.mjs` (سطر -1) — The server-side bot that should securely manage and use the token.
- `.env.example` (سطر 35) — Documents TELEGRAM_BOT_TOKEN as a server-side runtime environment variable.
- `src/test/TelegramSettings.test.jsx` — این فایل `TelegramSettings.jsx` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This change requires refactoring the client-side Telegram API calls to use a new backend proxy, impacting all features that interact with the Telegram bot (validation, message sending, menu setup). It also affects how the Telegram configuration is managed in App.jsx and stored in Firestore.
...
- [ ] The `TELEGRAM_BOT_TOKEN` is never present in the browser's developer tools or network requests initiated from the client.
- [ ] All Telegram API interactions from the client are successfully routed through a new backend proxy.
- [ ] The Telegram bot's functionality (token validation, chat ID detection, test messages, menu setup) remains fully operational.
- [ ] The `TelegramSettings.jsx` UI correctly reflects the connection status without exposing the token.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Refactor the Telegram integration to route all sensitive Telegram API calls (e.g., `getMe`, `detectChatIds`, `sendMessage`, `setupBotMenu`) through a dedicated backend proxy. The client-side `TelegramSettings` component should only interact with this proxy, never directly handling or displaying the `TELEGRAM_BOT_TOKEN`. The proxy will retrieve the token securely from its environment variables.

## 💡 نمونه‌های قبل/بعد
**Before (client-side token access)**
...
_بعد:_
```
const me = await getMeViaProxy(); // Token handled by proxy
```
...
_بعد:_
```
value="********" // Token not present in client-side state
```
...
## 🧪 دستورات اعتبارسنجی
- `Inspect browser's network tab and React DevTools for `TELEGRAM_BOT_TOKEN` after logging in and navigating to Telegram settings.`
- `Manually test all Telegram features in the UI (validate, detect, send test, apply menu).`
- `Verify server-side logs for proxy requests and token usage.`

## ⚠️ ریسک‌ها و موارد احتیاط
Significant refactoring effort, potential for new bugs in the proxy layer, increased deployment complexity for the proxy.
...
1. The `TELEGRAM_BOT_TOKEN` is never present in the browser's developer tools or network requests initiated from the client. _(verify: ui_interaction)_
2. All Telegram API interactions from the client are successfully routed through a new backend proxy. _(verify: ui_interaction)_
3. The Telegram bot's functionality (token validation, chat ID detection, test messages, menu setup) remains fully operational. _(verify: ui_interaction)_
4. The `TelegramSettings.jsx` UI correctly reflects the connection status without exposing the token. _(verify: ui_interaction)_

## Task Steps

### Step 1: جلوگیری از ارسال `tg.botToken` به سمت کلاینت
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و اصلاح منبعی است که `tg.botToken` را به سمت کلاینت ارسال می‌کند. این ممکن است شامل تغییر در API بک‌اند که داده‌ها را فراهم می‌کند یا کامپوننت والد React که این داده‌ها را fetch کرده و به `TelegramSettings.jsx` به عنوان prop ارسال می‌کند، باشد. هدف این است که اطمینان حاصل شود توکن حساس هرگز به مرورگر کاربر نرسد.
**Excerpt:**
```
The `TelegramSettings.jsx` component directly binds the `tg.botToken` from its props to a password input field. This means the sensitive bot token is present in the client-side React state and browser memory. This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets. An attacker with XSS capabilities or access to the user's browser could extract this token.
```

### Step 2: به‌روزرسانی `TelegramSettings.jsx` برای عدم انتظار `tg.botToken` در props
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح کامپوننت `TelegramSettings.jsx` برای حذف هرگونه وابستگی به `tg.botToken` در props آن است. هر منطق، رندرینگ یا اعتبارسنجی که به وجود این prop متکی است باید به‌روزرسانی یا حذف شود تا کامپوننت بتواند بدون دریافت توکن از طریق props به درستی کار کند.
**Excerpt:**
```
The `TelegramSettings.jsx` component directly binds the `tg.botToken` from its props to a password input field. This means the sensitive bot token is present in the client-side React state and browser memory. This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets. An attacker with XSS capabilities or access to the user's browser could extract this token.
```

### Step 3: پیاده‌سازی فیلد ورودی امن برای تنظیم توکن ربات در `TelegramSettings.jsx`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن یا اصلاح یک فیلد ورودی در `TelegramSettings.jsx` است که به کاربر اجازه می‌دهد یک توکن ربات تلگرام *جدید* را وارد کند. این فیلد باید از نوع 'password' باشد تا کاراکترها را پنهان کند و *هرگز* نباید توکن موجود (اگر قبلاً تنظیم شده باشد) را نمایش دهد. این فیلد باید فقط برای 'set-only' عمل کند.
**Excerpt:**
```
The `TelegramSettings.jsx` component directly binds the `tg.botToken` from its props to a password input field. This means the sensitive bot token is present in the client-side React state and browser memory. This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets. An attacker with XSS capabilities or access to the user's browser could extract this token.
```

### Step 4: تأیید/ایجاد API Endpoint برای ذخیره‌سازی امن توکن ربات تلگرام
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی معماری بک‌اند برای اطمینان از وجود یک API endpoint امن است که می‌تواند یک توکن ربات تلگرام جدید را از کلاینت دریافت کرده و آن را به صورت امن در سمت سرور ذخیره کند. اگر چنین endpointی وجود ندارد، باید ایجاد شود. این endpoint باید شامل اعتبارسنجی مناسب باشد و توکن را به هیچ عنوان به کلاینت بازنگرداند.
**Excerpt:**
```
This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets. An attacker with XSS capabilities or access to the user's browser could extract this token.
```

### Step 5: یکپارچه‌سازی `TelegramSettings.jsx` با API Endpoint امن برای ذخیره‌سازی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل پیاده‌سازی منطق در `TelegramSettings.jsx` است که مقدار وارد شده در فیلد ورودی توکن ربات (از مرحله 3) را به API endpoint امن بک‌اند (از مرحله 4) ارسال می‌کند. این ارسال باید هنگام ذخیره تغییرات توسط کاربر انجام شود و باید شامل مدیریت وضعیت بارگذاری، خطا و موفقیت باشد.
**Excerpt:**
```
The `TelegramSettings.jsx` component directly binds the `tg.botToken` from its props to a password input field. This means the sensitive bot token is present in the client-side React state and browser memory. This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets. An attacker with XSS capabilities or access to the user's browser could extract this token.
```

### Step 6: افزودن تست‌های واحد/یکپارچه‌سازی برای مدیریت امن توکن
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های خودکار (واحد و/یا یکپارچه‌سازی) برای تأیید اینکه `tg.botToken` دیگر در سمت کلاینت افشا نمی‌شود و مکانیسم جدید برای تنظیم امن توکن به درستی کار می‌کند. تست‌ها باید شامل موارد زیر باشند: عدم وجود توکن در props `TelegramSettings.jsx`، عملکرد صحیح فیلد ورودی 'set-only' و ارسال موفقیت‌آمیز توکن به بک‌اند.
**Excerpt:**
```
The `TelegramSettings.jsx` component directly binds the `tg.botToken` from its props to a password input field. This means the sensitive bot token is present in the client-side React state and browser memory. This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets. An attacker with XSS capabilities or access to the user's browser could extract this token.
```

### Step 7: به‌روزرسانی مستندات امنیتی مرتبط
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازبینی و به‌روزرسانی هرگونه مستندات پروژه، دستورالعمل‌های امنیتی، یا فایل‌های پیکربندی (مانند `.env.example`) است که ممکن است به نحوه مدیریت `tg.botToken` یا سایر اسرار سمت کلاینت اشاره داشته باشند. مستندات باید معماری امنیتی جدید را منعکس کنند و بهترین شیوه‌ها را برای توسعه‌دهندگان آینده تقویت کنند.
**Excerpt:**
```
This contradicts the security best practice of keeping API keys strictly server-side, as highlighted by the project's own AI proxy architecture and `.env.example` documentation for server-side secrets.
```
