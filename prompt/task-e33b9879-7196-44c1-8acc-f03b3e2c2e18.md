---
task_id: e33b9879-7196-44c1-8acc-f03b3e2c2e18
title: افزودن تست‌های واحد برای AI Proxy Server
type: security
priority: critical
...
created_at: '2026-06-12T19:08:52.919858+00:00'
updated_at: '2026-06-12T19:27:39.805562+00:00'
target_files:
- server/ai-proxy.mjs
---

# افزودن تست‌های واحد برای AI Proxy Server

## Raw Idea

فایل `server/ai-proxy.mjs` مسئول پراکسی کردن درخواست‌های هوش مصنوعی از فرانت‌اند به ارائه‌دهندگان AI است و کلیدهای API حساس را پنهان می‌کند. این فایل هیچ تست واحدی ندارد. عدم وجود تست‌ها یک ریسک امنیتی جدی است، زیرا می‌تواند منجر به افشای کلیدهای API یا شکست‌های پنهان در ارتباط با سرویس‌های AI شود.
...
- `server/ai-proxy.mjs:1-end` — `ai-proxy.mjs` — فایل اصلی AI Proxy که نیاز به تست دارد.
  ```
  import { createProxyMiddleware } from 'http-proxy-middleware';
  import { resolveAI } from './ai.mjs';
  // ... (truncated)
  ```
...
Node.js (ESM), http-proxy-middleware, Vitest (برای تست‌های جدید).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/lib/aiClient.js` — کلاینت فرانت‌اند که درخواست‌ها را از طریق این پراکسی ارسال می‌کند.
- `server/ai.mjs` — ماژول `ai.mjs` که منطق حل‌وفصل ارائه‌دهنده AI را برای پراکسی فراهم می‌کند.
- `.env.example` — متغیر محیطی `VITE_AI_PROXY_URL` که آدرس پراکسی را تعیین می‌کند.
- `server/telegram-bot.mjs` — این فایل `ai-proxy.mjs` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
AI Proxy یک لایه امنیتی و عملکردی حیاتی بین فرانت‌اند و ارائه‌دهندگان AI است. تست‌های آن باید اطمینان حاصل کنند که کلیدهای API هرگز به کلاینت نمی‌رسند و درخواست‌ها به درستی مسیریابی و مدیریت می‌شوند.
...
- [ ] فایل `server/test/ai-proxy.test.mjs` ایجاد شود.
- [ ] تست‌هایی برای تأیید اینکه درخواست‌ها به درستی به ارائه‌دهنده AI هدایت می‌شوند، اضافه شود.
- [ ] تست‌هایی برای تأیید اینکه کلیدهای API در سمت کلاینت افشا نمی‌شوند، اضافه شود.
- [ ] تست‌هایی برای مدیریت خطا در صورت عدم پاسخگویی ارائه‌دهنده AI یا خطاهای شبکه اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست واحد جدید (`server/test/ai-proxy.test.mjs`) برای اطمینان از عملکرد صحیح پراکسی، پنهان‌سازی کلیدهای API، و مدیریت خطاها.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `npm test -- server/test/ai-proxy.test.mjs`

## ⚠️ ریسک‌ها و موارد احتیاط
پیچیدگی Mock کردن درخواست‌های HTTP و اطمینان از عدم افشای اطلاعات حساس.
...
1. فایل `server/test/ai-proxy.test.mjs` ایجاد شود. _(verify: static)_
2. تست‌هایی برای تأیید اینکه درخواست‌ها به درستی به ارائه‌دهنده AI هدایت می‌شوند، اضافه شود. _(verify: backend_test)_
3. تست‌هایی برای تأیید اینکه کلیدهای API در سمت کلاینت افشا نمی‌شوند، اضافه شود. _(verify: backend_test)_
4. تست‌هایی برای مدیریت خطا در صورت عدم پاسخگویی ارائه‌دهنده AI یا خطاهای شبکه اضافه شود. _(verify: backend_test)_
