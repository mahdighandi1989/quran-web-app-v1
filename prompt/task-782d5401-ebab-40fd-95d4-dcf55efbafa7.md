---
task_id: 782d5401-ebab-40fd-95d4-dcf55efbafa7
...
created_at: '2026-06-12T19:21:42.625429+00:00'
updated_at: '2026-06-12T19:21:48.644352+00:00'
---
...
---
[scan #2 at 2026-06-12T19:21:43.276297+00:00]
## 📋 شرح
env var `GOOGLE_APPLICATION_CREDENTIALS` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
- علت: document
---
[scan #3 at 2026-06-12T19:21:43.629362+00:00]
## 📋 شرح
env var `LICENSE` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.

## 🤔 چرا مهم است
یا (الف) reader حذف شده و باعث config drift می‌شود، یا (ب) leak اطلاعات حساس به repository است (مخصوصاً اگر secret است).

## 🔍 جزئیات
- علت: documented in .env.example/READ
---
[scan #4 at 2026-06-12T19:21:43.957243+00:00]
## 📋 شرح
env var `NOTIFICATION_CRITICALITY` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
- علت: documented in 
---
[scan #5 at 2026-06-12T19:21:44.293061+00:00]
## 📋 شرح
env var `PORT` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.
...
---
[scan #6 at 2026-06-12T19:21:44.623209+00:00]
## 📋 شرح
env var `RENDER_EXTERNAL_URL` در `.env`/config تعریف شده ولی در هیچ `os.getenv` یا `process.env` خوانده نمی‌شود.

## 🤔 چرا مهم است
یا (الف) reader حذف شده و باعث config drift می‌شود، یا (ب) leak اطلاعات حساس به repository است (مخصوصاً اگر secret است).

## 🔍 جزئیات
- علت: documented in .env.
