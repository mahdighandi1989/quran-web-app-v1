...
updated_at: '2026-06-12T19:14:39.393443+00:00'
target_files:
...
1. فایل `tests/backend/app/handlers/test_scan_failure_handler.py` ایجاد شود.
2. تست‌هایی برای فراخوانی `handle_scan_failure` با پیام‌های خطای مختلف اضافه شود.
3. تست‌هایی برای تأیید اینکه `notify_event` با `event='scan_failed'` و `priority='high'` و `silent=False` فراخوانی می‌شود، اضافه شود.
4. تست‌هایی برای اطمینان از ثبت جزئیات خطا اضافه شود.
