---
task_id: 3bca25c7-facf-47b9-be92-4e23a462d138
title: افزودن تست‌های واحد برای Scan Failure Handler
type: bug
priority: high
...
created_at: '2026-06-12T19:14:39.393442+00:00'
updated_at: '2026-06-12T19:32:15.036088+00:00'
target_files:
- backend/app/handlers/scan_failure_handler.py
---

# افزودن تست‌های واحد برای Scan Failure Handler

## Raw Idea

فایل `backend/app/handlers/scan_failure_handler.py` مسئول مدیریت رویدادهای `scan_failed` است که در `docs/adr/00x_silent_flag_logic.md` به عنوان رویدادهای بحرانی طبقه‌بندی شده‌اند. این فایل هیچ تست واحدی ندارد. عدم وجود تست‌ها به این معنی است که ممکن است خطاهای بحرانی در پردازش داده‌های قرآن به درستی مدیریت یا اطلاع‌رسانی نشوند، که می‌تواند منجر به از دست رفتن داده‌ها یا عدم آگاهی کاربر از مشکلات سیستمی شود.
...
- `backend/app/handlers/scan_failure_handler.py:1-end` — `handle_scan_failure` — فایل اصلی handler که نیاز به تست دارد.
  ```python
  from backend.app.notification_pipeline import notify_event
  
  def handle_scan_failure(error_message: str, details: dict = None):
      # ... (truncated)
  ```
...
Python, Pytest, Mocking (برای `notify_event`).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `backend/app/notification_pipeline.py` — ماژولی که `notify_event` را فراهم می‌کند و `scan_failure_handler` از آن استفاده می‌کند.
- `backend/app/tasks/scan_tasks.py` — ماژولی که وظایف اسکن را انجام می‌دهد و در صورت شکست، این handler را فراخوانی می‌کند.
- `docs/adr/00x_silent_flag_logic.md` — سند معماری که اهمیت و رفتار اعلان‌های بحرانی را تعریف می‌کند.
- `tests/integration/test_notification_pipeline.py` — این فایل `scan_failure_handler.py` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
این handler بخشی از pipeline اعلان‌های بحرانی است و به `notification_pipeline` برای ارسال اعلان‌ها و به `scan_tasks` برای دریافت رویدادهای شکست وابسته است. تست‌ها باید اطمینان حاصل کنند که این ارتباطات به درستی کار می‌کنند.
...
- [ ] فایل `tests/backend/app/handlers/test_scan_failure_handler.py` ایجاد شود.
- [ ] تست‌هایی برای فراخوانی `handle_scan_failure` با پیام‌های خطای مختلف اضافه شود.
- [ ] تست‌هایی برای تأیید اینکه `notify_event` با `event='scan_failed'` و `priority='high'` و `silent=False` فراخوانی می‌شود، اضافه شود.
- [ ] تست‌هایی برای اطمینان از ثبت جزئیات خطا اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست واحد جدید (`tests/backend/app/handlers/test_scan_failure_handler.py`) برای پوشش دادن منطق `handle_scan_failure` و اطمینان از اینکه اعلان‌های بحرانی به درستی ارسال می‌شوند.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `pytest tests/backend/app/handlers/test_scan_failure_handler.py`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به Mock کردن دقیق `notify_event` برای جلوگیری از side effectهای واقعی.
...
1. فایل `tests/backend/app/handlers/test_scan_failure_handler.py` ایجاد شود. _(verify: static)_
2. تست‌هایی برای فراخوانی `handle_scan_failure` با پیام‌های خطای مختلف اضافه شود. _(verify: backend_test)_
3. تست‌هایی برای تأیید اینکه `notify_event` با `event='scan_failed'` و `priority='high'` و `silent=False` فراخوانی می‌شود، اضافه شود. _(verify: backend_test)_
4. تست‌هایی برای اطمینان از ثبت جزئیات خطا اضافه شود. _(verify: backend_test)_
