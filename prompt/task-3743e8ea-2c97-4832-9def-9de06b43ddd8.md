...
updated_at: '2026-06-12T19:14:08.612944+00:00'
target_files:
- src/App.jsx
---
...
1. فایل `src/data/quranPages.json` حاوی ساختار کامل 604 صفحه قرآن است.
2. `src/App.jsx` (یا ماژول مربوطه) دادهٔ کامل صفحات را از `src/data/quranPages.json` وارد و استفاده می‌کند.
3. اسکریپت `scripts/generate-quran-pages.mjs` قادر به تولید دادهٔ کامل و صحیح صفحات است.
4. ناوبری و نمایش صفحات قرآن در UI برای تمام 604 صفحه به درستی کار می‌کند.
5. تست‌های مربوط به ساختار صفحات قرآن (مانند `src/test/quranPages.test.js`) با دادهٔ کامل با موفقیت اجرا می‌شوند.
