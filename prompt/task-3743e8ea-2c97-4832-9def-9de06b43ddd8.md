---
task_id: 3743e8ea-2c97-4832-9def-9de06b43ddd8
title: ساختار دادهٔ 604 صفحه قرآن در App.jsx ناقص است
type: bug
priority: high
...
created_at: '2026-06-12T19:14:08.612941+00:00'
updated_at: '2026-06-12T19:30:44.703444+00:00'
target_files:
- src/App.jsx
---

# ساختار دادهٔ 604 صفحه قرآن در App.jsx ناقص است

## Raw Idea

دادهٔ `QURAN_PAGE_STRUCTURE_DEFAULT` در `src/App.jsx` (خطوط 358-377 بر اساس `task-173fd615`) فقط شامل 5 صفحه از 604 صفحه قرآن است. این نقص باعث می‌شود که نمایش صفحه‌بندی قرآن برای اکثر صفحات به درستی کار نکند و تجربهٔ کاربری را به شدت تحت تأثیر قرار دهد. وجود کامنت `// ... other pages` در خط 361 نشان‌دهندهٔ عمدی بودن این نقص است، اما هیچ مکانیزمی برای تکمیل خودکار یا بارگذاری دادهٔ کامل وجود ندارد.
...
- `src/App.jsx:358-377 (بر اساس task-173fd615)` — `QURAN_PAGE_STRUCTURE_DEFAULT` — این آرایه باید با دادهٔ کامل 604 صفحه جایگزین شود.
  ```jsx
  const QURAN_PAGE_STRUCTURE_DEFAULT = [
    { page: 1, surah: 1, ayah: 1, start: '1:1', end: '1:7' },
    { page: 2, surah: 2, ayah: 1, start: '2:1', end: '2:5' },
    // ... other pages
    { page: 602, surah: 109, ayah: 1, start: '109:1', end: '109:6' },
    { page: 603, surah: 113, ayah: 1, start: '113:1', end: '113:5' },
    { page: 604, surah: 114, ayah: 1, start: '114:1', end: '114:6' },
  ];
  ```
...
React, JavaScript, JSON data

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `scripts/generate-quran-pages.mjs` — اسکریپت تولیدکننده داده صفحات قرآن
- `src/data/quranPages.json` — فایل مقصد برای ذخیره داده کامل صفحات
- `src/lib/quran.js` — منطق مربوط به قرآن و صفحات
- `src/test/quranPages.test.js` — تست‌های مربوط به صفحات قرآن
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر تمام بخش‌های برنامه که به ساختار صفحات قرآن نیاز دارند (مانند ناوبری، نمایش، و تمرین) تأثیر می‌گذارد. نیاز به تولید دادهٔ جدید و به‌روزرسانی مکانیسم بارگذاری آن دارد.
...
- [ ] فایل `src/data/quranPages.json` حاوی ساختار کامل 604 صفحه قرآن است.
- [ ] `src/App.jsx` (یا ماژول مربوطه) دادهٔ کامل صفحات را از `src/data/quranPages.json` وارد و استفاده می‌کند.
- [ ] اسکریپت `scripts/generate-quran-pages.mjs` قادر به تولید دادهٔ کامل و صحیح صفحات است.
- [ ] ناوبری و نمایش صفحات قرآن در UI برای تمام 604 صفحه به درستی کار می‌کند.
- [ ] تست‌های مربوط به ساختار صفحات قرآن (مانند `src/test/quranPages.test.js`) با دادهٔ کامل با موفقیت اجرا می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ساختار کامل 604 صفحه قرآن را برای `QURAN_PAGE_STRUCTURE_DEFAULT` تولید و در `src/data/quranPages.json` ذخیره کنید. سپس `src/App.jsx` باید این دادهٔ کامل را از `src/data/quranPages.json` وارد کند. اسکریپت `scripts/generate-quran-pages.mjs` باید برای تولید این داده استفاده یا به‌روزرسانی شود.

## 💡 نمونه‌های قبل/بعد
**استفاده از داده کامل صفحات**

_قبل:_
```
const QURAN_PAGE_STRUCTURE_DEFAULT = [ ... 5 pages ... ];
```

_بعد:_
```
import quranPages from './data/quranPages.json';
const QURAN_PAGE_STRUCTURE_DEFAULT = quranPages;
```
...
## 🧪 دستورات اعتبارسنجی
- `node scripts/generate-quran-pages.mjs`
- `npm test src/test/quranPages.test.js`
- `Manual: برنامه را اجرا کنید و به صفحات مختلف قرآن (مثلاً صفحه 300) بروید تا از صحت نمایش اطمینان حاصل کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال خطای تولید داده یا ناسازگاری با منطق موجود در `src/lib/quran.js`.
...
1. فایل `src/data/quranPages.json` حاوی ساختار کامل 604 صفحه قرآن است. _(verify: static)_
2. `src/App.jsx` (یا ماژول مربوطه) دادهٔ کامل صفحات را از `src/data/quranPages.json` وارد و استفاده می‌کند. _(verify: static)_
3. اسکریپت `scripts/generate-quran-pages.mjs` قادر به تولید دادهٔ کامل و صحیح صفحات است. _(verify: backend_test)_
4. ناوبری و نمایش صفحات قرآن در UI برای تمام 604 صفحه به درستی کار می‌کند. _(verify: ui_interaction)_
5. تست‌های مربوط به ساختار صفحات قرآن (مانند `src/test/quranPages.test.js`) با دادهٔ کامل با موفقیت اجرا می‌شوند. _(verify: backend_test)_
