...
updated_at: '2026-06-12T19:32:52.198683+00:00'
target_files:
- README.md
- index.html
- src/App.jsx
...
1. کتابخانه XLSX به صورت پویا از یک CDN (مثلاً `unpkg.com/xlsx`) بارگذاری می‌شود. _(verify: static)_
2. تابع `parseExcelFile` به درستی از `window.XLSX` بارگذاری شده پویا استفاده می‌کند. _(verify: static)_
3. یک عنصر UI پایه (مثلاً یک دکمه) برای نمایش عملکرد Web Speech API اضافه شده است، یا یک کامنت TODO واضح برای پیاده‌سازی کامل اضافه شده است. _(verify: static)_
4. `README.md` به‌روزرسانی شده است تا وضعیت پیاده‌سازی هر دو ویژگی را به دقت منعکس کند. _(verify: static)_
