---
task_id: task_885da0f585ed
title: امن‌سازی کلید API و ورودی سیستم
type: other
priority: critical
...
created_at: '2026-06-06T05:22:29.163211+00:00'
updated_at: '2026-06-06T05:22:54.237528+00:00'
tags:
- consolidated
- post_verify_merge
---

# امن‌سازی کلید API و ورودی سیستم

## Raw Idea

🧬 این یک تسک تلفیقی است — از 4 تسک منفرد ساخته شده.
📌 دلیل تلفیق (rationale توسط AI): این خوشه شامل تمامی تسک‌های امنیتی با اولویت بالا و حیاتی است که به افشای کلید API، آسیب‌پذیری‌های XSS و Open Redirect، و اعتبارسنجی ورودی‌های حساس (مانند baseUrl در تنظیمات AI) می‌پردازد. همچنین شامل رفع یک anti-pattern امنیتی مرتبط با فرضیات کهنه است.
🎯 theme: رفع آسیب‌پذیری‌های امنیتی حیاتی و بهبود اعتبارسنجی ورودی
💎 estimated_difficulty: large

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 1 از 4
  id: 9231022b-1a13-4ba9-be67-ff3644d7440d
  عنوان اصلی: کلید API هوش مصنوعی در bundle کلاینت افشا می‌شود
  اولویت اصلی: critical
  وضعیت verify قبلی: error
  فایل‌های دخیل: src/components/AISettings.jsx, src/lib/aiClient.js

📋 acceptance_criteria کامل:
  - کلید API هرگز در network tab مرورگر دیده نشود [verify_method=ui_interaction] [verify_plan={"base": "frontend", "ui_steps": [{"action": "navigate", "url": "/ai-chat"}, {"action": "wait_for_load", "state": "networkidle"}, {"action": "fill", "selector": "[data-testid='ai-prompt-input']", "val]
  - پراکسی بک‌اند درخواست‌های AI را با کلید ذخیره‌شده در سرور ارسال کند [verify_method=api_response] [verify_plan={"method": "POST", "path": "/api/ai/proxy", "headers": {"Content-Type": "application/json"}, "json_body": {"prompt": "What is the capital of France?"}, "expected_status": 200, "required_fields": ["res]
  - کاربران مهمان (غیرواردشده) همچنان بتوانند از AI استفاده کنند (کلید در سشن موقت) [verify_method=ui_interaction] [verify_plan={"base": "frontend", "ui_steps": [{"action": "navigate", "url": "/ai-chat"}, {"action": "wait_for_load", "state": "networkidle"}, {"action": "assert_visible", "selector": "[data-testid='guest-user-ind]
  - هشدار موجود در AISettings حذف یا به مستندات [verify_method=static] [verify_plan={"grep_patterns": ["فراخوانی مستقیم از مرورگر، کلید را در همان درخواست افشا می‌کند"], "files_hint": ["src/components/AISettings.jsx"]}]
...
کلید API هوش مصنوعی در bundle کلاینت افشا می‌شود

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/components/AISettings.jsx:166-168` — `AISettings return` — هشدار موجود است اما راه‌حل پیاده‌سازی نشده
  ```jsx
  ...
  ```
- `src/lib/aiClient.js:1-50` — `chatComplete` — کلید در هدر Authorization به API خارجی ارسال می‌شود
  ```jsx
...
React 18 + Vite + Firebase Firestore + OpenAI/Anthropic/Gemini API

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/lib/aiStore.js` (سطر 1) — کلیدها را در Firestore ذخیره/بازیابی می‌کند
- `src/App.jsx` (سطر 1) — تنظیمات AI را به AISettings پاس می‌دهد
- `server/telegram-bot.mjs` (سطر 1) — می‌تواند به عنوان پراکسی استفاده شود
...
## 🌐 نقشهٔ وابستگی‌ها
AISettings توسط App.jsx استفاده می‌شود؛ aiClient.js توسط 5 فایل دیگر import شده است.

## 🔍 Context و وضعیت فعلی
در `src/components/AISettings.jsx` خط 166-168، هشدار صریحی وجود دارد: «فراخوانی مستقیم از مرورگر، کلید را در همان درخواست افشا می‌کند». کلید API کاربر (مثلاً OpenAI) در state کامپوننت نگه‌داری می‌شود و مستقیماً از `src/lib/aiClient.js` به endpointهای خارجی ارسال می‌شود. این یعنی هر کد مخرب یا افزونهٔ مرورگر می‌تواند کلید را از network tab یا حافظه بخواند. همچنین کلید در Firestore ذخیره می‌شود (برای کاربران واردشده) و در پاسخ‌های Firestore به کلاینت ارسال می‌شود.

## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
verify می‌تواند پیاده‌سازی متفاوت ولی هم‌ارز را قبول کند.

- [ ] کلید API هرگز در network tab مرورگر دیده نشود
- [ ] پراکسی بک‌اند درخواست‌های AI را با کلید ذخیره‌شده در سرور ارسال کند
- [ ] کاربران مهمان (غیرواردشده) همچنان بتوانند از AI استفاده کنند (کلید در سشن موقت)
- [ ] هشدار موجود در AISettings حذف یا به مستندات
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک پراکسی بک‌اند (مثلاً Cloud Function یا endpoint در server/telegram-bot.mjs) ایجاد کن که کلید را از سرور بخواند و درخواست‌های AI را از سمت سرور ارسال کند. کلاینت فقط یک شناسهٔ سشن یا درخواست رمزنگاری‌شده به پراکسی بفرستد.

## 💡 نمونه‌های قبل/بعد
**ارسال مستقیم کلید از کلاینت**
...
_بعد:_
```
fetch('/api/ai/chat', { method: 'POST', body: JSON.stringify({ provider, model, messages }) })
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 4
  id: 55260d04-cb65-4bda-99d0-9f71526ae59b
  عنوان اصلی: عدم اعتبارسنجی ورودی در handleInspectorCommand — آسیب‌پذیری XSS و Open Redirect
  اولویت اصلی: critical
  وضعیت verify قبلی: error
  فایل‌های دخیل: -
...
📋 acceptance_criteria کامل:
  ...
  - linter بدون warning عبور می‌کند [verify_method=static] [verify_plan={"grep_patterns": ["warning:", "Linter error:"], "files_hint": ["frontend/index.html", "frontend/js/"]}]
  - type-check موفق است [verify_method=static] [verify_plan={"grep_patterns": ["error TS", "Type error:"], "files_hint": ["frontend/index.html", "frontend/js/"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
عدم اعتبارسنجی ورودی در handleInspectorCommand — آسیب‌پذیری XSS و Open Redirect
...
## 🔍 Context و وضعیت فعلی
تابع `handleInspectorCommand` در `index.html` (خطوط 294-321) ورودی‌های `msg.selector` و `msg.url` را بدون هیچ sanitization به `document.querySelector` و `window.location.assign` می‌دهد. این می‌تواند منجر به XSS (از طریق selectorهای
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 4
  id: 58c3cc17-ff47-46a9-af63-967081733274
  عنوان اصلی: اعتبارسنجی baseUrl در AISettings.addCustomProvider
  اولویت اصلی: high
...
📋 acceptance_criteria کامل:
  ...
  - linter بدون warning عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/lint.py::test_no_warnings", "timeout_seconds": 60}]
  - type-check موفق است [verify_method=backend_test] [verify_plan={"test_node": "tests/typecheck.py::test_no_type_errors", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
AISettings: عدم اعتبارسنجی baseUrl در addCustomProvider
...
## 🔍 Context و وضعیت فعلی
در src/components/AISettings.jsx خط 54، baseUrl فقط با replace(/\/$/, '') تمیز می‌شود اما هیچ اعتبارسنجی روی پروتکل (HTTP/HTTPS) یا قالب URL انجام نمی‌شود. کاربر می‌تواند آدرس‌هایی مانند 'javascript:alert(1)' یا 'file:///etc/p
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 4 از 4
  id: 0b16076f-5869-493c-968e-b5c530241149
  عنوان اصلی: Address Stale Assumption Anti-pattern Security Risk
  اولویت اصلی: high
  وضعیت verify قبلی: partial
...
📋 acceptance_criteria کامل:
  ...
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["// Justification for anthropic-dangerous-direct-browser-access header:", "// This header is intentionally included because the aiClient.js file is designed to run directly in the b]
  - تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_ai_client_security.py::test_anthropic_header_handling_edge_case", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
Anti-pattern: Stale assumption / Security Risk

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/lib/aiClient.js:47`
...
## 🔍 Context و وضعیت فعلی
The `anthropic-dangerous-direct-browser-access: 'true'` header is included in the Anthropic API request. If this `aiClient.js` file is intended to run directly in the browser, this header is required, but it also implies that the `key` (API key) is exposed client-side, which is a significant security vulnerability. If this code is meant to run on a server-side proxy (e.g., a Next.js API route), th

📁 file: src/lib/aiClient.js (line 47)
...
- در commit message: `merged-from: 9231022b-1a13-4ba9-be67-ff3644d7440d, 55260d04-cb65-4bda-99d0-9f71526ae59b, 58c3cc17-ff47-46a9-af63-967081733274, 0b16076f-5869-493c-968e-b5c530241149`
...
1. کلید API هرگز در network tab مرورگر دیده نشود _(verify: ui_interaction)_
2. پراکسی بک‌اند درخواست‌های AI را با کلید ذخیره‌شده در سرور ارسال کند _(verify: api_response)_
3. کاربران مهمان (غیرواردشده) همچنان بتوانند از AI استفاده کنند (کلید در سشن موقت) _(verify: ui_interaction)_
4. هشدار موجود در AISettings حذف یا به مستندات _(verify: static)_
