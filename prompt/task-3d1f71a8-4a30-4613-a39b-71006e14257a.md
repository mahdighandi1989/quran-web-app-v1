---
task_id: 3d1f71a8-4a30-4613-a39b-71006e14257a
title: تابع `pickAyahs` در `AIExamGenerator.jsx` از الگوریتم shuffle بایاس‌دار استفاده می‌کند.
type: bug
priority: medium
...
created_at: '2026-06-12T19:19:54.113285+00:00'
updated_at: '2026-06-12T19:36:21.018612+00:00'
target_files:
- src/components/AIExamGenerator.jsx
---

# تابع `pickAyahs` در `AIExamGenerator.jsx` از الگوریتم shuffle بایاس‌دار استفاده می‌کند.

## Raw Idea

در کامپوننت `AIExamGenerator.jsx`، تابع `pickAyahs` (خط 25) از `Array.prototype.sort(() => Math.random() - 0.5)` برای shuffle کردن آیات استفاده می‌کند. این روش یک الگوریتم shuffle بایاس‌دار است که توزیع یکنواختی از جایگشت‌ها را تولید نمی‌کند. این می‌تواند منجر به تکرار الگوهای خاص در انتخاب آیات برای آزمون شود و تجربهٔ کاربری را تحت تأثیر قرار دهد، به خصوص اگر کاربر انتظار تنوع واقعی در سوالات را داشته باشد.
...
- `src/components/AIExamGenerator.jsx:25` — `pickAyahs`
  ```jsx
...
JavaScript array manipulation, randomness, React.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/components/AIExamGenerator.jsx` — حاوی تابع متأثر است.
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر داخلی به کامپوننت `AIExamGenerator` است و تصادفی بودن انتخاب آیات برای آزمون‌ها را بهبود می‌بخشد. هیچ وابستگی خارجی مستقیماً تحت تأثیر قرار نمی‌گیرد، اما کیفیت آزمون‌های تولید شده بهبود خواهد یافت.
...
- [ ] تابع `pickAyahs` از یک الگوریتم shuffling از نظر آماری صحیح استفاده می‌کند.
- [ ] آزمون‌های تولید شده توزیع یکنواخت‌تری از انتخاب آیات را نشان می‌دهند.
- [ ] تست‌های موجود برای `AIExamGenerator` (در صورت وجود) همچنان با موفقیت اجرا می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. الگوریتم shuffle را به یک روش صحیح و بدون بایاس مانند Fisher-Yates (Knuth shuffle) تغییر دهید تا اطمینان حاصل شود که آیات به طور تصادفی و بدون بایاس انتخاب می‌شوند.
...
**بعد (با استفاده از Fisher-Yates)**
...
// ... در تابع pickAyahs
const shuffled = shuffleArray(src.slice()).slice(0, Math.min(40, Math.max(count * 3, 10)));
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run dev (و بررسی دستی تولید آزمون‌های متعدد)`

## ⚠️ ریسک‌ها و موارد احتیاط
جزئی، اطمینان از اینکه منطق shuffle جدید مشکلات دیگری ایجاد نمی‌کند.
...
1. تابع `pickAyahs` از یک الگوریتم shuffling از نظر آماری صحیح استفاده می‌کند. _(verify: static)_
2. آزمون‌های تولید شده توزیع یکنواخت‌تری از انتخاب آیات را نشان می‌دهند. _(verify: manual_only)_
3. تست‌های موجود برای `AIExamGenerator` (در صورت وجود) همچنان با موفقیت اجرا می‌شوند. _(verify: backend_test)_
