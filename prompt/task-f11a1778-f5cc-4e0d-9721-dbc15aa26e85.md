---
task_id: f11a1778-f5cc-4e0d-9721-dbc15aa26e85
title: عدم استفاده از `VITE_FIREBASE_MEASUREMENT_ID` برای Firebase Analytics
type: bug
priority: high
...
created_at: '2026-06-12T19:14:39.050213+00:00'
updated_at: '2026-06-12T19:32:06.619461+00:00'
target_files:
- src/lib/firebase.js
---

# عدم استفاده از `VITE_FIREBASE_MEASUREMENT_ID` برای Firebase Analytics

## Raw Idea

فایل `.env.example` متغیر `VITE_FIREBASE_MEASUREMENT_ID` را برای پیکربندی Google Analytics 4 از طریق Firebase Analytics تعریف می‌کند. `src/lib/analytics.js` مسئول ثبت تعاملات کاربر است و باید این رویدادها را به GA4 ارسال کند. با این حال، هیچ شواهدی مبنی بر استفاده از `VITE_FIREBASE_MEASUREMENT_ID` برای مقداردهی اولیه Firebase Analytics در `src/lib/firebase.js` یا `src/lib/analytics.js` وجود ندارد. این بدان معناست که ردیابی KPIهای تعامل کاربر در GA4 ناقص است و داده‌های تحلیلی مهمی از دست می‌روند.
...
- `src/lib/firebase.js:نامشخص` — `initializeApp` — فایل مقداردهی اولیه Firebase که باید از متغیرهای محیطی استفاده کند.
  ```jsx
  نامشخص
  ```
...
Firebase, Google Analytics 4, Vite environment variables, JavaScript.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.env.example` (سطر 13) — تعریف متغیر محیطی `VITE_FIREBASE_MEASUREMENT_ID` و سایر متغیرهای Firebase
- `src/lib/analytics.js` (سطر نامشخص) — ماژول مسئول ثبت و ارسال رویدادهای تحلیلی
- `docs/engagement-kpi.md` (سطر 19) — مستندات تعریف‌کننده KPI و منبع حقیقت برای ردیابی
- `src/App.test.jsx` (سطر 16) — تست‌هایی که Firebase Analytics را mock می‌کنند
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر بر مقداردهی اولیه Firebase و ارسال رویدادهای تحلیلی به GA4 تأثیر می‌گذارد. اطمینان از استفاده صحیح از تمام متغیرهای `VITE_FIREBASE_` برای عملکرد کامل Firebase ضروری است.
...
- [ ] Firebase Analytics با `VITE_FIREBASE_MEASUREMENT_ID` به درستی مقداردهی اولیه شود.
- [ ] رویدادهای `trackInteraction` به Firebase Analytics ارسال شوند.
- [ ] تست واحد یا E2E برای تأیید ارسال رویدادهای تحلیلی به GA4 اضافه شود (با mock کردن Firebase Analytics).
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. در `src/lib/firebase.js`، `firebase.initializeApp` را با استفاده از `VITE_FIREBASE_MEASUREMENT_ID` (و سایر متغیرهای `VITE_FIREBASE_*`) مقداردهی اولیه کنید تا Firebase Analytics به درستی فعال شود. این پیکربندی باید شامل تمام متغیرهای `VITE_FIREBASE_` باشد که در `.env.example` تعریف شده‌اند.

## 💡 نمونه‌های قبل/بعد
**پیکربندی کامل Firebase با متغیرهای محیطی**

_قبل:_
```
const firebaseConfig = {
  // ... other configs
  // measurementId: "your-measurement-id", // potentially hardcoded or missing
};
...
## 🧪 دستورات اعتبارسنجی
- `(تست دستی) پس از تعامل با برنامه، کنسول Firebase/GA4 را برای رویدادهای ورودی بررسی کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال خطای پیکربندی Firebase.
...
1. Firebase Analytics با `VITE_FIREBASE_MEASUREMENT_ID` به درستی مقداردهی اولیه شود. _(verify: static)_
2. رویدادهای `trackInteraction` به Firebase Analytics ارسال شوند. _(verify: ui_interaction)_
3. تست واحد یا E2E برای تأیید ارسال رویدادهای تحلیلی به GA4 اضافه شود (با mock کردن Firebase Analytics). _(verify: backend_test)_
