---
task_id: 26989bdf-c17d-44dc-9656-b409483eaf3e
title: 'رفع آسیب‌پذیری امنیتی: حذف هدر `anthropic-dangerous-direct-browser-access` در `aiClient.js`'
type: security
priority: critical
...
created_at: '2026-06-12T19:06:47.543696+00:00'
updated_at: '2026-06-12T19:26:21.650902+00:00'
target_files:
- src/lib/aiClient.js
---

# رفع آسیب‌پذیری امنیتی: حذف هدر `anthropic-dangerous-direct-browser-access` در `aiClient.js`

## Raw Idea

فایل `src/lib/aiClient.js` احتمالاً شامل هدر `anthropic-dangerous-direct-browser-access: 'true'` در درخواست‌های API Anthropic است. این هدر برای دسترسی مستقیم از مرورگر لازم است، اما به این معنی است که کلید API (key) در سمت کلاینت افشا می‌شود که یک آسیب‌پذیری امنیتی جدی است. با توجه به وجود `VITE_AI_PROXY_URL` در `.env.example` و توضیحات `server/README.md` که به استفاده از یک پراکسی بک‌اند برای مسیریابی درخواست‌های AI اشاره دارد تا کلیدها به مرورگر نرسند، این هدر با استراتژی امنیتی پروژه در تضاد است.
...
- `src/lib/aiClient.js:~47` — `chatComplete`
  ```jsx
  // Hypothetical snippet from src/lib/aiClient.js
  // ...
  headers: {
    'x-api-key': key,
    'anthropic-version': '2023-06-01',
    'anthropic-beta': 'messages-2023-12-15',
    'anthropic-dangerous-direct-browser-access': 'true', // This line needs to be removed
  },
  // ...
  ```
...
Frontend (React/Vite), Backend (Node.js/Express-like proxy), AI API integration

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.env.example` — متغیر `VITE_AI_PROXY_URL` که برای استراتژی پراکسی مرکزی است را تعریف می‌کند.
- `server/ai-proxy.mjs` — پراکسی بک‌اند که باید درخواست‌های AI را به صورت امن مدیریت کند.
- `server/ai.mjs` — منطق AI بک‌اند.
- `src/components/AISettings.jsx` — کلیدها و پروایدرهای AI را مدیریت می‌کند و `aiClient.js` را فراخوانی می‌کند.
- `src/test/aiClientSecurity.test.js` — این تست باید تأیید کند که کلیدها در سمت کلاینت افشا نمی‌شوند.
- `src/components/AIExamGenerator.jsx` — از `aiClient.js` استفاده می‌کند.
- `src/components/AIWidgets.jsx` — از `aiClient.js` استفاده می‌کند.
- `src/App.jsx` — از `aiClient.js` استفاده می‌کند.
- `src/lib/telegramCommands.js` — از `aiClient.js` استفاده می‌کند.
- `src/test/aiClient.test.js` — تست‌های `aiClient.js`.
- `src/lib/aiProviders.js` — `aiClient.js` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این تغییر یک آسیب‌پذیری امنیتی جدی را برطرف می‌کند و تضمین می‌کند که کلیدهای API AI هرگز در مرورگر افشا نمی‌شوند، بلکه از طریق یک پراکسی امن در بک‌اند مدیریت می‌شوند.
...
- [ ] هدر `anthropic-dangerous-direct-browser-access` از `src/lib/aiClient.js` حذف شده است.
- [ ] `src/lib/aiClient.js` از `VITE_AI_PROXY_URL` (در صورت تنظیم) برای تمام فراخوانی‌های API AI استفاده می‌کند.
- [ ] یک تست جدید یا تست موجود در `src/test/aiClientSecurity.test.js` تأیید می‌کند که کلیدهای API در سمت مرورگر برای Anthropic یا سایر پروایدرها در صورت پیکربندی پراکسی ارسال نمی‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. بررسی `src/lib/aiClient.js` برای وجود هدر `anthropic-dangerous-direct-browser-access` و حذف آن. اطمینان حاصل کنید که `chatComplete` از `VITE_AI_PROXY_URL` (اگر تنظیم شده باشد) برای مسیریابی درخواست‌های AI استفاده می‌کند تا کلید API هرگز به مرورگر نرسد.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## ⚠️ ریسک‌ها و موارد احتیاط
ایجاد تغییرات مخرب در صورت عدم پیکربندی صحیح پراکسی یا نیاز برخی پروایدرهای AI به هدرهای خاص سمت کلاینت برای دسترسی مستقیم.
...
1. هدر `anthropic-dangerous-direct-browser-access` از `src/lib/aiClient.js` حذف شده است. _(verify: static)_
2. `src/lib/aiClient.js` از `VITE_AI_PROXY_URL` (در صورت تنظیم) برای تمام فراخوانی‌های API AI استفاده می‌کند. _(verify: ui_interaction)_
3. یک تست جدید یا تست موجود در `src/test/aiClientSecurity.test.js` تأیید می‌کند که کلیدهای API در سمت مرورگر برای Anthropic یا سایر پروایدرها در صورت پیکربندی پراکسی ارسال نمی‌شوند. _(verify: backend_test)_
