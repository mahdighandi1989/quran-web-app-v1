...
updated_at: '2026-06-12T19:06:47.543698+00:00'
target_files:
- src/lib/aiClient.js
---
...
1. هدر `anthropic-dangerous-direct-browser-access` از `src/lib/aiClient.js` حذف شده است.
2. `src/lib/aiClient.js` از `VITE_AI_PROXY_URL` (در صورت تنظیم) برای تمام فراخوانی‌های API AI استفاده می‌کند.
3. یک تست جدید یا تست موجود در `src/test/aiClientSecurity.test.js` تأیید می‌کند که کلیدهای API در سمت مرورگر برای Anthropic یا سایر پروایدرها در صورت پیکربندی پراکسی ارسال نمی‌شوند.
