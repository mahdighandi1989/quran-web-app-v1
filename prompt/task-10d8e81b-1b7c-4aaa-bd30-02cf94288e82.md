---
task_id: 10d8e81b-1b7c-4aaa-bd30-02cf94288e82
...
created_at: '2026-06-12T19:21:51.872360+00:00'
updated_at: '2026-06-12T19:38:22.404663+00:00'
target_files:
...
The `configureAnalytics` function (line 83) only validates that the provided `sink` object has a `logEvent` method. However, the `expected_inputs` contract for this module explicitly states that the analytics sink object should have both `logEvent` and `analytics` properties. Subsequently, `forwardToSink` (line 89) attempts to call `_sink.logEvent` with `_sink.analytics` as its first argument. Thi

📁 file: src/lib/analytics.js (line 83)
...
- `src/components/AIExamGenerator.jsx` — این فایل `analytics.js` را import می‌کند (caller)
- `src/components/AIWidgets.jsx` — این فایل `analytics.js` را import می‌کند (caller)
- `src/components/EngagementNudge.jsx` — این فایل `analytics.js` را import می‌کند (caller)
- `src/App.jsx` — این فایل `analytics.js` را import می‌کند (caller)
