---
task_id: 90807ac1-6504-4a3a-8cba-b67bd09f3572
title: No user feedback for clipboard copy failures in `AIWidgets.jsx`
type: bug
priority: low
...
created_at: '2026-06-12T19:23:13.260744+00:00'
updated_at: '2026-06-12T19:41:19.016280+00:00'
target_files:
- src/components/AIWidgets.jsx
---

# No user feedback for clipboard copy failures in `AIWidgets.jsx`

## Raw Idea

In `src/components/AIWidgets.jsx`, the `AIAssistButton` component includes a copy button (`📋`) that calls `navigator.clipboard?.writeText(text)`. While the optional chaining `?.` prevents a crash if `navigator.clipboard` is unavailable, the `writeText` operation itself can fail (e.g., due to browser security policies requiring user gesture, permissions issues, or the clipboard being temporarily unavailable). Currently, there is no visual feedback to the user if the copy operation succeeds or fails, leading to an ambiguous user experience where they might assume the text was copied when it wasn't.
...
- `src/components/AIWidgets.jsx:52-52` — `AIAssistButton` — The onClick handler for the copy button needs to provide feedback.
  ```jsx
  <button className="btn-icon" title="کپی" onClick={() => navigator.clipboard?.writeText(text)}>📋</button>
  ```
...
React, Web Clipboard API, user interface feedback.
...
- `src/App.jsx` — این فایل `AIWidgets.jsx` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
This is a client-side user experience improvement within a single component. It has no external dependencies or downstream impacts.
...
- [ ] When the copy button is clicked, a temporary visual indicator (e.g., a toast message or text change) confirms success.
- [ ] If the copy operation fails, a temporary visual indicator informs the user of the failure.
- [ ] The component's state is updated to manage the display of these feedback messages.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Implement a `try/catch` block around the `navigator.clipboard.writeText` call. On success, display a temporary 'Copied!' message. On failure, display a temporary 'Failed to copy.' message or a similar indication to the user.

## 💡 نمونه‌های قبل/بعد
**Silent copy vs. copy with feedback**
...
_بعد:_
```
onClick={async () => { try { await navigator.clipboard?.writeText(text); // Implement logic to show 'Copied!' message } catch (e) { console.error('Failed to copy to clipboard:', e); // Implement logic to show 'Failed to copy!' message } }}
```
...
## 🧪 دستورات اعتبارسنجی
- `Manually test the copy button in `AIAssistButton` in various browsers.`
- `Manually test the copy button in a browser where clipboard access is restricted or fails (e.g., by revoking permissions or simulating an error in dev tools) and verify failure feedback.`
...
1. When the copy button is clicked, a temporary visual indicator (e.g., a toast message or text change) confirms success. _(verify: ui_interaction)_
2. If the copy operation fails, a temporary visual indicator informs the user of the failure. _(verify: ui_interaction)_
3. The component's state is updated to manage the display of these feedback messages. _(verify: static)_
