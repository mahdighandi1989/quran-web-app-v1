---
task_id: e792788b-15df-4edb-b8a9-4a342b5188c8
title: '[منطق] Inconsistent `silent` Notification for Critical Events'
type: logic_audit
priority: critical
...
created_at: '2026-06-12T19:09:54.199630+00:00'
updated_at: '2026-06-12T19:28:05.453194+00:00'
---

# [منطق] Inconsistent `silent` Notification for Critical Events
...
The prompt specifically asks if `silent` notifications are used for critical events. Logically, critical events require immediate attention and should typically trigger an audible notification to ensure they are not missed. Using `silent` for critical events is a direct contradiction to the nature of a 'critical' alert, as it significantly increases the risk of a critical issue being overlooked.

## 💥 پیامد (impact)
If critical events are sent silently, users or operators might miss crucial alerts, leading to delayed responses, system failures, data corruption, or other severe consequences. This is a major design flaw for any alerting system.

## 🛠 پیشنهاد رفع اولیه
Ensure that `silent` notifications are *never* used for critical events. Implement a clear policy where critical events always trigger audible notifications. `silent` should be reserved for non-urgent, informational updates where an audible alert is not desired.
...
### Step 1: بررسی اولیه سیستم، شناسایی وابستگی‌ها و اعتبارسنجی مشکل
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی کامل ریپازیتوری، فایل‌های مرتبط، و ساختار کلی pipeline `notification` است تا وضعیت فعلی پیاده‌سازی و وجود مشکل ناسازگاری `silent` برای رویدادهای حیاتی تأیید شود. همچنین، شناسایی تمام وابستگی‌های upstream و downstream برای اطمینان از عدم ایجاد شکست در بخش‌های دیگر سیستم حیاتی است. هدف این است که قبل از هرگونه تغییر کد، درک کاملی از محیط و تأثیرات احتمالی به دست آید و از پیاده‌سازی مجدد قابلیت‌های موجود جلوگیری شود.
**Excerpt:**
```
ممکن است **بخشی یا تمامِ** این درخواست قبلاً (به صورت کامل یا ناقص) در repo پیاده‌سازی شده باشد. پیش از شروع، با grep/search و خواندن فایل‌های مرتبط بررسی کن که چه چیزی **از قبل وجود دارد**. اگر یک قابلیت/فایل/تابع از قبل موجود است: آن را **دوباره نساز**؛ فقط موارد ناقص یا اشتباه را اصلاح/تکمیل کن. اگر همه چیز از قبل به‌درستی انجام شده: یک کامیت توضیحی (no-op) ثبت کن که چرا تغییری لازم نبود و دقیقاً کدام فایل‌ها این درخواست را پوشش می‌دهند. پیش از هر تغییر، خودت ساختار repo، فایل‌های ذکرشده، و وابستگی‌های آن‌ها را مستقل بررسی کن. اگر تشخیص دادی موقعیت ذکرشده در پرامپت اشتباه است یا فایل دیگری مناسب‌تر است، بر اساس قضاوت خودت عمل کن — این پرامپت نمی‌تواند بهانهٔ کار اشتباه باشد ("خودت گفتی" قابل قبول نیست). پیش از و حین تغییر، تمام وابستگی‌ها را در **چهار جهت** به‌طور **کامل و بدون هیچ خلاصه‌سازی** شناسایی و همگام کن: ۱. وابستگی‌های upstream (این تسک به چه چیزهایی متکی است): چه فایل‌ها، توابع، کلاس‌ها، API endpoint ها، schema های دیتابیس، env vars، یا config هایی که این تسک نیاز دارد؟ آیا قرار است چیزی را ویرایش/حذف کنی که جای دیگر (signature، رفتار، return type، side effect) از آن انتظار خاصی می‌رود؟ اگر dependency جدیدی اضافه می‌کنی، آیا با dependencyهای موجود تداخل دارد (نسخه، compat، lock file)؟ ۲. وابستگی‌های downstream (چه چیزهایی به این تسک متکی‌اند):
```

### Step 2: شناسایی تعاریف و نقاط فعال‌سازی رویدادهای حیاتی
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی یافتن و مستندسازی دقیق مکان‌هایی در کد `notification pipeline` تمرکز دارد که رویدادها به عنوان 'حیاتی' (critical) طبقه‌بندی می‌شوند و نحوه فعال‌سازی اعلان‌های مربوط به آن‌ها. این شامل شناسایی توابع، کلاس‌ها، یا ماژول‌هایی است که مسئول تشخیص و پردازش رویدادهای حیاتی هستند. درک این مکانیسم برای اطمینان از اینکه تغییرات بعدی به درستی اعمال می‌شوند، ضروری است.
**Excerpt:**
```
The prompt specifically asks if `silent` notifications are used for critical events. Logically, critical events require immediate attention and should typically trigger an audible notification to ensure they are not missed.
```

### Step 3: شناسایی موارد استفاده فعلی از اعلان‌های 'silent' برای رویدادهای حیاتی
**Status:** `pending` (0%)
**Scope:** در این مرحله، باید تمام نقاطی در `notification pipeline` که در حال حاضر اعلان‌های 'silent' برای رویدادهایی که به عنوان 'حیاتی' شناسایی شده‌اند، استفاده می‌شوند، پیدا و مستند شوند. این شامل بررسی منطق ارسال اعلان و پارامترهای مربوط به نوع اعلان (مثلاً `silent=true`) در کنار شرایطی است که رویداد به عنوان حیاتی طبقه‌بندی می‌شود. هدف، شناسایی دقیق ناسازگاری منطقی موجود است.
**Excerpt:**
```
Using `silent` for critical events is a direct contradiction to the nature of a 'critical' alert, as it significantly increases the risk of a critical issue being overlooked.
```

### Step 4: پیاده‌سازی سیاست: اطمینان از اعلان‌های شنیداری برای رویدادهای حیاتی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح منطق `notification pipeline` است تا اطمینان حاصل شود که رویدادهای حیاتی (critical events) هرگز از اعلان‌های 'silent' استفاده نمی‌کنند و همیشه یک اعلان شنیداری (audible notification) را فعال می‌کنند. این ممکن است شامل تغییر پارامترهای تابع ارسال اعلان، اضافه کردن منطق شرطی برای نادیده گرفتن تنظیمات 'silent' در صورت حیاتی بودن رویداد، یا به‌روزرسانی پیکربندی‌های مربوطه باشد. هدف اصلی، حذف کامل امکان نادیده گرفته شدن هشدارهای حیاتی است.
**Excerpt:**
```
Ensure that `silent` notifications are *never* used for critical events. Implement a clear policy where critical events always trigger audible notifications.
```

### Step 5: پیاده‌سازی سیاست: اختصاص اعلان‌های 'silent' به به‌روزرسانی‌های غیرفوری و اطلاعاتی
**Status:** `pending` (0%)
**Scope:** در این مرحله، منطق `notification pipeline` باید به‌گونه‌ای تنظیم شود که اعلان‌های 'silent' منحصراً برای به‌روزرسانی‌های غیرفوری و اطلاعاتی که نیازی به هشدار شنیداری ندارند، استفاده شوند. این شامل بازبینی و اصلاح هرگونه منطق پیش‌فرض یا پیکربندی است که ممکن است به اشتباه اعلان‌های 'silent' را برای رویدادهای با اهمیت بالاتر فعال کند. هدف، روشن کردن کاربرد صحیح اعلان‌های 'silent' و جلوگیری از سوءاستفاده از آن‌ها است.
**Excerpt:**
```
`silent` should be reserved for non-urgent, informational updates where an audible alert is not desired.
```

### Step 6: به‌روزرسانی پیکربندی‌ها و شمای اعلان‌ها (در صورت لزوم)
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بازبینی و به‌روزرسانی هرگونه فایل پیکربندی، شمای دیتابیس، یا تعاریف API است که بر رفتار `notification pipeline` تأثیر می‌گذارد. اگر سیاست‌های جدید (اعلان شنیداری برای رویدادهای حیاتی و 'silent' برای غیرفوری) در این ساختارها تعریف شده‌اند، باید اصلاح شوند تا با منطق جدید همخوانی داشته باشند. این کار برای اطمینان از پایداری و سازگاری بلندمدت سیستم حیاتی است و از بازگشت به وضعیت ناسازگار جلوگیری می‌کند.
**Excerpt:**
```
This is a major design flaw for any alerting system. Implement a clear policy where critical events always trigger audible notifications.
```

### Step 7: توسعه/به‌روزرسانی تست‌های خودکار برای رفتار اعلان‌ها
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد تست‌های واحد (unit tests) و تست‌های یکپارچه‌سازی (integration tests) جدید یا به‌روزرسانی تست‌های موجود است تا رفتار جدید `notification pipeline` را تأیید کند. تست‌ها باید اطمینان حاصل کنند که رویدادهای حیاتی همیشه اعلان‌های شنیداری را فعال می‌کنند و رویدادهای غیرفوری می‌توانند اعلان‌های 'silent' را دریافت کنند. این کار برای جلوگیری از رگرسیون و اطمینان از پایداری تغییرات در آینده ضروری است، به خصوص با توجه به اینکه 'coherence issue' ها در تست‌های معمولی به سختی پیدا می‌شوند.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 8: به‌روزرسانی مستندات سیستم و دستورالعمل‌های سیاست
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی تمام مستندات مرتبط با `notification pipeline` است تا سیاست جدید در مورد استفاده از اعلان‌های 'silent' و شنیداری برای رویدادهای مختلف را منعکس کند. این شامل مستندات فنی، راهنماهای توسعه‌دهندگان، و هرگونه دستورالعمل عملیاتی است. هدف، اطمینان از اینکه تمام اعضای تیم از سیاست جدید آگاه هستند و در آینده نیز آن را رعایت می‌کنند.
**Excerpt:**
```
Implement a clear policy where critical events always trigger audible notifications. `silent` should be reserved for non-urgent, informational updates where an audible alert is not desired.
```
