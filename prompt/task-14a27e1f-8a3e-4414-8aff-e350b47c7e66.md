---
task_id: 14a27e1f-8a3e-4414-8aff-e350b47c7e66
title: کامپوننت AIChat (پرسش‌وپاسخ با هوش مصنوعی) در UI استفاده نمی‌شود.
...
created_at: '2026-06-12T19:14:08.942966+00:00'
updated_at: '2026-06-12T19:31:00.265826+00:00'
target_files:
- src/App.jsx
- src/routes.jsx
---

# کامپوننت AIChat (پرسش‌وپاسخ با هوش مصنوعی) در UI استفاده نمی‌شود.

## Raw Idea

کامپوننت `AIChat` در `src/components/AIWidgets.jsx` برای ارائه قابلیت پرسش‌وپاسخ آزاد با هوش مصنوعی (AI_USE_CASE.QA) طراحی شده است، همانطور که در `docs/ai-chat-use-cases.md` مستند شده است. با این حال، این کامپوننت در هیچ کجای رابط کاربری (مانند `src/App.jsx` یا `src/routes.jsx`) import یا render نمی‌شود، که باعث می‌شود این قابلیت کلیدی هوش مصنوعی برای کاربران غیرقابل دسترس باشد. این یک ناسازگاری بین مستندات و پیاده‌سازی فعلی است و یک ویژگی وعده داده شده را غیرقابل استفاده می‌کند.
...
- `src/App.jsx:به عنوان نقطهٔ ورود اصلی، App.jsx یا کامپوننت‌های فرعی آن باید AIChat را import و render کنند.` — `App`
  ```jsx
  export default function App() { /* ... */ }
  ```
- `src/routes.jsx:اگر از سیستم مسیریابی استفاده می‌شود، AIChat باید به یک مسیر اختصاصی متصل شود.` — `Routes`
  ```jsx
...
React, Vite, AI integration, Firebase, React Router (احتمالاً).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/components/AIWidgets.jsx` (سطر 63) — فایل حاوی تعریف کامپوننت AIChat است.
- `docs/ai-chat-use-cases.md` (سطر 20) — این سند استفادهٔ مورد نظر از AIChat را برای QA مستند می‌کند.
- `src/lib/analytics.js` (سطر 8) — AIChat از trackAIInteraction برای AI_USE_CASE.QA استفاده می‌کند.
- `src/lib/aiClient.js` — AIChat برای تکمیل چت به aiClient وابسته است.
- `src/lib/aiTasks.js` — AIChat از qaPrompt از این ماژول استفاده می‌کند.
...
## 🌐 نقشهٔ وابستگی‌ها
کامپوننت AIChat به ماژول‌های `aiClient.js`، `aiTasks.js` و `analytics.js` وابسته است. ادغام آن در UI نیاز به اطمینان از دسترسی به `getConfig` و `buildPrompt` مناسب دارد.
...
- [ ] کامپوننت `AIChat` در رابط کاربری برنامه قابل دسترسی و رندر می‌شود.
- [ ] کاربران می‌توانند سوالات خود را در رابط `AIChat` وارد کرده و پاسخ‌های هوش مصنوعی را دریافت کنند.
- [ ] فراخوانی `trackAIInteraction(AI_USE_CASE.QA)` پس از دریافت پاسخ موفق از هوش مصنوعی در `AIChat` ثبت می‌شود.
- [ ] یک تست e2e جدید برای تأیید عملکرد `AIChat` و ردیابی تعامل اضافه می‌شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. کامپوننت `AIChat` را در یک تب یا صفحهٔ مناسب در `src/App.jsx` یا `src/routes.jsx` ادغام کنید تا قابلیت پرسش‌وپاسخ با هوش مصنوعی برای کاربران قابل دسترسی باشد. این ممکن است شامل ایجاد یک مسیر جدید یا افزودن آن به یک تب موجود باشد.

## 💡 نمونه‌های قبل/بعد
**نمونهٔ قبل (App.jsx یا یک کامپوننت صفحه)**

_قبل:_
```
return (<main> <SomeOtherFeature /> </main>);
```

**نمونهٔ بعد (App.jsx یا یک کامپوننت صفحه با تب جدید)**

_بعد:_
```
return (<main> <Tabs> <Tab title="..." /> <Tab title="پرسش‌وپاسخ AI"> <AIChat getConfig={getConfig} buildPrompt={qaPrompt} /> </Tab> </Tabs> </main>);
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run dev (و بررسی دستی در مرورگر)`
- `npm run test -- src/test/e2e/test_ai_conversation_initiation.py (اگر تست جدید اضافه شود)`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال تداخل با طرح‌بندی UI موجود، نیاز به مدیریت صحیح state و props برای `AIChat`.
...
1. کامپوننت `AIChat` در رابط کاربری برنامه قابل دسترسی و رندر می‌شود. _(verify: ui_interaction)_
2. کاربران می‌توانند سوالات خود را در رابط `AIChat` وارد کرده و پاسخ‌های هوش مصنوعی را دریافت کنند. _(verify: ui_interaction)_
3. فراخوانی `trackAIInteraction(AI_USE_CASE.QA)` پس از دریافت پاسخ موفق از هوش مصنوعی در `AIChat` ثبت می‌شود. _(verify: ui_interaction)_
4. یک تست e2e جدید برای تأیید عملکرد `AIChat` و ردیابی تعامل اضافه می‌شود. _(verify: backend_test)_
