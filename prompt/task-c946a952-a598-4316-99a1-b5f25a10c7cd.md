---
task_id: c946a952-a598-4316-99a1-b5f25a10c7cd
title: Critical `scan_failed` event is not triggering notifications in backend
type: bug
priority: critical
...
created_at: '2026-06-12T19:08:53.222522+00:00'
updated_at: '2026-06-12T19:27:48.714460+00:00'
target_files:
- backend/app/handlers/scan_failure_handler.py
- backend/app/tasks/scan_tasks.py
---

# Critical `scan_failed` event is not triggering notifications in backend

## Raw Idea

The `docs/adr/00x_silent_flag_logic.md` explicitly classifies `scan_failed` as a `critical` notification type, meaning it should always trigger a loud alert to the user. However, the raw idea from `prompt/archive/task-0eb7aa00.md` states that 'event `scan_failed` در سیستم به‌عنوان critical شناخته شده ولی هیچ `notify_event` call برای آن پیدا نشد.' While `src/App.test.jsx` includes a test for `notifyCriticalEvent('scan_failed', ...)` demonstrating the *mechanism* works, there is no evidence that the actual failure detection in the backend (likely `backend/app/handlers/scan_failure_handler.py` or `backend/app/tasks/scan_tasks.py`) is calling this notification function. This means that if the Quran Excel import or scan process fails, the user will not be notified, leading to a severe data integrity and operational awareness gap. The same logic applies to `task_failed` and `verify_failed` if they are also critical events.
...
- `backend/app/handlers/scan_failure_handler.py:(unknown)` — `handle_scan_failure` — This handler is the most likely place to trigger the notification for scan failures.
  ```python
  (No explicit notify_event call found)
  ```
- `backend/app/tasks/scan_tasks.py:(unknown)` — `scan_quran_files` — The task itself might need to trigger the notification on exception.
  ```python
  (No explicit notify_event call found)
  ```
...
Python backend, notification pipeline, critical event handling.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `docs/adr/00x_silent_flag_logic.md` (سطر 15) — Documents the criticality of scan_failed and other critical events.
- `src/App.test.jsx` (سطر 380) — Tests the client-side notification mechanism for scan_failed, confirming the sink exists.
- `backend/app/notification_pipeline.py` — Backend's ground truth for notification policy and event handling.
- `backend/app/tasks/scan_tasks.py` — این فایل `scan_failure_handler.py` را import می‌کند (caller)
- `tests/integration/test_notification_pipeline.py` — این فایل `scan_failure_handler.py` را import می‌کند (caller)
- `backend/app/handlers/scan_failure_handler.py` — `scan_tasks.py` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
This is a cross-tier dependency. The frontend `notifyCriticalEvent` is wired to a backend sink (Telegram bot). The backend needs to actually *call* this sink when a critical event occurs.
...
- [ ] When a `scan_failed` event occurs in the backend, a critical notification is sent via the configured notification pipeline.
- [ ] The notification is always 'loud' (not silent), as per ADR 00x.
- [ ] A new integration test is added in `tests/backend/data_pipeline/` to verify this behavior.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Identify the exact location in the backend Python code where `scan_failed` (and potentially `task_failed`, `verify_failed`) is detected (likely within `backend/app/tasks/scan_tasks.py` or handled by `backend/app/handlers/scan_failure_handler.py`) and add a call to `notifyCriticalEvent` (or its Python equivalent, assuming a bridge exists) with a high priority and non-silent flag.

## 💡 نمونه‌های قبل/بعد
**Missing notification call (conceptual, as backend file content is not provided)**
...
try:
    ...
    log_error(e)
    # Assuming a Python equivalent of notifyCriticalEvent exists or is bridged
    notify_critical_event('scan_failed', f'Quran scan failed: {e}', {'priority': 'high', 'silent': False})
```
...
## 🧪 دستورات اعتبارسنجی
- `pytest tests/backend/data_pipeline/test_integration.py -k scan_failure_notification (after adding a new test)`
- `Manually trigger a scan failure and verify Telegram notification.`

## ⚠️ ریسک‌ها و موارد احتیاط
Requires understanding backend Python code for scan process and its error paths.
...
1. When a `scan_failed` event occurs in the backend, a critical notification is sent via the configured notification pipeline. _(verify: backend_test)_
2. The notification is always 'loud' (not silent), as per ADR 00x. _(verify: backend_test)_
3. A new integration test is added in `tests/backend/data_pipeline/` to verify this behavior. _(verify: backend_test)_
