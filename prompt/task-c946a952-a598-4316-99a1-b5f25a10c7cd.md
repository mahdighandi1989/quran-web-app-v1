...
updated_at: '2026-06-12T19:08:53.222524+00:00'
target_files:
...
1. When a `scan_failed` event occurs in the backend, a critical notification is sent via the configured notification pipeline.
2. The notification is always 'loud' (not silent), as per ADR 00x.
3. A new integration test is added in `tests/backend/data_pipeline/` to verify this behavior.
