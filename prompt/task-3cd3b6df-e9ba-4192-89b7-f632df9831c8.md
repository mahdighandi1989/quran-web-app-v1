---
task_id: 3cd3b6df-e9ba-4192-89b7-f632df9831c8
title: افزودن اسکریپت‌های `test` و `test:watch` به `package.json` ریشه
type: bug
priority: high
...
created_at: '2026-06-12T19:11:44.046109+00:00'
updated_at: '2026-06-12T19:29:14.639760+00:00'
target_files:
- package.json
---

# افزودن اسکریپت‌های `test` و `test:watch` به `package.json` ریشه

## Raw Idea

فایل `package.json` در ریشه پروژه فاقد اسکریپت‌های `test` و `test:watch` است، در حالی که `README.md` (بخش «تست») به صراحت دستورات `npm test` و `npm run test:watch` را مستند کرده است. این عدم تطابق بین مستندات و پیکربندی پروژه باعث می‌شود که کاربران نتوانند تست‌های فرانت‌اند (Vitest) را به راحتی اجرا کنند و همچنین مانع از اجرای خودکار تست‌ها در CI/CD pipeline می‌شود. وجود فایل `vitest.config.js` و تعداد زیادی فایل تست در `src/test/` نشان‌دهنده قصد توسعه‌دهنده برای داشتن تست‌های فرانت‌اند است.
...
- `package.json:~10-11` — `scripts`
  ```json
  {
    ...
    "scripts": {
      ...
      // Missing "test" and "test:watch" here
    },
    "dependencies": {
      "firebase": "^12.1.0",
      "react": "^18.2.0",
...
Node.js, npm, Vitest, React

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `README.md` — اسکریپت‌های از دست رفته را مستند می‌کند.
- `vitest.config.js` — پیکربندی Vitest برای اجرای تست‌ها.
- `src/test/setup.js` — تنظیمات گلوبال برای تست‌های Vitest.
- `package-lock.json` — وابستگی‌های توسعه مانند `vitest` را تأیید می‌کند.

## 🌐 نقشهٔ وابستگی‌ها
این تغییر برای قابلیت اجرای تست‌های فرانت‌اند و یکپارچگی CI/CD ضروری است.
...
- [ ] `package.json` در دایرکتوری ریشه شامل اسکریپت‌های `test` و `test:watch` است.
- [ ] دستورات `npm test` و `npm run test:watch` تست‌های Vitest را به درستی اجرا می‌کنند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. اضافه کردن اسکریپت‌های `test` و `test:watch` به بخش `scripts` در `package.json` ریشه پروژه.
...
هیچ، زیرا Vitest از قبل به عنوان یک وابستگی توسعه نصب شده است.
...
1. `package.json` در دایرکتوری ریشه شامل اسکریپت‌های `test` و `test:watch` است. _(verify: static)_
2. دستورات `npm test` و `npm run test:watch` تست‌های Vitest را به درستی اجرا می‌کنند. _(verify: manual_only)_
