...
updated_at: '2026-06-12T19:19:52.773235+00:00'
target_files:
- index.html
- .env.example
...
1. خط 208 `index.html` برای استفاده از `'%VITE_INSPECTOR_PROJECT_ID%'` (با یک مقدار پیش‌فرض) به‌روزرسانی شده است.
2. `.env.example` شامل `VITE_INSPECTOR_PROJECT_ID` و `FIREBASE_PROJECT_ID` (برای سرور) است.
3. فایل‌های سرور Node.js (`server/telegram-bot.mjs` و غیره) به صراحت از `process.env.FIREBASE_PROJECT_ID` برای مقداردهی اولیه Firebase Admin استفاده می‌کنند.
4. یک تست جدید (یا به‌روزرسانی تست موجود) تأیید می‌کند که سرور Firebase Admin را با شناسه پروژه صحیح مقداردهی اولیه می‌کند.
