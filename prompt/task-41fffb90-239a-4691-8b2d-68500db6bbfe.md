---
task_id: 41fffb90-239a-4691-8b2d-68500db6bbfe
title: شناسه پروژه سخت‌کد شده در Inspector Bridge و عدم وجود شناسه پروژه Firebase صریح برای سرور Node.js
type: refactor
priority: medium
...
created_at: '2026-06-12T19:19:52.773232+00:00'
updated_at: '2026-06-12T19:35:47.342195+00:00'
target_files:
- index.html
- .env.example
...
# شناسه پروژه سخت‌کد شده در Inspector Bridge و عدم وجود شناسه پروژه Firebase صریح برای سرور Node.js

## Raw Idea

فایل `index.html` (خط 208) `PROJECT_ID = 'gh_mahdighandi1989_quran_web_app_v1'` را برای Inspector Bridge سخت‌کد کرده است. در حالی که این شناسه احتمالاً برای شناسایی پروژه در ابزار بازرسی استفاده می‌شود، سخت‌کد کردن آن انعطاف‌ناپذیر است و می‌تواند با شناسه واقعی پروژه Firebase که توسط سایر قسمت‌های برنامه استفاده می‌شود (مثلاً `src/components/TelegramSettings.jsx` از `VITE_FIREBASE_PROJECT_ID` استفاده می‌کند) اشتباه گرفته شود. علاوه بر این، کامپوننت‌های سرور Node.js (بات تلگرام، AI proxy) به Firebase Admin SDK متکی هستند، اما هیچ اشاره صریحی به خواندن متغیر محیطی `FIREBASE_PROJECT_ID` برای مقداردهی اولیه وجود ندارد، که یک روش خوب حتی هنگام استفاده از حساب‌های خدماتی است.
...
- `index.html:208` — `PROJECT_ID`
  ```
  const PROJECT_ID = 'gh_mahdighandi1989_quran_web_app_v1';
  ```
- `.env.example:نامشخص` — `Environment variables`
  ```
  # Add VITE_INSPECTOR_PROJECT_ID and FIREBASE_PROJECT_ID (for server)
  ```
- `server/telegram-bot.mjs:نامشخص` — `Firebase Admin SDK initialization`
  ```
  // Ensure process.env.FIREBASE_PROJECT_ID is read and used
  ```
...
متغیرهای محیطی Vite، سرور Node.js، پیکربندی Firebase Admin SDK.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/components/TelegramSettings.jsx` — از `VITE_FIREBASE_PROJECT_ID` به درستی استفاده می‌کند.
- `src/lib/firebase.js` — مقداردهی اولیه Firebase سمت فرانت‌اند.
...
## 🌐 نقشهٔ وابستگی‌ها
شناسه‌های پروژه سازگار و قابل تنظیم برای استقرار در محیط‌های مختلف و جلوگیری از سردرگمی بین انواع مختلف شناسه‌های پروژه بسیار مهم هستند. پیکربندی صریح شناسه پروژه Firebase برای سرور تضمین می‌کند که به پروژه صحیح متصل می‌شود.
...
- [ ] خط 208 `index.html` برای استفاده از `'%VITE_INSPECTOR_PROJECT_ID%'` (با یک مقدار پیش‌فرض) به‌روزرسانی شده است.
- [ ] `.env.example` شامل `VITE_INSPECTOR_PROJECT_ID` و `FIREBASE_PROJECT_ID` (برای سرور) است.
- [ ] فایل‌های سرور Node.js (`server/telegram-bot.mjs` و غیره) به صراحت از `process.env.FIREBASE_PROJECT_ID` برای مقداردهی اولیه Firebase Admin استفاده می‌کنند.
- [ ] یک تست جدید (یا به‌روزرسانی تست موجود) تأیید می‌کند که سرور Firebase Admin را با شناسه پروژه صحیح مقداردهی اولیه می‌کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. 1. برای `PROJECT_ID` Inspector Bridge در `index.html`، آن را از طریق یک متغیر محیطی اختصاصی Vite (مثلاً `VITE_INSPECTOR_PROJECT_ID`) قابل تنظیم کنید تا از سخت‌کد کردن جلوگیری شود و هدف آن به عنوان یک شناسه پروژه برای بازرس، متمایز از شناسه پروژه Firebase، روشن شود. 2. اطمینان حاصل کنید که کامپوننت‌های سرور Node.js به صراحت یک متغیر محیطی `FIREBASE_PROJECT_ID` (بدون پیشوند `VITE_`، زیرا یک متغیر محیطی زمان اجرا برای سرور است) را می‌خوانند و از آن در طول مقداردهی اولیه Firebase Admin SDK استفاده می‌کنند. 3. `.env.example` را برای بازتاب این متغیرهای محیطی جدید/روشن شده به‌روز کنید.

## 💡 نمونه‌های قبل/بعد
**قبل: شناسه پروژه سخت‌کد شده در HTML**
...
**بعد: شناسه پروژه قابل تنظیم در HTML و سرور**

_بعد:_
```
<!-- index.html -->
const PROJECT_ID = '%VITE_INSPECTOR_PROJECT_ID%' || 'gh_mahdighandi1989_quran_web_app_v1_default';

// .env.example
VITE_INSPECTOR_PROJECT_ID=your-inspector-project-id
FIREBASE_PROJECT_ID=your-firebase-project-id

// server/telegram-bot.mjs (or similar)
...
## 🧪 دستورات اعتبارسنجی
- `grep -A 5 "const PROJECT_ID" index.html`
- `grep "VITE_INSPECTOR_PROJECT_ID" .env.example`
- `grep "FIREBASE_PROJECT_ID=" .env.example`
- `grep -r "FIREBASE_PROJECT_ID" server/`

## ⚠️ ریسک‌ها و موارد احتیاط
نام‌های متغیر محیطی یا تجزیه نادرست می‌تواند منجر به خرابی اتصال Firebase شود.
...
1. خط 208 `index.html` برای استفاده از `'%VITE_INSPECTOR_PROJECT_ID%'` (با یک مقدار پیش‌فرض) به‌روزرسانی شده است. _(verify: static)_
2. `.env.example` شامل `VITE_INSPECTOR_PROJECT_ID` و `FIREBASE_PROJECT_ID` (برای سرور) است. _(verify: static)_
3. فایل‌های سرور Node.js (`server/telegram-bot.mjs` و غیره) به صراحت از `process.env.FIREBASE_PROJECT_ID` برای مقداردهی اولیه Firebase Admin استفاده می‌کنند. _(verify: static)_
4. یک تست جدید (یا به‌روزرسانی تست موجود) تأیید می‌کند که سرور Firebase Admin را با شناسه پروژه صحیح مقداردهی اولیه می‌کند. _(verify: backend_test)_

## Task Steps

### Step 1: Externalize hardcoded PROJECT_ID in index.html for Inspector Bridge
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و جایگزینی مقدار سخت‌کدشده `PROJECT_ID = 'gh_mahdighandi1989_quran_web_app_v1'` در فایل `index.html` (خط 208) با یک متغیر محیطی یا یک placeholder قابل تنظیم است. هدف این است که شناسه پروژه Inspector Bridge از کد جدا شده و از طریق پیکربندی خارجی قابل مدیریت باشد. این مرحله شامل تعریف متغیر محیطی در سیستم یا استفاده از آن در سایر فایل‌ها نیست، بلکه فقط تغییر در `index.html` را پوشش می‌دهد.
**Excerpt:**
```
فایل `index.html` (خط 208) `PROJECT_ID = 'gh_mahdighandi1989_quran_web_app_v1'` را برای Inspector Bridge سخت‌کد کرده است. در حالی که این شناسه احتمالاً برای شناسایی پروژه در ابزار بازرسی استفاده می‌شود، سخت‌کد کردن آن انعطاف‌ناپذیر است و می‌تواند با شناسه واقعی پروژه Firebase که توسط سایر قسمت‌های برنامه استفاده می‌شود (مثلاً `src/components/TelegramSettings.jsx` از `VITE_FIREBASE_PROJECT_ID` استفاده می‌کند) اشتباه گرفته شود.
```

### Step 2: Implement explicit reading of FIREBASE_PROJECT_ID in Node.js server components
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی کامپوننت‌های سرور Node.js تمرکز دارد، به ویژه بات تلگرام و AI proxy. هدف این است که اطمینان حاصل شود که این کامپوننت‌ها به صراحت متغیر محیطی `FIREBASE_PROJECT_ID` را برای مقداردهی اولیه Firebase Admin SDK می‌خوانند. این کار باعث می‌شود که پیکربندی پروژه Firebase به جای اینکه به حساب‌های خدماتی یا تنظیمات پیش‌فرض متکی باشد، به صورت واضح و قابل مدیریت از طریق متغیرهای محیطی انجام شود.
**Excerpt:**
```
علاوه بر این، کامپوننت‌های سرور Node.js (بات تلگرام، AI proxy) به Firebase Admin SDK متکی هستند، اما هیچ اشاره صریحی به خواندن متغیر محیطی `FIREBASE_PROJECT_ID` برای مقداردهی اولیه وجود ندارد، که یک روش خوب حتی هنگام استفاده از حساب‌های خدماتی است.
```

### Step 3: Ensure consistency and clarity between project IDs across frontend and backend
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و اطمینان از عدم سردرگمی بین شناسه‌های پروژه مختلف در برنامه است. به طور خاص، باید بررسی شود که آیا `PROJECT_ID` که اکنون در `index.html` از یک متغیر محیطی بارگذاری می‌شود، با `VITE_FIREBASE_PROJECT_ID` که در `src/components/TelegramSettings.jsx` استفاده می‌شود، همخوانی دارد یا خیر. اگر این دو شناسه به یک پروژه Firebase اشاره دارند، باید نام‌گذاری و مقادیر آن‌ها یکسان شود. اگر به پروژه‌های متفاوتی اشاره دارند، باید نام‌گذاری آن‌ها به وضوح تمایز را نشان دهد و مستندسازی شود.
**Excerpt:**
```
در حالی که این شناسه احتمالاً برای شناسایی پروژه در ابزار بازرسی استفاده می‌شود، سخت‌کد کردن آن انعطاف‌ناپذیر است و می‌تواند با شناسه واقعی پروژه Firebase که توسط سایر قسمت‌های برنامه استفاده می‌شود (مثلاً `src/components/TelegramSettings.jsx` از `VITE_FIREBASE_PROJECT_ID` استفاده می‌کند) اشتباه گرفته شود.
```
