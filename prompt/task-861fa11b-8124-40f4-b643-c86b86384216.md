---
task_id: 861fa11b-8124-40f4-b643-c86b86384216
title: 'Anti-pattern: Stale assumption / Conditional inconsistency'
type: bug_fix
priority: high
...
created_at: '2026-06-12T19:16:39.203375+00:00'
updated_at: '2026-06-12T19:33:56.154361+00:00'
target_files:
- src/lib/aiProviders.js
---

# Anti-pattern: Stale assumption / Conditional inconsistency

## Raw Idea

The design goal explicitly states that 'the Settings UI fetches the real, current model list from each provider at validation time (see validateProviderKey, which returns live `models`)'. However, for providers like Anthropic (which typically lacks a public `/models` endpoint), `validateProviderKey` cannot actually fetch and return a live model list. The incomplete code for Anthropic implies it wo

📁 file: src/lib/aiProviders.js (line 83)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-06-12T19:16:39.847342+00:00]
The `validateProviderKey` function, despite its name, relies solely on the `http` helper's `res.ok` status for success. It lacks specific content or schema validation for the API response bodies (e.g., checking if the OpenAI `/models` endpoint actually returns a list of models with expected structur
