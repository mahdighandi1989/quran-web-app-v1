---
task_id: 0d2b9ba0-f410-409a-8896-fb9d7feeaf45
title: 'Anti-pattern: Security Vulnerability / Stale Assumption / Condit'
type: bug_fix
priority: high
...
created_at: '2026-06-12T19:16:38.236157+00:00'
updated_at: '2026-06-12T19:23:14.265969+00:00'
target_files:
- src/lib/aiClient.js
---

# Anti-pattern: Security Vulnerability / Stale Assumption / Condit

## Raw Idea

The Gemini API call places the user's API key directly in the URL query string (`?key=...`). This exposes the key in browser history, server logs, and potentially referrer headers, directly violating the stated key security model which aims to keep keys out of the browser/network tab and network requests for signed-in users (via proxy) and handle them transiently for guests. This is a critical sec

📁 file: src/lib/aiClient.js (line 109)

🎯 پیشنهاد: این الگو معمولاً منطق سیستم را در شرایط لبه می‌شکند.
---
[scan #2 at 2026-06-12T19:16:38.892373+00:00]
The `JSON.stringify` body for the Gemini provider is incomplete, ending abruptly with `contents, genera`. This is a syntax error that will cause the Gemini client to fail at runtime and prevents a full logical audit of its implementation and response parsing.

📁 file: src/lib/aiClient.js (line 112)

---
[scan #3 at 2026-06-12T19:16:39.203069+00:00]
The file description states the client is 'Unified AI chat client over the three provider shapes (openai-compatible / anthropic / gemini)'. However, the `chatComplete` function only provides explicit implementations for Anthropic and Gemini. The `openai-compatible` provider kind is missing, leading 
---
[scan #4 at 2026-06-12T19:23:14.265962+00:00]
The `http` function and provider-specific response parsing (e.g., Anthropic) primarily handle missing fields (`|| []`, `|| ''`) but lack strict validation of the response's type and structure. While robust for common cases, unexpected response formats from the AI provider (e.g., `data.content` being
