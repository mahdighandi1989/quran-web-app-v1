---
task_id: 6d5542d1-8a40-4602-bf43-e26c89cfd952
title: افزودن تست‌های واحد برای منطق اصلی بات تلگرام
type: bug
priority: critical
...
created_at: '2026-06-12T19:08:52.625982+00:00'
updated_at: '2026-06-12T19:27:33.463423+00:00'
target_files:
- server/telegram-bot.mjs
---

# افزودن تست‌های واحد برای منطق اصلی بات تلگرام

## Raw Idea

فایل `server/telegram-bot.mjs` که منطق اصلی بات تلگرام را پیاده‌سازی می‌کند، فاقد تست‌های واحد است. این بات مسئول دریافت وب‌هوک‌ها، پردازش دستورات، و تعامل با Firestore برای ذخیره و بازیابی تنظیمات کاربر است. عدم وجود تست‌ها ریسک بالایی برای رگرسیون‌های عملکردی، مشکلات امنیتی، و خطاهای پایداری در تعامل با تلگرام و پایگاه داده ایجاد می‌کند.
...
- `server/telegram-bot.mjs:1-end` — `telegram-bot.mjs` — فایل اصلی بات تلگرام که نیاز به تست دارد.
  ```
...
Node.js (ESM), Telegraf, Firebase Admin SDK, Vitest (برای تست‌های جدید).

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/lib/telegram.js` — کتابخانه سمت کلاینت برای تعامل با تلگرام که به عملکرد بات وابسته است.
- `src/components/TelegramSettings.jsx` — کامپوننت تنظیمات تلگرام که به عملکرد بات برای اعتبارسنجی و ارسال پیام‌های آزمایشی متکی است.
- `firestore.rules` — قوانین امنیتی Firestore که بات برای دسترسی به داده‌ها باید رعایت کند.
- `.env.example` — متغیرهای محیطی مورد نیاز برای پیکربندی بات تلگرام.
- `server/ai.mjs` — `telegram-bot.mjs` این فایل را import می‌کند
- `server/ai-proxy.mjs` — `telegram-bot.mjs` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
بات تلگرام یک جزء حیاتی در معماری پروژه است که به Firebase Admin SDK برای دسترسی به Firestore و به API تلگرام برای ارسال و دریافت پیام‌ها وابسته است. تغییرات در آن بر روی قابلیت‌های اعلان و تعامل کاربر با اپ از طریق تلگرام تأثیر مستقیم دارد.
...
- [ ] فایل `server/test/telegram-bot.test.mjs` ایجاد شود.
- [ ] تست‌هایی برای پردازش دستور `/start` و `/status` اضافه شود.
- [ ] تست‌هایی برای اعتبارسنجی توکن بات و تنظیم منوی بات اضافه شود.
- [ ] تست‌هایی برای اطمینان از تعامل صحیح با Firestore (خواندن/نوشتن) اضافه شود.
- [ ] تست‌هایی برای مدیریت خطا در فراخوانی‌های API تلگرام اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست واحد جدید (`server/test/telegram-bot.test.mjs`) برای پوشش دادن سناریوهای کلیدی بات تلگرام، شامل پردازش دستورات، تعامل با Firestore، و مدیریت خطاها.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `npm test -- server/test/telegram-bot.test.mjs`

## ⚠️ ریسک‌ها و موارد احتیاط
پیچیدگی Mock کردن Firebase Admin و Telegraf API؛ نیاز به درک عمیق از منطق بات.
...
1. فایل `server/test/telegram-bot.test.mjs` ایجاد شود. _(verify: static)_
2. تست‌هایی برای پردازش دستور `/start` و `/status` اضافه شود. _(verify: backend_test)_
3. تست‌هایی برای اعتبارسنجی توکن بات و تنظیم منوی بات اضافه شود. _(verify: backend_test)_
4. تست‌هایی برای اطمینان از تعامل صحیح با Firestore (خواندن/نوشتن) اضافه شود. _(verify: backend_test)_
5. تست‌هایی برای مدیریت خطا در فراخوانی‌های API تلگرام اضافه شود. _(verify: backend_test)_
