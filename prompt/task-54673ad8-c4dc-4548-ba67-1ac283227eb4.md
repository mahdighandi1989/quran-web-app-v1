...
updated_at: '2026-06-12T19:07:30.540496+00:00'
target_files:
...
1. فایل‌های `server/telegram-bot.mjs`، `server/ai.mjs` و `server/ai-proxy.mjs` به صراحت `process.env.FIREBASE_SERVICE_ACCOUNT` یا `process.env.GOOGLE_APPLICATION_CREDENTIALS` را می‌خوانند.
2. Firebase Admin SDK با اعتبارنامه‌های بازیابی شده مقداردهی اولیه می‌شود.
3. یک تست جدید (مثلاً در `tests/server/test_firebase_admin.py`) اتصال سرور به Firestore را با اعتبارنامه‌های mock شده تأیید می‌کند.
