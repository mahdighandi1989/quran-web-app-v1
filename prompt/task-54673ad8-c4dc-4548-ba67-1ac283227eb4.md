---
task_id: 54673ad8-c4dc-4548-ba67-1ac283227eb4
title: عدم استفاده از متغیرهای محیطی Firebase Admin SDK در سرور Node.js
type: bug
priority: critical
...
created_at: '2026-06-12T19:07:30.540491+00:00'
updated_at: '2026-06-12T19:26:30.415249+00:00'
target_files:
...
# عدم استفاده از متغیرهای محیطی Firebase Admin SDK در سرور Node.js

## Raw Idea

فایل `.env.example` به صراحت `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` را به عنوان متغیرهای مورد نیاز برای دسترسی Firebase Admin SDK به Firestore در سرور بات تلگرام (Node.js) مستند کرده است. با این حال، بررسی نشان می‌دهد که این متغیرهای محیطی در فایل‌های سرور Node.js (مانند `server/telegram-bot.mjs`, `server/ai.mjs`, `server/ai-proxy.mjs`) خوانده نمی‌شوند. این بدان معناست که سرور احتمالاً بدون احراز هویت صحیح Firebase Admin اجرا می‌شود که منجر به کاهش عملکرد دستورات داده/هوش مصنوعی یا آسیب‌پذیری‌های امنیتی می‌شود.
...
- `server/telegram-bot.mjs:نامشخص` — `Firebase Admin SDK initialization`
  ```
  import admin from 'firebase-admin';
  // ...
  // Add logic to read FIREBASE_SERVICE_ACCOUNT or GOOGLE_APPLICATION_CREDENTIALS
  // and initialize admin.initializeApp() with appropriate credentials.
  ```
- `server/ai.mjs:نامشخص` — `Firebase Admin SDK initialization`
  ```
  import admin from 'firebase-admin';
  // ...
  // Add logic to read FIREBASE_SERVICE_ACCOUNT or GOOGLE_APPLICATION_CREDENTIALS
  // and initialize admin.initializeApp() with appropriate credentials.
  ```
- `server/ai-proxy.mjs:نامشخص` — `Firebase Admin SDK initialization`
  ```
  import admin from 'firebase-admin';
  // ...
  // Add logic to read FIREBASE_SERVICE_ACCOUNT or GOOGLE_APPLICATION_CREDENTIALS
...
Node.js, Firebase Admin SDK, Firestore

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.env.example` — متغیرهای محیطی مورد نیاز را مستند می‌کند.
- `server/package.json` — وابستگی `firebase-admin` را لیست می‌کند.
- `docs/architecture/data-pipeline.md` — به خواندن Firestore توسط بات اشاره می‌کند.
- `server/ai.mjs` — `telegram-bot.mjs` این فایل را import می‌کند
- `server/ai-proxy.mjs` — `telegram-bot.mjs` این فایل را import می‌کند
- `server/telegram-bot.mjs` — این فایل `ai.mjs` را import می‌کند (caller)

## 🌐 نقشهٔ وابستگی‌ها
مقداردهی اولیه Firebase Admin SDK یک وابستگی اصلی برای توانایی سرور در تعامل با Firestore است. بدون آن، بات نمی‌تواند داده‌های کاربر (`appState`, `aiConfigs`) را بخواند یا بنویسد، که به شدت بر عملکرد آن تأثیر می‌گذارد.
...
- [ ] فایل‌های `server/telegram-bot.mjs`، `server/ai.mjs` و `server/ai-proxy.mjs` به صراحت `process.env.FIREBASE_SERVICE_ACCOUNT` یا `process.env.GOOGLE_APPLICATION_CREDENTIALS` را می‌خوانند.
- [ ] Firebase Admin SDK با اعتبارنامه‌های بازیابی شده مقداردهی اولیه می‌شود.
- [ ] یک تست جدید (مثلاً در `tests/server/test_firebase_admin.py`) اتصال سرور به Firestore را با اعتبارنامه‌های mock شده تأیید می‌کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. خواندن `FIREBASE_SERVICE_ACCOUNT` یا `GOOGLE_APPLICATION_CREDENTIALS` را در فایل‌های سرور Node.js (`server/telegram-bot.mjs`, `server/ai.mjs`, `server/ai-proxy.mjs`) پیاده‌سازی کنید تا Firebase Admin SDK به درستی مقداردهی اولیه شود. در صورت وجود، `FIREBASE_SERVICE_ACCOUNT` را اولویت دهید، در غیر این صورت `GOOGLE_APPLICATION_CREDENTIALS` را بررسی کنید.

## 💡 نمونه‌های قبل/بعد
**قبل: مقداردهی اولیه بدون اعتبارنامه صریح**

_قبل:_
```
import admin from 'firebase-admin';
admin.initializeApp(); // Relying on default application credentials or failing
```

**بعد: مقداردهی اولیه با اعتبارنامه از متغیر محیطی**
...
try {
  if (serviceAccountJson) {
    ...
    firebaseAdminApp = admin.initializeApp(); // SDK automatically finds GOOGLE_APPLICATION_CREDENTIALS
  } else {
    console.warn('Firebase Admin SDK: No service account or GOOGLE_APPLICATION_CREDENTIALS found. Bot functionality might be limited.');
    firebaseAdminApp = admin.initializeApp();
  }
} catch (e) {
...
## 🧪 دستورات اعتبارسنجی
- `grep -r "FIREBASE_SERVICE_ACCOUNT" server/`
- `grep -r "GOOGLE_APPLICATION_CREDENTIALS" server/`
- `pytest tests/server/test_firebase_admin.py`

## ⚠️ ریسک‌ها و موارد احتیاط
تجزیه نادرست اعتبارنامه یا مقداردهی اولیه می‌تواند از شروع سرور یا دسترسی به Firestore جلوگیری کند.
...
1. فایل‌های `server/telegram-bot.mjs`، `server/ai.mjs` و `server/ai-proxy.mjs` به صراحت `process.env.FIREBASE_SERVICE_ACCOUNT` یا `process.env.GOOGLE_APPLICATION_CREDENTIALS` را می‌خوانند. _(verify: static)_
2. Firebase Admin SDK با اعتبارنامه‌های بازیابی شده مقداردهی اولیه می‌شود. _(verify: static)_
3. یک تست جدید (مثلاً در `tests/server/test_firebase_admin.py`) اتصال سرور به Firestore را با اعتبارنامه‌های mock شده تأیید می‌کند. _(verify: backend_test)_

## Task Steps

### Step 1: بررسی مکانیسم فعلی بارگذاری متغیرهای محیطی در سرور Node.js
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی ساختار پروژه Node.js، فایل‌های اصلی سرور (مانند `server.mjs` یا `app.js`) و وابستگی‌های `package.json` برای شناسایی نحوه بارگذاری متغیرهای محیطی است. هدف این است که مشخص شود آیا کتابخانه‌ای مانند `dotenv` استفاده می‌شود و اگر بله، چگونه پیکربندی شده است. این مرحله صرفاً یک تحقیق و تشخیص است و شامل هیچ تغییر کدی نمی‌شود.
**Excerpt:**
```
فایل `.env.example` به صراحت `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` را به عنوان متغیرهای مورد نیاز برای دسترسی Firebase Admin SDK به Firestore در سرور بات تلگرام (Node.js) مستند کرده است. با این حال، بررسی نشان می‌دهد که این متغیرهای محیطی در فایل‌های سرور Node.js (مانند `server/telegram-bot.mjs`, `server/ai.mjs`, `server/ai-proxy.mjs`) خوانده نمی‌شوند.
```

### Step 2: پیکربندی صحیح `dotenv` برای بارگذاری متغیرهای محیطی
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اطمینان از اینکه متغیرهای محیطی از فایل‌های `.env` به درستی در دسترس برنامه Node.js قرار می‌گیرند، تمرکز دارد. اگر پکیج `dotenv` نصب نشده باشد، باید نصب شود. سپس، باید اطمینان حاصل شود که `dotenv` در ابتدایی‌ترین نقطه چرخه حیات برنامه (معمولاً در فایل ورودی اصلی سرور) فراخوانی می‌شود تا متغیرهایی مانند `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` در `process.env` در دسترس باشند. این مرحله شامل استفاده از این متغیرها برای مقداردهی اولیه Firebase نیست.
**Excerpt:**
```
فایل `.env.example` به صراحت `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` را به عنوان متغیرهای مورد نیاز برای دسترسی Firebase Admin SDK به Firestore در سرور بات تلگرام (Node.js) مستند کرده است. با این حال، بررسی نشان می‌دهد که این متغیرهای محیطی در فایل‌های سرور Node.js (مانند `server/telegram-bot.mjs`, `server/ai.mjs`, `server/ai-proxy.mjs`) خوانده نمی‌شوند.
```

### Step 3: به‌روزرسانی `server/telegram-bot.mjs` برای مقداردهی اولیه Firebase Admin SDK
**Status:** `pending` (0%)
**Scope:** این مرحله به طور خاص فایل `server/telegram-bot.mjs` را هدف قرار می‌دهد. شامل افزودن واردات لازم برای Firebase Admin SDK و مقداردهی اولیه آن با استفاده از اعتبارنامه‌های `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` است که از `process.env` بازیابی می‌شوند. هدف اطمینان از این است که عملیات Firestore مربوط به بات تلگرام به درستی احراز هویت می‌شوند. این تغییرات فقط در این فایل اعمال می‌شوند و بر سایر فایل‌های سرور تأثیری ندارند.
— [merged] این مرحله به طور خاص فایل `server/ai.mjs` را هدف قرار می‌دهد. شامل افزودن واردات لازم برای Firebase Admin SDK و مقداردهی اولیه آن با استفاده از اعتبارنامه‌های `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` است که از `process.env` بازیابی می‌شوند. این کار تضمین می‌کند که هر منطق مرتبط با هوش مصنوعی در این فایل که با Firestore تعامل دارد، به درستی احراز هویت می‌شود. این مرحله مستقل از تغییرات در `server/telegram-bot.mjs` یا `server/ai-proxy.mjs` است.
— [merged] این مرحله به طور خاص فایل `server/ai-proxy.mjs` را هدف قرار می‌دهد. شامل افزودن واردات لازم برای Firebase Admin SDK و مقداردهی اولیه آن با استفاده از اعتبارنامه‌های `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` است که از `process.env` بازیابی می‌شوند. این کار تضمین می‌کند که هر منطق پروکسی برای خدمات هوش مصنوعی که با Firestore تعامل دارد، به درستی احراز هویت می‌شود. این مرحله مستقل از تغییرات در `server/telegram-bot.mjs` یا `server/ai.mjs` است.
**Excerpt:**
```
فایل `.env.example` به صراحت `FIREBASE_SERVICE_ACCOUNT` و `GOOGLE_APPLICATION_CREDENTIALS` را به عنوان متغیرهای مورد نیاز برای دسترسی Firebase Admin SDK به Firestore در سرور بات تلگرام (Node.js) مستند کرده است. با این حال، بررسی نشان می‌دهد که این متغیرهای محیطی در فایل‌های سرور Node.js (مانند `server/telegram-bot.mjs`, `server/ai.mjs`, `server/ai-proxy.mjs`) خوانده نمی‌شوند.
```

### Step 4: پیاده‌سازی تأییدیه سرتاسری احراز هویت Firebase Admin SDK
**Status:** `pending` (0%)
**Scope:** این مرحله بر تأیید راه‌حل کلی تمرکز دارد. شامل ایجاد یا به‌روزرسانی تست‌های یکپارچه‌سازی موجود یا یک اسکریپت/اندپوینت تشخیصی اختصاصی برای انجام یک عملیات خواندن/نوشتن ساده و احراز هویت شده در Firestore با استفاده از Firebase Admin SDK از یکی از فایل‌های سرور متأثر است. این یک بررسی سرتاسری برای اطمینان از اینکه همه مراحل قبلی به طور جمعی مشکل را حل کرده‌اند، می‌باشد. این مرحله شامل اصلاح منطق اصلی بات یا ویژگی‌های هوش مصنوعی نیست، بلکه فقط تعامل آنها با Firebase را تأیید می‌کند.
**Excerpt:**
```
این بدان معناست که سرور احتمالاً بدون احراز هویت صحیح Firebase Admin اجرا می‌شود که منجر به کاهش عملکرد دستورات داده/هوش مصنوعی یا آسیب‌پذیری‌های امنیتی می‌شود.
```
