...
updated_at: '2026-06-12T19:08:16.348212+00:00'
target_files:
- src/lib/aiClient.js
...
1. کلیدهای API هوش مصنوعی هرگز مستقیماً از مرورگر به ارائه‌دهندگان خارجی ارسال نمی‌شوند.
2. اگر `VITE_AI_PROXY_URL` تنظیم شده باشد، تمام درخواست‌های AI از طریق آن مسیریابی می‌شوند.
3. اگر `VITE_AI_PROXY_URL` تنظیم نشده باشد، قابلیت‌های AI که نیاز به کلید دارند، غیرفعال یا با هشدار امنیتی نمایش داده می‌شوند.
4. هدر `anthropic-dangerous-direct-browser-access` فقط از سمت سرور (پراکسی) ارسال می‌شود.
5. تست‌های امنیتی موجود (مانند `src/test/aiClientSecurity.test.js` و `tests/test_ai_client_security.py`) این سناریو را پوشش می‌دهند.
