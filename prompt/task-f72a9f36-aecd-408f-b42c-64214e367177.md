---
task_id: f72a9f36-aecd-408f-b42c-64214e367177
title: کلیدهای API هوش مصنوعی در صورت عدم استفاده از پراکسی در سمت کلاینت افشا می‌شوند
type: security
priority: critical
...
created_at: '2026-06-12T19:08:16.348209+00:00'
updated_at: '2026-06-12T19:26:56.346401+00:00'
target_files:
- src/lib/aiClient.js
- src/components/AISettings.jsx
---

# کلیدهای API هوش مصنوعی در صورت عدم استفاده از پراکسی در سمت کلاینت افشا می‌شوند

## Raw Idea

پروژه دارای یک مکانیزم پراکسی بک‌اند (VITE_AI_PROXY_URL) برای مسیریابی درخواست‌های هوش مصنوعی و جلوگیری از افشای کلیدهای API در مرورگر است. با این حال، فایل `src/lib/aiClient.js` به صورت پیش‌فرض امکان فراخوانی مستقیم API ارائه‌دهندگان هوش مصنوعی را از مرورگر فراهم می‌کند، در صورتی که VITE_AI_PROXY_URL تنظیم نشده باشد. در این حالت، کلید API که در `src/components/AISettings.jsx` پیکربندی و در `ai.keys[active.id]` ذخیره می‌شود، مستقیماً در سمت کلاینت (مرورگر) در معرض دید قرار می‌گیرد. این یک آسیب‌پذیری امنیتی حیاتی است. وجود هدر `anthropic-dangerous-direct-browser-access: 'true'` در درخواست‌های Anthropic (همانطور که در `task-0b16076f` ذکر شده) نشان‌دهنده آگاهی از این خطر است، اما راه‌حل فعلی (fallback به دسترسی مستقیم) این خطر را برطرف نمی‌کند.
...
- `src/lib/aiClient.js:47` — `chatComplete` — تابع اصلی که درخواست‌های API هوش مصنوعی را انجام می‌دهد و باید از پراکسی استفاده کند.
  ```jsx
  ...
  ```
- `src/components/AISettings.jsx:68-75` — `validateKey / checkModel` — جایی که کلید API در سمت کلاینت برای اعتبارسنجی استفاده می‌شود.
  ```jsx
...
React, Vite, OpenAI/Anthropic API, Node.js backend proxy

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/components/AIExamGenerator.jsx` (سطر 4) — از aiClient.js استفاده می‌کند
- `src/components/AIWidgets.jsx` (سطر 3) — از aiClient.js استفاده می‌کند
- `server/ai-proxy.mjs` — پیاده‌سازی پراکسی بک‌اند
- `.env.example` (سطر 19) — تعریف VITE_AI_PROXY_URL
- `docs/architecture/data-pipeline.md` — مستندات مرز امنیتی کلیدهای AI
...
## 🌐 نقشهٔ وابستگی‌ها
این مشکل بر تمام قابلیت‌های هوش مصنوعی در فرانت‌اند تأثیر می‌گذارد و نیازمند هماهنگی بین منطق فراخوانی API در فرانت‌اند، پیکربندی متغیرهای محیطی و رفتار پراکسی بک‌اند است.
...
- [ ] کلیدهای API هوش مصنوعی هرگز مستقیماً از مرورگر به ارائه‌دهندگان خارجی ارسال نمی‌شوند.
- [ ] اگر `VITE_AI_PROXY_URL` تنظیم شده باشد، تمام درخواست‌های AI از طریق آن مسیریابی می‌شوند.
- [ ] اگر `VITE_AI_PROXY_URL` تنظیم نشده باشد، قابلیت‌های AI که نیاز به کلید دارند، غیرفعال یا با هشدار امنیتی نمایش داده می‌شوند.
- [ ] هدر `anthropic-dangerous-direct-browser-access` فقط از سمت سرور (پراکسی) ارسال می‌شود.
- [ ] تست‌های امنیتی موجود (مانند `src/test/aiClientSecurity.test.js` و `tests/test_ai_client_security.py`) این سناریو را پوشش می‌دهند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. برای جلوگیری از افشای کلیدهای API، تمام فراخوانی‌های API هوش مصنوعی باید از طریق پراکسی بک‌اند مسیریابی شوند. اگر `VITE_AI_PROXY_URL` تنظیم نشده باشد، قابلیت‌های هوش مصنوعی که نیاز به کلید API دارند باید غیرفعال شوند یا یک هشدار امنیتی برجسته به کاربر نمایش داده شود. هدر `anthropic-dangerous-direct-browser-access` فقط باید از سمت سرور (پراکسی) ارسال شود.

## 💡 نمونه‌های قبل/بعد
**استفاده از پراکسی برای فراخوانی API**

_قبل:_
```
const out = await chatComplete(active.provider, active.key, active.model, messages, { ... });
```

_بعد:_
```
const out = await chatComplete(active.provider, active.model, messages, { ... }); // key از طریق پراکسی مدیریت می‌شود
// یا: اگر پراکسی تنظیم نشده، قابلیت AI غیرفعال شود.
```
...
## 🧪 دستورات اعتبارسنجی
- `npm test src/test/aiClientSecurity.test.js`
- `pytest tests/test_ai_client_security.py`
- `Manual: `VITE_AI_PROXY_URL` را در `.env` حذف کنید، برنامه را اجرا کرده و درخواست‌های شبکه را برای افشای کلید API بررسی کنید.`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به تغییر در منطق `aiClient.js` و `AISettings.jsx`، احتمال شکستن قابلیت‌های AI در صورت عدم پیکربندی صحیح پراکسی.
...
1. کلیدهای API هوش مصنوعی هرگز مستقیماً از مرورگر به ارائه‌دهندگان خارجی ارسال نمی‌شوند. _(verify: ui_interaction)_
2. اگر `VITE_AI_PROXY_URL` تنظیم شده باشد، تمام درخواست‌های AI از طریق آن مسیریابی می‌شوند. _(verify: ui_interaction)_
3. اگر `VITE_AI_PROXY_URL` تنظیم نشده باشد، قابلیت‌های AI که نیاز به کلید دارند، غیرفعال یا با هشدار امنیتی نمایش داده می‌شوند. _(verify: ui_interaction)_
4. هدر `anthropic-dangerous-direct-browser-access` فقط از سمت سرور (پراکسی) ارسال می‌شود. _(verify: ui_interaction)_
5. تست‌های امنیتی موجود (مانند `src/test/aiClientSecurity.test.js` و `tests/test_ai_client_security.py`) این سناریو را پوشش می‌دهند. _(verify: backend_test)_

## Task Steps

### Step 1: تحلیل منطق موجود برای دسترسی مستقیم مرورگر در `src/lib/aiClient.js`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق کد موجود در `src/lib/aiClient.js` برای شناسایی منطق فعلی است که به صورت پیش‌فرض امکان فراخوانی مستقیم API ارائه‌دهندگان هوش مصنوعی را از مرورگر فراهم می‌کند. هدف درک چگونگی تشخیص تنظیم بودن `VITE_AI_PROXY_URL` و نحوه fallback به دسترسی مستقیم است. این مرحله شامل هیچ تغییری در کد نیست، بلکه صرفاً یک فاز تحلیل و درک است تا پایه‌ای برای مراحل بعدی فراهم شود.
**Excerpt:**
```
پروژه دارای یک مکانیزم پراکسی بک‌اند (VITE_AI_PROXY_URL) برای مسیریابی درخواست‌های هوش مصنوعی و جلوگیری از افشای کلیدهای API در مرورگر است. با این حال، فایل `src/lib/aiClient.js` به صورت پیش‌فرض امکان فراخوانی مستقیم API ارائه‌دهندگان هوش مصنوعی را از مرورگر فراهم می‌کند، در صورتی که VITE_AI_PROXY_URL تنظیم نشده باشد. در این حالت، کلید API که در `src/components/AISettings.jsx` پیکربندی و در `ai.keys[active.id]` ذخیره می‌شود، مستقیماً در سمت کلاینت (مرورگر) در معرض دید قرار می‌گیرد. این یک آسیب‌پذیری امنیتی حیاتی است.
```

### Step 2: حذف منطق fallback دسترسی مستقیم مرورگر در `src/lib/aiClient.js`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح `src/lib/aiClient.js` برای حذف کامل منطق fallback است که امکان فراخوانی مستقیم API ارائه‌دهندگان هوش مصنوعی را از مرورگر فراهم می‌کند. هدف این است که `aiClient.js` همیشه از `VITE_AI_PROXY_URL` برای مسیریابی درخواست‌ها استفاده کند و هرگز کلید API را مستقیماً در سمت کلاینت در معرض دید قرار ندهد. این مرحله تضمین می‌کند که حتی در صورت عدم تنظیم `VITE_AI_PROXY_URL`، فراخوانی مستقیم انجام نشود و به جای آن یک خطا رخ دهد.
**Excerpt:**
```
فایل `src/lib/aiClient.js` به صورت پیش‌فرض امکان فراخوانی مستقیم API ارائه‌دهندگان هوش مصنوعی را از مرورگر فراهم می‌کند، در صورتی که VITE_AI_PROXY_URL تنظیم نشده باشد. در این حالت، کلید API که در `src/components/AISettings.jsx` پیکربندی و در `ai.keys[active.id]` ذخیره می‌شود، مستقیماً در سمت کلاینت (مرورگر) در معرض دید قرار می‌گیرد. این یک آسیب‌پذیری امنیتی حیاتی است.
```

### Step 3: پیاده‌سازی مدیریت امن برای عدم تنظیم `VITE_AI_PROXY_URL`
**Status:** `pending` (0%)
**Scope:** این مرحله بر پیاده‌سازی یک مکانیزم مدیریت خطا یا غیرفعال‌سازی ویژگی‌های هوش مصنوعی در `src/lib/aiClient.js` تمرکز دارد، در صورتی که `VITE_AI_PROXY_URL` تنظیم نشده باشد. به جای fallback به دسترسی مستقیم ناامن، سیستم باید به طور واضح به کاربر اطلاع دهد که سرویس هوش مصنوعی در دسترس نیست یا ویژگی‌های مرتبط را غیرفعال کند. این شامل نمایش پیام خطا در UI یا غیرفعال کردن دکمه‌ها/ورودی‌های مرتبط با AI است تا تجربه کاربری بهتری ارائه شود.
**Excerpt:**
```
فایل `src/lib/aiClient.js` به صورت پیش‌فرض امکان فراخوانی مستقیم API ارائه‌دهندگان هوش مصنوعی را از مرورگر فراهم می‌کند، در صورتی که VITE_AI_PROXY_URL تنظیم نشده باشد.
```

### Step 4: بازبینی و امن‌سازی مدیریت کلید API در `src/components/AISettings.jsx`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق `src/components/AISettings.jsx` برای اطمینان از اینکه کلیدهای API پس از پیکربندی به صورت امن مدیریت می‌شوند و هرگز برای فراخوانی مستقیم API از سمت کلاینت استفاده نمی‌شوند. هدف این است که تأیید شود `ai.keys[active.id]` فقط برای ارسال به پراکسی بک‌اند (که مسئول استفاده از آن است) استفاده می‌شود و هیچ مسیر کد دیگری در این کامپوننت یا کامپوننت‌های مرتبط، کلید را برای دسترسی مستقیم مرورگر به API هوش مصنوعی به کار نمی‌برد. این بازبینی شامل اطمینان از عدم نمایش کلیدها در UI پس از ذخیره نیز می‌شود.
**Excerpt:**
```
کلید API که در `src/components/AISettings.jsx` پیکربندی و در `ai.keys[active.id]` ذخیره می‌شود، مستقیماً در سمت کلاینت (مرورگر) در معرض دید قرار می‌گیرد.
```

### Step 5: بازنگری و به‌روزرسانی منطق هدر `anthropic-dangerous-direct-browser-access`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی مجدد منطق مربوط به اضافه کردن هدر `anthropic-dangerous-direct-browser-access: 'true'` به درخواست‌های Anthropic است. با توجه به اینکه دسترسی مستقیم مرورگر به API هوش مصنوعی اکنون مسدود شده است، باید ارزیابی شود که آیا این هدر هنوز ضروری است یا خیر. اگر این هدر صرفاً یک پرچم هشدار برای دسترسی مستقیم بوده، ممکن است اکنون که دسترسی مستقیم حذف شده، بتوان آن را حذف یا منطق آن را به‌روزرسانی کرد تا فقط در شرایط خاص (مثلاً برای تست‌های امنیتی) استفاده شود. هدف، پاکسازی کد و جلوگیری از سردرگمی‌های آینده است.
**Excerpt:**
```
وجود هدر `anthropic-dangerous-direct-browser-access: 'true'` در درخواست‌های Anthropic (همانطور که در `task-0b16076f` ذکر شده) نشان‌دهنده آگاهی از این خطر است، اما راه‌حل فعلی (fallback به دسترسی مستقیم) این خطر را برطرف نمی‌کند.
```

### Step 6: افزودن تست‌های واحد و یکپارچه‌سازی برای اعمال پراکسی و مدیریت امن کلید
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد (Unit Tests) و تست‌های یکپارچه‌سازی (Integration Tests) برای تأیید اینکه `src/lib/aiClient.js` به درستی پراکسی را اعمال می‌کند و هرگز به دسترسی مستقیم مرورگر fallback نمی‌کند. همچنین، تست‌هایی برای اطمینان از اینکه کلیدهای API از `src/components/AISettings.jsx` به صورت امن مدیریت می‌شوند و در معرض دید قرار نمی‌گیرند، اضافه خواهد شد. این تست‌ها باید سناریوهایی را پوشش دهند که `VITE_AI_PROXY_URL` تنظیم شده و تنظیم نشده است تا پوشش کاملی از جنبه‌های امنیتی فراهم شود.
**Excerpt:**
```
پروژه دارای یک مکانیزم پراکسی بک‌اند (VITE_AI_PROXY_URL) برای مسیریابی درخواست‌های هوش مصنوعی و جلوگیری از افشای کلیدهای API در مرورگر است. ... این یک آسیب‌پذیری امنیتی حیاتی است.
```
