...
updated_at: '2026-06-12T19:21:24.577858+00:00'
target_files:
...
1. توابع `toFa` و `uid` از کامپوننت‌های مربوطه حذف شده‌اند.
2. یک فایل جدید `src/lib/utils.js` حاوی این توابع ایجاد شده است.
3. تمام کامپوننت‌هایی که قبلاً از این توابع استفاده می‌کردند، اکنون آن‌ها را از `src/lib/utils.js` وارد می‌کنند.
4. عملکرد برنامه بدون تغییر باقی می‌ماند.
