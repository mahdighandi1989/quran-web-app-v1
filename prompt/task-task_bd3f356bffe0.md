---
task_id: task_bd3f356bffe0
title: بهینه‌سازی هسته و رابط کاربری سیستم هوش مصنوعی
type: other
priority: high
...
updated_at: '2026-06-06T05:25:41.419777+00:00'
tags:
- consolidated
- post_verify_merge
---

# بهینه‌سازی هسته و رابط کاربری سیستم هوش مصنوعی

## Raw Idea

🧬 این یک تسک تلفیقی است — از 10 تسک منفرد ساخته شده.
📌 دلیل تلفیق (rationale توسط AI): این خوشه شامل تسک‌هایی است که به اعتبارسنجی خروجی AI، مدیریت state در AISettings، رفع anti-patternهای مرتبط با AI، تعریف موارد استفاده چت AI، بهبود دسترسی و کشف‌پذیری سیستم AI، رفع باگ‌های UI در AIExamGenerator، به‌روزرسانی متن‌های placeholder، و روشن‌سازی هدف و مستندسازی کامپوننت‌های AI می‌پردازد. این تسک‌ها به طور کلی کیفیت و قابلیت استفاده از سیستم AI را ارتقا می‌دهند.
🎯 theme: بهبود عملکرد، تجربه کاربری و کیفیت کد سیستم هوش مصنوعی
💎 estimated_difficulty: large

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 1 از 10
  id: cef27988-8ab8-4b58-9fad-f161e2b76d88
  عنوان اصلی: اعتبارسنجی خروجی هوش مصنوعی در کامپوننت AIExamGenerator
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/components/AIExamGenerator.jsx

📋 acceptance_criteria کامل:
  - تابع validateExamQuestions در aiTasks.js وجود دارد [verify_method=static] [verify_plan={"grep_patterns": ["function validateExamQuestions", "const validateExamQuestions ="], "files_hint": ["src/utils/aiTasks.js"]}]
  - اگر سوالی فاقد q یا choices باشد، خطای مشخص برمی‌گرداند [verify_method=backend_test] [verify_plan={"test_node": "tests/unit/test_ai_tasks.py::test_validate_exam_questions_invalid_structure", "timeout_seconds": 30}]
  - تست واحد برای validateExamQuestions اضافه شده [verify_method=backend_test] [verify_plan={"test_node": "tests/unit/test_ai_tasks.py::test_validate_exam_questions_all_cases", "timeout_seconds": 30}]
  - AIExamGenerator بعد از validation نامعتبر، خطا را به کاربر نشان می‌دهد نه crash [verify_method=ui_interaction] [verify_plan={"base": "frontend", "ui_steps": [{"action": "navigate", "url": "/exam-generator"}, {"action": "wait_for_load", "state": "networkidle"}, {"action": "fill", "selector": "[data-testid='topic-input']", "]
...
کامپوننت AIExamGenerator از prop 'getConfig' استفاده می‌کند اما هیچ validation روی ساختار خروجی AI ندارد

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/components/AIExamGenerator.jsx:43-44` — `generate function` — بعد از parseJsonArray، validation ساختاری انجام نمی‌شود
  ```jsx
  ...
  ```
- `src/components/AIExamGenerator.jsx:82-103` — `render questions` — اگر q.q undefined باشد یا q.choices آرایه نباشد، خطای رندر می‌دهد
  ```jsx
...
- `src/lib/aiTasks.js` (سطر 1) — تابع parseJsonArray در اینجاست؛ validation جدید هم اینجا اضافه شود
- `src/test/aiTasks.test.js` (سطر 1) — تست‌های موجود برای aiTasks باید به‌روز شوند
...
## 🌐 نقشهٔ وابستگی‌ها
AIExamGenerator توسط App.jsx استفاده می‌شود؛ تغییر در aiTasks.js روی telegramCommands.js هم اثر می‌گذارد.

## 🔍 Context و وضعیت فعلی
در src/components/AIExamGenerator.jsx خط 43، خروجی مدل (parseJsonArray(out)) بدون validation کامل به setQuestions می‌رود. اگر مدل JSON نامعتبر یا ساختار متفاوت برگرداند (مثلاً فیلد q به جای question، یا choices به جای گزینه‌ها)، کامپوننت در رندر خطوط 82-103 با undefined crash می‌کند. همچنین prop 'getConfig' از والد می‌آید اما هیچ type checking یا fallback ندارد.

## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
verify می‌تواند پیاده‌سازی متفاوت ولی هم‌ارز را قبول کند.

- [ ] تابع validateExamQuestions در aiTasks.js وجود دارد
- [ ] اگر سوالی فاقد q یا choices باشد، خطای مشخص برمی‌گرداند
- [ ] تست واحد برای validateExamQuestions اضافه شده
- [ ] AIExamGenerator بعد از validation نامعتبر، خطا را به کاربر نشان می‌دهد نه crash
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک تابع validateExamQuestions(data) در src/lib/aiTasks.js اضافه کن که هر سوال راستی‌آزمایی کند: وجود فیلد q (string)، choices (array با حداقل 2 آیتم)، answer (عدد بین 0 تا length-1). در صورت نامعتبر بودن، خطای مشخص برگردان. همچنین propTypes یا defaultProps برای getConfig اضافه کن.

## 💡 نمونه‌های قبل/بعد
**validation بعد از parseJsonArray**
...
```
const arr = parseJsonArray(out).slice(0, count);
const validated = validateExamQuestions(arr);
...
## 🧪 دستورات اعتبارسنجی
- `npm test -- src/test/aiTasks.test.js`
- `npm test -- src/test/AIExamGenerator.test.jsx`

## ⚠️ ریسک‌ها و موارد احتیاط
ندارد — pure function validation
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 10
  id: c99a184d-3967-4f2e-91dc-7e69849254bb
  عنوان اصلی: جلوگیری از تغییر مستقیم state در AISettings
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "tests/", "timeout_seconds": 300}]
  - linter بدون warning عبور می‌کند [verify_method=static] [verify_plan={"grep_patterns": ["state\\.[a-zA-Z_]+\\s*="], "files_hint": ["src/components/AISettings.tsx"]}]
  - type-check موفق است [verify_method=static] [verify_plan={"grep_patterns": ["@ts-ignore", "@ts-expect-error"], "files_hint": ["src/components/AISettings.tsx"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
کامپوننت AISettings از setConfig به صورت ناامن استفاده می‌کند (mutate مستقیم state)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 10
  id: 5ae9a961-0658-4e8f-b429-b9ef00728cef
  عنوان اصلی: Fix stale assumption anti-pattern
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/lib/aiProviders.js
...
📋 acceptance_criteria کامل:
  ...
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["// TODO: Review/update BUILTIN_PROVIDERS model names", "// NOTE: Model names in BUILTIN_PROVIDERS are placeholders/examples"], "files_hint": ["src/lib/aiProviders.js"]}]
  - تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_aiProviders.py::test_validateProviderKey_edge_cases", "timeout_seconds": 60}]
...
- `src/App.jsx` — این فایل `aiProviders.js` را import می‌کند (caller)
- `src/lib/aiClient.js` — این فایل `aiProviders.js` را import می‌کند (caller)
- `src/lib/aiStore.js` — این فایل `aiProviders.js` را import می‌کند (caller)
- `src/test/AISettings.test.jsx` — این فایل `aiProviders.js` را import می‌کند (caller)

## 🔍 Context و وضعیت فعلی
لیست مدل‌های پیش‌فرض در `BUILTIN_PROVIDERS` برای OpenAI، Anthropic و Google به نظر می‌رسد منسوخ شده یا حاوی نام‌های غیر استاندارد/فرضی است (مانند `gpt-4.1`, `claude-opus-4-1`, `gemini-2.5-pro`). این امر می‌تواند منجر به تلاش کاربران برای استفاده از مدل‌های ناموجود یا عدم آگاهی از جدیدترین مدل‌های واقعی شود.

📁 file: src/lib/aiProviders.js (line 19)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 4 از 10
  id: af434411-2560-4e77-85f3-2d7997604173
  عنوان اصلی: Define Core AI Chat Use Cases
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -
...
📋 acceptance_criteria کامل:
  ...
  - کد تغییر کرد تا outcome target محقق شود [verify_method=static] [verify_plan={"grep_patterns": ["def new_feature_logic", "class ChatAIInteractionService", "router.post(\"/chat/interact\")"], "files_hint": ["backend/app/services/chat_ai.py", "backend/app/api/endpoints/chat.py",]
  - test E2E که outcome را اندازه می‌گیرد عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/e2e/test_chat_ai_outcome_measurement.py::test_interaction_increase", "timeout_seconds": 120}]
  - metric/log اضافه شد تا در production outcome rate قابل تشخیص باشد [verify_method=static] [verify_plan={"grep_patterns": ["logger.info(\"chat_ai_interaction_event\",", "metrics.increment(\"chat_ai.interaction_count\")", "metrics.gauge(\"chat_ai.outcome_rate\")"], "files_hint": ["backend/app/services/ch]
...
## 🎯 هدف (خلاصه ساختاریافته)
[Effectiveness] Undefined or Unclear Core Use Cases
...
Identify and validate at least 3 primary user scenarios where the chat AI provides significant value, leading to a 20% increase in interactions related to these specific scenarios within the next 60 days.

## 📊 وضعیت فعلی
The low interaction volume suggests users may not understand the specific problems the chat AI is designed to solve for them, or its value proposition is not compelling enough to drive consistent usage.

## 🛠 اقدام پیشنهادی
Conduct a workshop with stakeholders and potential users to define clear use cases and user stories. Develop targeted features or content that highlight these specific applications and integrate user feedback into the product roadmap.
...
🔧 مراحل remaining که در super-task باید انجام شوند:
  - شناسایی و اعتبارسنجی حداقل ۳ سناریوی کاربری اصلی برای چت AI — تعریف و اعتبارسنجی کامل حداقل ۳ سناریوی کاربری اصلی برای چت AI و تعیین اهداف ۲۰ درصدی افزایش تعامل.
  - برگزاری کارگاه با ذی‌نفعان و کاربران بالقوه برای تعریف موارد استفاده و داستان‌های کاربری — برنامه‌ریزی و اجرای کارگاه با ذی‌نفعان برای تعریف موارد استفاده و داستان‌های کاربری چت AI.
  - اندازه‌گیری outcome مطلوب برای effectiveness issue چت AI — پیاده‌سازی کامل مکانیزم اندازه‌گیری outcome مطلوب (مانند ارسال ایمیل توسط فرم) و اطمینان از عملکرد صحیح آن.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 5 از 10
  id: 60d41ab7-301f-44c6-b734-fc5ff0247218
  عنوان اصلی: بهبود دسترسی و کشف‌پذیری سیستم AI
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -
...
📋 acceptance_criteria کامل:
  - outcome target به‌صورت measurable بازنویسی شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - کد تغییر کرد تا outcome target محقق شود [verify_method=ui_interaction] [verify_plan={"base": "frontend", "ui_steps": [{"action": "navigate", "url": "/"}, {"action": "wait_for_load", "state": "networkidle"}, {"action": "screenshot", "label": "homepage_loaded"}, {"action": "assert_visi]
  - test E2E که outcome را اندازه می‌گیرد عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/e2e/test_ai_conversation_initiation.py::test_user_can_initiate_ai_conversation", "timeout_seconds": 120}]
  - metric/log اضافه شد تا در production outcome rate قابل تشخیص باشد [verify_method=static] [verify_plan={"grep_patterns": ["logger.info.*ai_conversation_initiated", "metrics.increment.*ai_conversation_initiated"], "files_hint": ["backend/app/routes/ai_chat.py", "frontend/src/components/AIChat.jsx"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
[Effectiveness] Limited System Accessibility or Discoverability
...
## 🔍 Context و وضعیت فعلی
## 🎯 هدف مطلوب (outcome target)
Increase the number of unique users who successfully initiate a conversation with the AI by 100% within the next 30 days.

## 📊 وضعیت فعلی
With only 100 interactions over a month, it's possible that potential users are unaware of the system's existence, find it difficult to access, or the entry points are not prominent enough.

## 🛠 اقدام پیشنهادی
Review current deployment and access mechanisms. Implement clear communication channels (e.g., internal announcements, public-facing documentation, direct links) to ensure target users can easily find and start using the chat AI. Consider integrating the AI into existing workflows or platforms where users already operate.
...
🔧 مراحل remaining که در super-task باید انجام شوند:
  - افزایش تعداد کاربران منحصربه‌فردی که مکالمه با AI را آغاز می‌کنند به میزان 100% در 30 روز آینده — پیاده‌سازی کانال‌های ارتباطی واضح و یکپارچه‌سازی AI در workflows برای افزایش تعامل کاربران.
  - تشخیص و رفع effectiveness issue: اطمینان از اینکه کد outcome مطلوب را تولید می‌کند — شناسایی و اصلاح gaps در current implementation برای دستیابی به outcome مطلوب.
  - بررسی کامل و اجرای مو-به-موی پرامپت با شناسایی وابستگی‌ها و جلوگیری از خطاهای رایج — رعایت دقیق الزامات پرامپت، جلوگیری از خلاصه‌سازی یا تغییر نام‌ها، و شناسایی کامل وابستگی‌ها.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 6 از 10
  id: d2846fc0-c8ae-4893-9be7-2d0c0559e6fa
  عنوان اصلی: دکمه‌ی UI بدون handler: setAnswers((a) => ({ ...a, [i]: ci }))}>
                        {c}{correct ? '
  اولویت اصلی: medium
  وضعیت verify قبلی: error
  فایل‌های دخیل: src/components/AIExamGenerator.jsx

📋 acceptance_criteria کامل:
  - git blame مشخص می‌کند چرا این دکمه `setAnswers((a) => ({ ...a, [i]: ci }))}>
                        {c}{correct ? '` فاقد handler است [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  ...
  - اگر دکمه باقی ماند، تست end-to-end (Playwright یا cypress) برای کلیک و تأیید رفتار اضافه شده [verify_method=ui_interaction] [verify_plan={"base": "frontend", "ui_steps": [{"action": "navigate", "url": "/ai-exam-generator"}, {"action": "wait_for_load", "state": "networkidle"}, {"action": "screenshot", "label": "before_answer_selection"}]
...
## 🎯 هدف (خلاصه ساختاریافته)
دکمه‌ی UI بدون handler: setAnswers((a) => ({ ...a, [i]: ci }))}>
                        {c}{correct ? '
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/AIExamGenerator.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setAnswers((a) => ({ ...a, [i]: ci }))}>
                        {c}{correct ? '`
- فایل: `src/components/AIExamGenerator.jsx`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 7 از 10
  id: 3a14829d-8738-4163-8330-b381a325021c
  عنوان اصلی: به‌روزرسانی متن‌های placeholder در کامپوننت‌های AI و Telegram
  اولویت اصلی: medium
  وضعیت verify قبلی: pending
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - اعمال تغییر بدون شکستن تست‌های موجود [verify_method=backend_test] [verify_plan={"test_node": "", "timeout_seconds": 180}]
  - linter بدون warning عبور می‌کند [verify_method=static] [verify_plan={"grep_patterns": ["Linter found 0 warnings", "Linter found 0 errors"], "files_hint": ["frontend/", "backend/"]}]
  - type-check موفق است [verify_method=static] [verify_plan={"grep_patterns": ["No issues found.", "Type checking passed"], "files_hint": ["frontend/", "backend/"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
متن‌های placeholder در کامپوننت‌های AI و Telegram به‌روز نیستند
...
## 🔍 Context و وضعیت فعلی
کامپوننت‌های AISettings.jsx و TelegramSettings.jsx حاوی متن‌های راهنما و placeholder هستند که برخی از آن‌ها به قابلیت‌های جدید (مثلاً پروایدرهای سفارشی، تشخیص Chat ID) اشاره نمی‌کنند. همچنین در AIExamGenerator.jsx، متن خطا برای 'دیتاست بارگذاری نشده' مبهم است و کاربر را به اقدام مشخصی راهنمایی نمی‌کند. این باعث سردرگمی کاربر و افزایش درخواست‌های
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 8 از 10
  id: a51feeae-4be2-4359-a430-60698cfad380
  عنوان اصلی: روشن‌سازی هدف فایل AISettings.jsx
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/components/AISettings.jsx

📋 acceptance_criteria کامل:
  - فایل docstring/header comment ۳-۵ خطی دارد که purpose را روشن می‌کند [verify_method=static] [verify_plan={"grep_patterns": ["/**", "* @file", "// Purpose:", "// Description:"], "files_hint": ["src/components/AISettings.jsx"]}]
  - اگر فایل بخشی از pipeline است، در docstring به upstream/downstream اشاره شده [verify_method=static] [verify_plan={"grep_patterns": ["pipeline", "upstream", "downstream"], "files_hint": ["src/components/AISettings.jsx"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
فایل با هدف مبهم: AISettings.jsx
...
- `src/App.jsx` — این فایل `AISettings.jsx` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
فایل `src/components/AISettings.jsx` هیچ docstring/comment توضیحی در بالای فایل ندارد و نام آن purpose را روشن نمی‌کند.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 9 از 10
  id: 5aa1c806-e554-4a30-953a-04756d271c36
  عنوان اصلی: Fix Threshold-Outcome / Stale assumption anti-pattern
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/components/AISettings.jsx
...
📋 acceptance_criteria کامل:
  - ریشه anti-pattern تشخیص داده شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["uuidv4", "crypto.randomUUID", "// Justification for uid() usage:", "/* Justification for uid() usage:"], "files_hint": ["src/components/AISettings.jsx"]}]
  - تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/unit/test_id_generation.py::test_uid_edge_cases", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
Anti-pattern: Threshold-Outcome mismatch / Stale assumption

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/components/AISettings.jsx:10`
...
## 🔍 Context و وضعیت فعلی
The `uid()` function generates a short, random ID (6 alphanumeric characters) which is used for `customProviders`. Given that `customProviders` are updated via `setConfig` and the component's top-level comment indicates `config` (and thus its contents like `customProviders`) can be persisted per-user in Firestore, this ID generation method is not robust enough for long-term uniqueness. The probabi

📁 file: src/components/AISettings.jsx (line 10)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 10 از 10
  id: dfe398cf-e7d8-47b9-907d-56b71e1c2caa
  عنوان اصلی: مستندسازی و بازبینی نام aiClient.js
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/lib/aiClient.js

📋 acceptance_criteria کامل:
  - فایل docstring/header comment ۳-۵ خطی دارد که purpose را روشن می‌کند [verify_method=static] [verify_plan={"grep_patterns": ["^\\/\\*\\*.*", "^\\/\\/.*"], "files_hint": ["src/lib/aiClient.js"]}]
  ...
  - نام فایل (اگر مبهم) به نام معنادار rename شده [verify_method=static] [verify_plan={"grep_patterns": [], "files_hint": ["src/lib/aiClient.js"]}]
...
- `src/components/AIExamGenerator.jsx` — این فایل `aiClient.js` را import می‌کند (caller)
- `src/components/AIWidgets.jsx` — این فایل `aiClient.js` را import می‌کند (caller)
...
## 🔍 Context و وضعیت فعلی
## 📋 شرح
فایل `src/lib/aiClient.js` هیچ docstring/comment توضیحی در بالای فایل ندارد و نام آن purpose را روشن نمی‌کند.
...
- در commit message: `merged-from: cef27988-8ab8-4b58-9fad-f161e2b76d88, c99a184d-3967-4f2e-91dc-7e69849254bb, 5ae9a961-0658-4e8f-b429-b9ef00728cef, af434411-2560-4e77-85f3-2d7997604173, 60d41ab7-301f-44c6-b734-fc5ff0247218, d2846fc0-c8ae-4893-9be7-2d0c0559e6fa, 3a14829d-8738-4163-8330-b381a325021c, a51feeae-4be2-4359-a430-60698cfad380, 5aa1c806-e554-4a30-953a-04756d271c36, dfe398cf-e7d8-47b9-907d-56b71e1c2caa`
...
1. تابع validateExamQuestions در aiTasks.js وجود دارد _(verify: static)_
2. اگر سوالی فاقد q یا choices باشد، خطای مشخص برمی‌گرداند _(verify: backend_test)_
3. تست واحد برای validateExamQuestions اضافه شده _(verify: backend_test)_
4. AIExamGenerator بعد از validation نامعتبر، خطا را به کاربر نشان می‌دهد نه crash _(verify: ui_interaction)_
...
8. ریشه anti-pattern تشخیص داده شد _(verify: manual_only)_
9. یا کد اصلاح شد، یا کامنت توجیهی اضافه شد _(verify: static)_
10. تست edge case نوشته شد _(verify: backend_test)_
11. outcome target به‌صورت measurable بازنویسی شد _(verify: manual_only)_
12. کد تغییر کرد تا outcome target محقق شود _(verify: static)_
13. test E2E که outcome را اندازه می‌گیرد عبور می‌کند _(verify: backend_test)_
14. metric/log اضافه شد تا در production outcome rate قابل تشخیص باشد _(verify: static)_
15. git blame مشخص می‌کند چرا این دکمه `setAnswers((a) => ({ ...a, [i]: ci }))}>
                        {c}{correct ? '` فاقد handler است _(verify: manual_only)_
16. یکی از این سه حالت تعیین شده: (a) handler restore شده + کار می‌کند، (b) دکمه حذف شده، (c) به‌صورت decorative علامت‌گذاری شده _(verify: manual_only)_
17. اگر دکمه باقی ماند، تست end-to-end (Playwright یا cypress) برای کلیک و تأیید رفتار اضافه شده _(verify: ui_interaction)_
18. فایل docstring/header comment ۳-۵ خطی دارد که purpose را روشن می‌کند _(verify: static)_
19. اگر فایل بخشی از pipeline است، در docstring به upstream/downstream اشاره شده _(verify: static)_
20. نام فایل (اگر مبهم) به نام معنادار rename شده _(verify: manual_only)_
