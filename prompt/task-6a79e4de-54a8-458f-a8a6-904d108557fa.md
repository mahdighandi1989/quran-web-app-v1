---
task_id: 6a79e4de-54a8-458f-a8a6-904d108557fa
title: پیاده‌سازی لود داینامیک XLSX و Web Speech API
...
created_at: '2026-06-12T19:12:15.314910+00:00'
updated_at: '2026-06-12T19:29:32.395842+00:00'
target_files:
- src/lib/excel.js
- src/lib/speech.js
---

# پیاده‌سازی لود داینامیک XLSX و Web Speech API

## Raw Idea

`README.md` پروژه به دو قابلیت کلیدی اشاره می‌کند: ۱) کتابخانه XLSX به صورت داینامیک از CDN لود می‌شود. ۲) برای تشخیص گفتار (🎙️) مرورگر باید Web Speech API را پشتیبانی کند. با بررسی کد، مشخص شد که هیچ مکانیزمی برای لود داینامیک XLSX از CDN در فایل‌های ارائه شده وجود ندارد و کد `src/lib/excel.js` مستقیماً از `window.XLSX` استفاده می‌کند و در صورت عدم وجود آن هشدار می‌دهد. همچنین، هیچ کدی برای راه‌اندازی یا استفاده از Web Speech API برای تشخیص گفتار یافت نشد. این دو فیچر در `README.md` تبلیغ شده‌اند اما پیاده‌سازی نشده‌اند که باعث سردرگمی کاربر و شکست در قابلیت‌های وعده داده شده می‌شود.
---
[scan #2 at 2026-06-12T19:14:08.612484+00:00]
فایل `README.md` پروژه (خطوط 25-26) به دو قابلیت کلیدی اشاره می‌کند: 1) کتابخانه XLSX به صورت دینامیک از CDN بارگذاری می‌شود و 2) برای تشخیص گفتار (🎙️) مرورگر باید Web Speech API را پشتیبانی کند. با این حال، `task-38df6aee` نشان می‌دهد که هیچ مکانیزمی برای بارگذاری دینامیک XLSX از CDN وجود ندارد و ک
...
- `src/lib/excel.js:all` — `parseExcelFile`
  ```jsx
  // In src/lib/excel.js or a new utility
  function loadXLSXScript() {
    if (window.XLSX) return Promise.resolve();
    return new Promise((resolve, reject) => {
      const script = document.createElement('script');
      script.src = 'https://cdn.sheetjs.com/xlsx-0.20.2/xlsx.full.min.js';
  ...
  ```
- `src/lib/speech.js:new file` — `(new functions/hooks)`
  ```jsx
  // src/lib/speech.js
...
Frontend (React/Vite), Web APIs (Web Speech API), Dynamic Script Loading

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `README.md` — قابلیت‌ها را مستند می‌کند.
- `src/App.test.jsx` — تست‌های `parseExcelFile` و بررسی `window.XLSX`.
- `src/App.jsx` — منطق اصلی برنامه، که این قابلیت‌ها را ادغام خواهد کرد.

## 🌐 نقشهٔ وابستگی‌ها
این تغییرات قابلیت‌های اصلی وعده داده شده در README را فعال می‌کنند و تجربه کاربری را بهبود می‌بخشند.
...
- [ ] کتابخانه XLSX به صورت داینامیک از CDN در صورت نیاز بارگذاری می‌شود و `parseExcelFile` به درستی کار می‌کند.
- [ ] یک ابزار جدید (`src/lib/speech.js`) برای کپسوله‌سازی منطق Web Speech API ایجاد شده است.
- [ ] یک عنصر UI (مثلاً دکمه میکروفون) برای فعال کردن تشخیص گفتار در بخش مربوطه از برنامه (مثلاً جلسه تمرین) اضافه شده است.
- [ ] `README.md` برای منعکس کردن وضعیت پیاده‌سازی به‌روزرسانی شده است.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ۱. **برای XLSX**: اضافه کردن منطق برای بارگذاری اسکریپت `xlsx.full.min.js` از یک CDN (مثلاً `https://cdn.sheetjs.com/xlsx-0.20.2/xlsx.full.min.js`) به صورت داینامیک (مثلاً با ایجاد یک تگ `<script>`) در `src/lib/excel.js` یا `src/App.jsx`، به گونه‌ای که قبل از استفاده از `window.XLSX`، اطمینان حاصل شود که کتابخانه بارگذاری شده است. ۲. **برای Web Speech API**: پیاده‌سازی یک wrapper یا hook در `src/lib/speech.js` (جدید) که از `window.SpeechRecognition` یا `window.webkitSpeechRecognition` استفاده کند و قابلیت تشخیص گفتار را برای ورودی‌های کاربر فراهم آورد. سپس این قابلیت را در UI (مثلاً در یک کامپوننت تمرین) ادغام کنید.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## ⚠️ ریسک‌ها و موارد احتیاط
سازگاری مرورگر برای Web Speech API؛ تأثیر احتمالی بر عملکرد بارگذاری اسکریپت داینامیک در صورت عدم مدیریت صحیح.
...
1. کتابخانه XLSX به صورت داینامیک از CDN در صورت نیاز بارگذاری می‌شود و `parseExcelFile` به درستی کار می‌کند. _(verify: ui_interaction)_
2. یک ابزار جدید (`src/lib/speech.js`) برای کپسوله‌سازی منطق Web Speech API ایجاد شده است. _(verify: static)_
3. یک عنصر UI (مثلاً دکمه میکروفون) برای فعال کردن تشخیص گفتار در بخش مربوطه از برنامه (مثلاً جلسه تمرین) اضافه شده است. _(verify: ui_interaction)_
4. `README.md` برای منعکس کردن وضعیت پیاده‌سازی به‌روزرسانی شده است. _(verify: static)_

## Task Steps

### Step 1: پیاده‌سازی مکانیزم لود داینامیک کتابخانه XLSX از CDN
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ایجاد یک تابع یا ماژول جدید است که مسئولیت بارگذاری اسکریپت کتابخانه XLSX را از یک CDN مشخص به صورت پویا بر عهده دارد. هدف این است که اسکریپت به DOM اضافه شود و پس از بارگذاری کامل، یک callback یا Promise را resolve کند. این مرحله شامل تغییر فایل `src/lib/excel.js` یا استفاده از `window.XLSX` نیست، بلکه فقط مکانیزم بارگذاری را فراهم می‌کند.
**Excerpt:**
```
`README.md` پروژه به دو قابلیت کلیدی اشاره می‌کند: ۱) کتابخانه XLSX به صورت داینامیک از CDN لود می‌شود. ... با بررسی کد، مشخص شد که هیچ مکانیزمی برای لود داینامیک XLSX از CDN در فایل‌های ارائه شده وجود ندارد...
```

### Step 2: یکپارچه‌سازی لودر داینامیک XLSX در `src/lib/excel.js`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل اصلاح فایل `src/lib/excel.js` است تا به جای استفاده مستقیم از `window.XLSX`، از مکانیزم بارگذاری داینامیک پیاده‌سازی شده در مرحله قبل استفاده کند. کد باید منتظر بماند تا کتابخانه XLSX بارگذاری شود و سپس از آن استفاده کند. همچنین، باید مدیریت خطای مناسب برای سناریوهایی که بارگذاری CDN با شکست مواجه می‌شود، اضافه شود تا هشدارها به درستی مدیریت شوند.
**Excerpt:**
```
...و کد `src/lib/excel.js` مستقیماً از `window.XLSX` استفاده می‌کند و در صورت عدم وجود آن هشدار می‌دهد. ... این دو فیچر در `README.md` تبلیغ شده‌اند اما پیاده‌سازی نشده‌اند که باعث سردرگمی کاربر و شکست در قابلیت‌های وعده داده شده می‌شود.
```

### Step 3: پیاده‌سازی راه‌اندازی اولیه Web Speech API
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن کد لازم برای بررسی پشتیبانی مرورگر از Web Speech API و ایجاد یک نمونه (instance) از `SpeechRecognition` است. هدف این است که API برای استفاده‌های بعدی آماده شود و وضعیت پشتیبانی آن مشخص گردد. این مرحله شامل شروع فرآیند تشخیص گفتار یا پردازش نتایج آن نیست، بلکه فقط زیرساخت اولیه را فراهم می‌کند.
**Excerpt:**
```
...۲) برای تشخیص گفتار (🎙️) مرورگر باید Web Speech API را پشتیبانی کند. ... همچنین، هیچ کدی برای راه‌اندازی یا استفاده از Web Speech API برای تشخیص گفتار یافت نشد.
```

### Step 4: پیاده‌سازی منطق تشخیص گفتار با Web Speech API
**Status:** `pending` (0%)
**Scope:** این مرحله شامل افزودن منطق برای شروع و توقف فرآیند تشخیص گفتار با استفاده از نمونه `SpeechRecognition` ایجاد شده در مرحله قبل است. همچنین، باید event listenerها برای دریافت نتایج تشخیص (مانند `onresult`) و مدیریت خطاها (مانند `onerror`) پیاده‌سازی شوند. هدف این است که گفتار کاربر به متن تبدیل شود و این متن قابل دسترسی باشد.
**Excerpt:**
```
...همچنین، هیچ کدی برای راه‌اندازی یا استفاده از Web Speech API برای تشخیص گفتار یافت نشد. این دو فیچر در `README.md` تبلیغ شده‌اند اما پیاده‌سازی نشده‌اند که باعث سردرگمی کاربر و شکست در قابلیت‌های وعده داده شده می‌شود.
```
