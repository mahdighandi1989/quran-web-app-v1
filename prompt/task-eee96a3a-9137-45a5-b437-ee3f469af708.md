---
task_id: eee96a3a-9137-45a5-b437-ee3f469af708
title: Ensure App.jsx loads full Quran page structure from quranPages.json
type: bug
priority: high
...
created_at: '2026-06-12T19:13:04.347530+00:00'
updated_at: '2026-06-12T19:30:12.404208+00:00'
target_files:
- src/App.jsx
- src/data/quranPages.json
---

# Ensure App.jsx loads full Quran page structure from quranPages.json

## Raw Idea

A previous task (`task-173fd615`, 'تکمیل ساختار 604 صفحه قرآن', marked 'applied externally, pending verify') indicated that `App.jsx` was using a partial `QURAN_PAGE_STRUCTURE_DEFAULT`. The project now includes `src/data/quranPages.json`, which is the logical place for the complete 604-page structure. If `App.jsx` still relies on a hardcoded, incomplete default instead of dynamically loading the full data from `quranPages.json`, the application's page-based navigation and display for most of the Quran will be broken. This is a critical data loading inconsistency that impacts core functionality.
...
- `src/App.jsx` — The main application component responsible for initializing and loading core data. (Specific lines cannot be provided without the full content of `App.jsx`, but the change would be in the data initialization section).
- `src/data/quranPages.json` — The source of the complete page structure data.

## 🧭 هدف اصلی پروژه (از یادداشت کاربر)
[auto-re-registered from github_import at 2026-05-29T10:02:34.527455+00:00]

## 🧱 پشتهٔ فناوری و معماری
React, JSON data loading.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `scripts/generate-quran-pages.mjs` — Script that likely generates `quranPages.json`.
- `src/lib/quran.js` — Utility functions related to Quran data, may need to consume the full structure.
- `src/test/quranPages.test.js` — Tests related to Quran page data, should verify the full structure is loaded.
- `prompt/archive/task-173fd615-c79d-4dea-bc4b-5214a7827daa.md` — Previous task related to this issue, needs re-verification.
...
## 🌐 نقشهٔ وابستگی‌ها
This is a frontend data loading fix. It ensures the application has complete Quran data, which is crucial for navigation and display. It affects UI components that display or navigate Quran pages. No backend changes are expected.
...
- [ ] The application successfully loads and uses the full 604-page structure from `src/data/quranPages.json`.
- [ ] Page navigation and display for all Quran pages (1-604) function correctly.
- [ ] Any partial hardcoded `QURAN_PAGE_STRUCTURE_DEFAULT` in `App.jsx` is removed or replaced by the imported data.
- [ ] The status of `task-173fd615` is updated to 'verified' after confirming the fix.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Modify `src/App.jsx` to import and utilize the complete Quran page structure data from `src/data/quranPages.json`. Remove or replace any partial hardcoded `QURAN_PAGE_STRUCTURE_DEFAULT` if it still exists and is being used, ensuring the application always uses the full dataset.

## 💡 نمونه‌های قبل/بعد
**App.jsx before (using partial default)**

_قبل:_
```
const QURAN_PAGE_STRUCTURE_DEFAULT = [ { page: 1, surah_number: 1, ayah_number: 1, ... }, { page: 2, ... }, /* ... other pages (only a few) ... */ ];
// ... then using QURAN_PAGE_STRUCTURE_DEFAULT
```

**App.jsx after (loading full data from JSON)**

_بعد:_
```
import quranPagesData from './data/quranPages.json';
// ... then use quranPagesData (which contains all 604 pages) in application state or context.
```
...
## 🧪 دستورات اعتبارسنجی
- `npm run dev`
- `npm test src/test/quranPages.test.js`

## ⚠️ ریسک‌ها و موارد احتیاط
Incorrect data parsing or integration could lead to display errors or incorrect page navigation.
...
1. The application successfully loads and uses the full 604-page structure from `src/data/quranPages.json`. _(verify: ui_interaction)_
2. Page navigation and display for all Quran pages (1-604) function correctly. _(verify: ui_interaction)_
3. Any partial hardcoded `QURAN_PAGE_STRUCTURE_DEFAULT` in `App.jsx` is removed or replaced by the imported data. _(verify: static)_
4. The status of `task-173fd615` is updated to 'verified' after confirming the fix. _(verify: manual_only)_

## Task Steps

### Step 1: وارد کردن (Import) ساختار کامل صفحات قرآن از `src/data/quranPages.json` به `App.jsx`
**Status:** `pending` (0%)
**Scope:** این مرحله بر روی اصلاح `App.jsx` تمرکز دارد تا به جای استفاده از هرگونه ساختار پیش‌فرض ناقص یا hardcoded، ساختار کامل ۶۰۴ صفحه‌ای قرآن را از فایل `src/data/quranPages.json` به صورت پویا بارگذاری کند. هدف این است که داده‌های صحیح و کامل در دسترس کامپوننت `App.jsx` قرار گیرد، بدون اینکه هنوز نحوه استفاده از آن در منطق ناوبری تغییر کند. این گام، دسترسی به منبع داده معتبر را تضمین می‌کند.
**Excerpt:**
```
A previous task (`task-173fd615`, 'تکمیل ساختار 604 صفحه قرآن', marked 'applied externally, pending verify') indicated that `App.jsx` was using a partial `QURAN_PAGE_STRUCTURE_DEFAULT`. The project now includes `src/data/quranPages.json`, which is the logical place for the complete 604-page structure. If `App.jsx` still relies on a hardcoded, incomplete default instead of dynamically loading the full data from `quranPages.json`, the application's page-based navigation and display for most of the Quran will be broken.
```

### Step 2: به‌روزرسانی منطق `App.jsx` برای استفاده از داده‌های `quranPages.json` به جای ساختار پیش‌فرض
**Status:** `pending` (0%)
**Scope:** پس از اطمینان از اینکه `src/data/quranPages.json` در `App.jsx` در دسترس است، این مرحله شامل شناسایی و جایگزینی تمام ارجاعات به `QURAN_PAGE_STRUCTURE_DEFAULT` یا هر ساختار hardcoded و ناقص دیگری با داده‌های بارگذاری شده از `quranPages.json` در منطق داخلی `App.jsx` است. این تغییر تضمین می‌کند که هر تابعی که به ساختار صفحات قرآن نیاز دارد، از داده‌های کامل و صحیح استفاده کند.
**Excerpt:**
```
If `App.jsx` still relies on a hardcoded, incomplete default instead of dynamically loading the full data from `quranPages.json`, the application's page-based navigation and display for most of the Quran will be broken. This is a critical data loading inconsistency that impacts core functionality.
```

### Step 3: تأیید عملکرد ناوبری و نمایش صفحات قرآن با ساختار داده کامل
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تست جامع و تأیید عملکرد ناوبری مبتنی بر صفحه و نمایش محتوای قرآن در سراسر برنامه است. هدف این است که اطمینان حاصل شود که پس از به‌روزرسانی منبع داده در `App.jsx`، تمام ۶۰۴ صفحه قرآن به درستی قابل دسترسی، ناوبری و نمایش هستند و هیچ مشکلی در بارگذاری یا نمایش محتوا وجود ندارد، به خصوص برای صفحاتی که قبلاً به دلیل داده‌های ناقص دچار مشکل بودند.
**Excerpt:**
```
If `App.jsx` still relies on a hardcoded, incomplete default instead of dynamically loading the full data from `quranPages.json`, the application's page-based navigation and display for most of the Quran will be broken. This is a critical data loading inconsistency that impacts core functionality.
```
