---
task_id: 7cb33796-2e31-4e89-8ac1-dd39458f4d21
title: افزودن تست‌های واحد برای کتابخانه اصلی قرآن
type: bug
priority: high
...
created_at: '2026-06-12T19:14:40.032927+00:00'
updated_at: '2026-06-12T19:32:31.044480+00:00'
target_files:
- src/lib/quran.js
---

# افزودن تست‌های واحد برای کتابخانه اصلی قرآن

## Raw Idea

فایل `src/lib/quran.js` حاوی منطق اصلی برای دسترسی و مدیریت داده‌های قرآن (مانند آیات، سوره‌ها، صفحات) است. این فایل هیچ تست واحد اختصاصی ندارد. از آنجایی که این کتابخانه توسط چندین کامپوننت و ویژگی برنامه استفاده می‌شود، عدم وجود تست‌ها می‌تواند منجر به خطاهای پنهان در نمایش داده‌های قرآن یا منطق برنامه شود.
...
- `src/lib/quran.js:1-end` — `quran.js` — فایل کتابخانه اصلی قرآن که نیاز به تست دارد.
  ```jsx
  /* Quran data access and utility functions */
  // (truncated)
  ```
...
- `src/data/quranPages.json` — فایل داده‌ای که `quran.js` برای کار با آن طراحی شده است.
- `src/components/AIExamGenerator.jsx` — کامپوننتی که از داده‌های قرآن برای تولید سوالات استفاده می‌کند و به صحت `quran.js` وابسته است.
- `src/App.jsx` — کامپوننت اصلی که احتمالاً از توابع `quran.js` برای بارگذاری و نمایش داده‌ها استفاده می‌کند.

## 🌐 نقشهٔ وابستگی‌ها
این کتابخانه یک لایه انتزاعی برای داده‌های قرآن فراهم می‌کند و توسط ویژگی‌های مختلف برنامه استفاده می‌شود. تست‌ها باید اطمینان حاصل کنند که داده‌ها به درستی بازیابی و فرمت می‌شوند.
...
- [ ] فایل `src/test/quran.test.js` ایجاد شود.
- [ ] تست‌هایی برای توابع `getAyah`, `getSurah`, `getPage` اضافه شود.
- [ ] تست‌هایی برای سناریوهای لبه (مانند شماره سوره/آیه نامعتبر) اضافه شود.
- [ ] تست‌هایی برای اطمینان از صحت داده‌های بازگردانده شده اضافه شود.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد یک فایل تست واحد جدید (`src/test/quran.test.js`) برای پوشش دادن توابع اصلی در `src/lib/quran.js`، شامل بازیابی آیات، سوره‌ها، و مدیریت صفحات.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## 🧪 دستورات اعتبارسنجی
- `npm test -- src/test/quran.test.js`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به Mock کردن یا بارگذاری داده‌های `quranPages.json` برای تست‌های مستقل.
...
1. فایل `src/test/quran.test.js` ایجاد شود. _(verify: static)_
2. تست‌هایی برای توابع `getAyah`, `getSurah`, `getPage` اضافه شود. _(verify: backend_test)_
3. تست‌هایی برای سناریوهای لبه (مانند شماره سوره/آیه نامعتبر) اضافه شود. _(verify: backend_test)_
4. تست‌هایی برای اطمینان از صحت داده‌های بازگردانده شده اضافه شود. _(verify: backend_test)_
