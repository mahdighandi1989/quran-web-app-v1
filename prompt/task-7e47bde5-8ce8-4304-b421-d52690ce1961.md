---
task_id: 7e47bde5-8ce8-4304-b421-d52690ce1961
title: تکمیل ساختار ۶۰۴ صفحه قرآن و تولید `quranPages.json`
type: bug
priority: high
...
created_at: '2026-06-12T19:11:43.746106+00:00'
updated_at: '2026-06-12T19:29:02.005618+00:00'
target_files:
- scripts/generate-quran-pages.mjs
- src/data/quranPages.json
---

# تکمیل ساختار ۶۰۴ صفحه قرآن و تولید `quranPages.json`

## Raw Idea

داده‌های `QURAN_PAGE_STRUCTURE_DEFAULT` (که در تسک `173fd615-c79d-4dea-bc4b-5214a7827daa.md` به آن اشاره شده) و احتمالاً `src/data/quranPages.json` ناقص هستند و فقط شامل تعداد محدودی از صفحات قرآن (مثلاً ۵ صفحه از ۶۰۴ صفحه) می‌باشند. این نقص باعث می‌شود که نمایش صفحه‌بندی قرآن برای اکثر صفحات به درستی کار نکند و قابلیت‌های مرتبط با ناوبری و تمرین بر اساس صفحه، مختل شود. وجود اسکریپت `scripts/generate-quran-pages.mjs` نشان می‌دهد که مکانیزمی برای تولید این داده‌ها وجود دارد، اما به نظر می‌رسد که این اسکریپت اجرا نشده یا خروجی آن به درستی در پروژه قرار نگرفته است.
...
- `scripts/generate-quran-pages.mjs:all` — `(script execution)`
  ```
  // scripts/generate-quran-pages.mjs
  // This script is responsible for generating the full Quran page structure.
  ```
- `src/data/quranPages.json:all` — `(file content)`
  ```json
  // Expected to contain data for all 604 pages
  ```
...
Node.js (for script), React (for frontend consumption), JSON data

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/App.jsx` — کامپوننت اصلی برنامه که داده‌های ساختار صفحه را مصرف می‌کند.
- `src/lib/quran.js` — احتمالاً شامل منطق مربوط به داده‌های قرآن است.
- `src/test/quranPages.test.js` — تست‌های مربوط به داده‌های صفحات قرآن.

## 🌐 نقشهٔ وابستگی‌ها
این مشکل بر روی تمام قابلیت‌های برنامه که به ساختار کامل صفحات قرآن متکی هستند (مانند ناوبری، نمایش پیشرفت، و تمرینات صفحه‌محور) تأثیر می‌گذارد.
...
- [ ] `src/data/quranPages.json` شامل داده‌های کامل برای تمام ۶۰۴ صفحه قرآن است.
- [ ] برنامه به درستی تمام ۶۰۴ صفحه را نمایش داده و بین آن‌ها ناوبری می‌کند.
- [ ] تست‌های مرتبط (مانند `src/test/quranPages.test.js`) با موفقیت اجرا می‌شوند و کامل بودن و صحت ساختار صفحه را تأیید می‌کنند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. ۱. اجرای اسکریپت `scripts/generate-quran-pages.mjs` برای تولید کامل داده‌های ساختار ۶۰۴ صفحه قرآن. ۲. اطمینان از اینکه خروجی این اسکریپت (احتمالاً `src/data/quranPages.json`) به درستی در پروژه قرار گرفته و توسط `src/App.jsx` بارگذاری می‌شود. ۳. بررسی `src/App.jsx` برای اطمینان از اینکه `QURAN_PAGE_STRUCTURE_DEFAULT` (اگر هنوز استفاده می‌شود) یا `quranPages.json` به درستی بارگذاری و استفاده می‌شود.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## ⚠️ ریسک‌ها و موارد احتیاط
اسکریپت ممکن است دارای باگ باشد یا به داده‌های ورودی خاصی نیاز داشته باشد که موجود نیست.
...
1. `src/data/quranPages.json` شامل داده‌های کامل برای تمام ۶۰۴ صفحه قرآن است. _(verify: static)_
2. برنامه به درستی تمام ۶۰۴ صفحه را نمایش داده و بین آن‌ها ناوبری می‌کند. _(verify: ui_interaction)_
3. تست‌های مرتبط (مانند `src/test/quranPages.test.js`) با موفقیت اجرا می‌شوند و کامل بودن و صحت ساختار صفحه را تأیید می‌کنند. _(verify: backend_test)_
