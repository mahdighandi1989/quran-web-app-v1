---
task_id: b9dc0934-bc78-4451-bf44-bb476e5a4601
title: ⬆️ ai.mjs ممکن است نیاز به update داشته باشد به‌خاطر تغییرات در telegram-bot.mjs
...
created_at: '2026-06-12T19:23:08.509712+00:00'
updated_at: '2026-06-12T19:39:32.631240+00:00'
target_files:
- server/ai.mjs
---

# ⬆️ ai.mjs ممکن است نیاز به update داشته باشد به‌خاطر تغییرات در telegram-bot.mjs
...
## 🔍 جزئیات
- 📂 فایل تغییریافته (consumer): `server/telegram-bot.mjs`
- 📂 فایل upstream (producer — نیازمند update): `server/ai.mjs`
- 🔥 risk: **medium**
- 📝 reason: فایل تغییر یافته `server/telegram-bot.mjs` اکنون توابع `resolveAI`, `chat` (با نام مستعار `aiChat`), و `prompts` (با نام مستعار `aiPrompts`) را از `server/ai.mjs` ایمپورت می‌کند. برای اینکه `telegram-bot.mjs` به درستی کار کند، `server/ai.mjs` باید این توابع/متغیرها را اکسپورت کند. بدون دسترسی به محتوای `server/ai.mjs`، مشخص نیست که آیا این اکسپورت‌ها در حال حاضر وجود دارند یا خیر.
...
---
[scan #2 at 2026-06-12T19:23:09.181551+00:00]
## 📋 شرح
یک فایل تغییر کرده و حالا از API/behavior جدیدی استفاده می‌کند که upstream فعلی ممکن است پشتیبانی نکند.

## 🔍 جزئیات
- 📂 فایل تغییریافته (consumer): `server/telegram-bot.mjs`
- 📂 فایل upstream (producer — نیازمند update): `server/ai-proxy.mjs`
- 🔥 risk: **medium**
- 📝 reason: فایل تغییر یاف
...
- `server/telegram-bot.mjs` — consumer که تغییر کرده

## 🌐 نقشهٔ وابستگی‌ها
`server/telegram-bot.mjs` به `server/ai.mjs` وابسته است.
...
- [ ] بررسی شد `server/telegram-bot.mjs` چه API ای از `server/ai.mjs` می‌خواهد
- [ ] اگر API در `server/ai.mjs` نیست، اضافه شد
...
1. گام ۱: `server/telegram-bot.mjs` را بخوان و usages جدید از `server/ai.mjs` را پیدا کن.
گام ۲: API contract در `server/ai.mjs` چک کن.
گام ۳: اگر contract ناسازگار است، `server/ai.mjs` را extend کن.

## 📤 خروجی مورد انتظار
تغییر کد در فایل‌های مرتبط، commit یا PR جدید با پیام واضح، و عبور تمام معیارهای پذیرش.

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر `server/ai.mjs` ممکن است بقیه consumers را تحت تأثیر قرار دهد — قبل از تغییر، همهٔ مصرف‌کنندگانش را check کن.
...
1. بررسی شد `server/telegram-bot.mjs` چه API ای از `server/ai.mjs` می‌خواهد _(verify: static)_
2. اگر API در `server/ai.mjs` نیست، اضافه شد _(verify: static)_
