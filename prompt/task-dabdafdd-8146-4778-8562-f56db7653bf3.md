---
task_id: dabdafdd-8146-4778-8562-f56db7653bf3
title: عدم وجود اسکریپت خودکار برای تولید داده‌های صفحات قرآن
type: bug
priority: high
...
created_at: '2026-06-12T19:14:38.730742+00:00'
updated_at: '2026-06-12T19:31:56.969357+00:00'
target_files:
- package.json
- scripts/generate-quran-pages.mjs
---

# عدم وجود اسکریپت خودکار برای تولید داده‌های صفحات قرآن

## Raw Idea

فایل `scripts/generate-quran-pages.mjs` برای تولید `src/data/quranPages.json` که برای نمایش صفحات قرآن ضروری است، وجود دارد. با این حال، هیچ اسکریپتی در `package.json` ریشه یا پیکربندی `vite` برای اجرای خودکار این اسکریپت یافت نشد. این نشان می‌دهد که تولید داده‌های صفحات قرآن یک مرحلهٔ دستی است که می‌تواند منجر به عدم به‌روزرسانی داده‌ها، خطاهای انسانی، و پیچیدگی در فرآیند build/deployment شود.
...
- `package.json:10-16` — `scripts` — بخش `scripts` در `package.json` ریشه
  ```json
  "scripts": {
  ...
  ```
- `scripts/generate-quran-pages.mjs:نامشخص` — `نامشخص` — اسکریپت تولیدکنندهٔ داده که باید توسط اسکریپت npm فراخوانی شود.
  ```
  نامشخص
  ```
...
Node.js, npm scripts, Vite build process.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/data/quranPages.json` (سطر نامشخص) — فایل خروجی تولید شده توسط اسکریپت
- `src/lib/quran.js` (سطر نامشخص) — ماژولی که از داده‌های صفحات قرآن استفاده می‌کند
- `README.md` (سطر نامشخص) — مستندات پروژه که ممکن است نیاز به به‌روزرسانی داشته باشد

## 🌐 نقشهٔ وابستگی‌ها
این اسکریپت داده‌های حیاتی برنامه را تولید می‌کند. اتوماسیون آن، قابلیت اطمینان و سهولت توسعه/استقرار را افزایش می‌دهد. افزودن اسکریپت جدید به `package.json` تأثیری بر وابستگی‌های موجود ندارد.
...
- [ ] دستور `npm run generate-quran-data` فایل `src/data/quranPages.json` را با موفقیت تولید یا به‌روز کند.
- [ ] `README.md` در صورت لزوم، دستورالعمل‌های مربوط به این اسکریپت را به‌روز کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک اسکریپت جدید (مثلاً `generate-quran-data`) را به بخش `scripts` در `package.json` ریشه اضافه کنید که `scripts/generate-quran-pages.mjs` را با استفاده از `node` اجرا کند. همچنین، در صورت لزوم، این اسکریپت را به workflowهای build یا pre-commit اضافه کنید.

## 💡 نمونه‌های قبل/بعد
**افزودن اسکریپت تولید داده**
...
"scripts": {
        ...
        "test": "vitest",
        "test:watch": "vitest --watch",
        "generate-quran-data": "node scripts/generate-quran-pages.mjs"
      },
```
...
احتمال خطای مسیردهی یا اجرای اسکریپت.
...
1. دستور `npm run generate-quran-data` فایل `src/data/quranPages.json` را با موفقیت تولید یا به‌روز کند. _(verify: backend_test)_
2. `README.md` در صورت لزوم، دستورالعمل‌های مربوط به این اسکریپت را به‌روز کند. _(verify: static)_
