---
task_id: 40ec73aa-958f-44ec-bc0c-9884b4da661a
title: مقداردهی اولیه Firebase Analytics خطاها را بی‌صدا می‌بلعد
type: bug
priority: high
...
created_at: '2026-06-05T18:43:58.183840+00:00'
updated_at: '2026-06-06T04:58:05.129622+00:00'
target_files:
- src/lib/firebase.js
---

# مقداردهی اولیه Firebase Analytics خطاها را بی‌صدا می‌بلعد

## Raw Idea

در index.html (اسکریپت inline) و احتمالاً در src/lib/firebase.js، فراخوانی getAnalytics در یک try/catch خالی پیچیده شده است. این باعث می‌شود هر خطای اولیه‌سازی Analytics (مثلاً مسدود شدن توسط ad-blocker، ناسازگاری مرورگر، یا پیکربندی ناقص Firebase) کاملاً بی‌صدا swallow شود. در نتیجه تیم توسعه هرگز از شکست Analytics مطلع نمی‌شود و حلقه بازخورد (feedback loop) برای مانیتورینگ و دیباگ قطع می‌شود. شواهد: کامنت 'try { getAnalytics(fbApp); } catch {}' در کد.
...
- `src/lib/firebase.js:25-30` — `getAnalytics` — catch block خالی — باید لاگ و notification اضافه شود
  ```jsx
  try { getAnalytics(fbApp); } catch (e) { /* silent */ }
  ```
...
Firebase v12 Web SDK, Vite, React 18

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/lib/storage.js` (سطر 45) — حاوی notifyCriticalEvent برای اعلان خطاهای بحرانی
- `src/App.jsx` (سطر 12) — firebase.js را import می‌کند و از appStateStore برای mirror کردن وضعیت استفاده می‌کند
- `src/App.test.jsx` — این فایل `firebase.js` را import می‌کند (caller)
...
## 🌐 نقشهٔ وابستگی‌ها
این تغییر فقط روی firebase.js اثر می‌گذارد؛ downstreamها (App.jsx, aiStore.js, appStateStore.js, telegramStore.js) تغییری در interface نمی‌بینند.
...
- [ ] catch block دیگر خالی نیست و خطا را با console.warn لاگ می‌کند
- [ ] notifyCriticalEvent با event name 'analytics_init_failed' فراخوانی می‌شود
- [ ] تست واحد جدید در src/test/firebaseConfig.test.js اضافه شود که catch block را با mock کردن getAnalytics تست کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. catch block را با یک console.warn معنادار جایگزین کن که event name و خطا را لاگ کند. همچنین یک critical-event notification (از src/lib/storage.js) برای خطاهای Analytics اضافه کن تا اگر sink (مثلاً تلگرام) متصل است، اعلان شود.
...
_بعد:_
```
try { getAnalytics(fbApp); } catch (e) { console.warn('Firebase Analytics init failed:', e); notifyCriticalEvent('analytics_init_failed', 'راه‌اندازی Firebase Analytics ناموفق بود: ' + e.message, { priority: 'low', silent: true }); }
```
...
## 🧪 دستورات اعتبارسنجی
- `npm test -- --run src/test/firebaseConfig.test.js`
- `grep -r 'analytics_init_failed' src/`

## ⚠️ ریسک‌ها و موارد احتیاط
هیچ — فقط catch block را بهبود می‌بخشد
...
1. catch block دیگر خالی نیست و خطا را با console.warn لاگ می‌کند _(verify: static)_
2. notifyCriticalEvent با event name 'analytics_init_failed' فراخوانی می‌شود _(verify: static)_
3. تست واحد جدید در src/test/firebaseConfig.test.js اضافه شود که catch block را با mock کردن getAnalytics تست کند _(verify: backend_test)_
