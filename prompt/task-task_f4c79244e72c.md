...
updated_at: '2026-06-06T06:05:45.257428+00:00'
tags:
- consolidated
- post_verify_merge
---
...
📌 دلیل تلفیق (rationale توسط AI): این خوشه بر توسعه منطق نوتیفیکیشن‌های پیشگیرانه، تعریف و اعمال پالیسی‌های 'silent flag' بر اساس اهمیت نوتیفیکیشن، رفع ابهامات در کامپوننت TelegramSettings، و افزودن قابلیت تولید کپشن برای نوتیفیکیشن‌ها تمرکز دارد. همچنین شامل افزودن نوتیفیکیشن برای رویدادهای خاص و رفع یک anti-pattern در کامپوننت تنظیمات تلگرام است.
🎯 theme: توسعه و بهبود سیستم نوتیفیکیشن‌های پیشگیرانه و مدیریت پالیسی‌ها
💎 estimated_difficulty: large

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 1 از 6
  id: 9aa136b7-c5f1-4a19-bdb0-60bfb154e3b9
  عنوان اصلی: Develop Proactive Notification Triggering Logic
  اولویت اصلی: critical
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - هر دو طرف ناسازگاری شناسایی + فرض‌هایشان مستند شد [verify_method=static] [verify_plan={"grep_patterns": ["inconsistency identified", "assumptions documented", "proactive notification logic"], "files_hint": ["src/lib/appStateStore.js", "src/lib/telegramCommands.js", "src/services/notifi]
  - ground truth تعیین شد و طرف دیگر align شد [verify_method=static] [verify_plan={"grep_patterns": ["ground truth established", "aligned with new logic", "proactive notification trigger"], "files_hint": ["src/lib/appStateStore.js", "src/services/notification_pipeline.js", "src/eve]
  - integration test برای pipeline `notification` بدون شکست عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_notification_pipeline.py::test_proactive_notification_triggering", "timeout_seconds": 60}]
...
[منطق] Undefined Proactive Notification Triggering & Logic
...
The provided components lack a clear mechanism for detecting application events (e.g., session completion, daily progress, reminders) and initiating a proactive notification flow. `src/lib/appStateStore.js` stores relevant data, but its purpose is persistence, not event triggering. `src/lib/telegramCommands.js` handles reactive command responses, not proactive system-generated notifications. There's no component responsible for the logic of deciding *when* to send a notification based on applica

## 💥 پیامد (impact)
The notification pipeline can only respond to direct user commands. It cannot send automated, time-based, or event-driven notifications, which are often a core part of a 'notification pipeline' for user engagement and reminders. This severely limits the utility and completeness of the pipeline.

## 🛠 پیشنهاد رفع اولیه
Introduce a new service (e.g., `src/lib/notificationScheduler.js` or a server-side component) responsible for monitoring application state changes, evaluating notification conditions, and triggering `src/lib/telegram.js` to send messages. This service would embody the 'event → notify_event' logic.
...
✅ مراحل قبلاً done شده (در super-task به‌عنوان pre_done):
  - بررسی وابستگی‌های upstream: اطمینان از سازگاری با appStateStore.js

🔧 مراحل remaining که در super-task باید انجام شوند:
  - ایجاد سرویس جدید notificationScheduler.js برای مانیتورینگ وضعیت اپلیکیشن — فایل `src/lib/notificationScheduler.js` ایجاد نشده و سرویس جدید پیاده‌سازی نشده است.
  - پیاده‌سازی منطق تشخیص رویدادهای اپلیکیشن (session completion, daily progress, reminders) — منطق تشخیص رویدادهای اپلیکیشن به دلیل عدم وجود `notificationScheduler.js` پیاده‌سازی نشده است.
  - پیاده‌سازی منطق ارزیابی شرایط نوتیفیکیشن (notification conditions) — منطق ارزیابی شرایط نوتیفیکیشن به دلیل عدم وجود `notificationScheduler.js` پیاده‌سازی نشده است.
  - اتصال notificationScheduler به telegram.js برای ارسال نوتیفیکیشن — اتصال `notificationScheduler` به `telegram.js` به دلیل عدم وجود فایل اصلی انجام نشده است.
  - بررسی وابستگی‌های downstream: اطمینان از عدم شکستن telegramCommands.js — هیچ شواهدی از بررسی یا تغییر `src/lib/telegramCommands.js` یافت نشد.
  - نوشتن تست‌های واحد برای notificationScheduler.js — تست‌های واحد برای `notificationScheduler.js` به دلیل عدم وجود فایل اصلی یافت نشد.
  - نوشتن تست‌های یکپارچه‌سازی برای pipeline نوتیفیکیشن — تست‌های یکپارچه‌سازی برای pipeline نوتیفیکیشن در حال انجام است اما کامل نیست یا با شکست مواجه می‌شود.
  - بررسی نهایی و مستندسازی تغییرات — بررسی نهایی و مستندسازی معماری جدید به دلیل عدم پیاده‌سازی اجزای اصلی انجام نشده است.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 6
  id: defeca98-2356-40ad-8212-03877bd94fc7
  عنوان اصلی: Enforce Criticality-Based Silent Flag Policy
  اولویت اصلی: critical
  وضعیت verify قبلی: partial
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - هر دو طرف ناسازگاری شناسایی + فرض‌هایشان مستند شد [verify_method=static] [verify_plan={"grep_patterns": ["inconsistency identified", "assumptions documented", "silent flag logic"], "files_hint": ["backend/app/notification_pipeline.py", "docs/adr/00x_silent_flag_logic.md"]}]
  - ground truth تعیین شد و طرف دیگر align شد [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_notification_pipeline.py::test_silent_flag_ground_truth", "timeout_seconds": 60}]
  - integration test برای pipeline `notification` بدون شکست عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_notification_pipeline.py", "timeout_seconds": 120}]
  - PR description توضیح می‌دهد چرا این تصمیم گرفته شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
🔧 مراحل remaining که در super-task باید انجام شوند:
  - تعریف پالیسی clear برای silent flag: non-critical قابل تنظیم کاربر، critical غیرقابل سایلنت — تعریف پالیسی clear برای silent flag: non-critical قابل تنظیم کاربر، critical غیرقابل سایلنت.
  - پیاده‌سازی مکانیزم کلاسیفای کردن نوتیفیکیشن‌ها به سه سطح: critical, important, routine — پیاده‌سازی مکانیزم کلاسیفای کردن نوتیفیکیشن‌ها به سه سطح: critical, important, routine.
  - اعمال پالیسی silent در منطق ارسال نوتیفیکیشن: criticalها هرگز سایلنت نشوند — اعمال پالیسی silent در منطق ارسال نوتیفیکیشن: criticalها هرگز سایلنت نشوند.
  - به‌روزرسانی TelegramSettings.jsx برای نمایش تنظیمات silent فقط برای نوتیفیکیشن‌های غیربحرانی — به‌روزرسانی TelegramSettings.jsx برای نمایش تنظیمات silent فقط برای نوتیفیکیشن‌های غیربحرانی.
  - نوشتن تست‌های واحد برای مکانیزم کلاسیفای کردن و اعمال پالیسی silent — نوشتن تست‌های واحد برای مکانیزم کلاسیفای کردن و اعمال پالیسی silent.
  - بررسی وابستگی‌های upstream: شناسایی فایل‌ها و توابعی که به silent flag وابسته هستند — بررسی وابستگی‌های upstream: شناسایی فایل‌ها و توابعی که به silent flag وابسته هستند.
  - بررسی وابستگی‌های downstream: شناسایی بخش‌هایی که به این تسک متکی هستند — بررسی وابستگی‌های downstream: شناسایی بخش‌هایی که به این تسک متکی هستند.

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 6
  id: 1b3acfe3-9d19-4da8-968a-0d86628821c2
  عنوان اصلی: Resolve Anti-pattern: Under-engineering
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/components/TelegramSettings.jsx
...
📋 acceptance_criteria کامل:
  ...
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["patch\\(", "addReminder"], "files_hint": ["src/components/TelegramSettings.jsx"]}]
  - تست edge case نوشته شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
- `src/components/TelegramSettings.jsx:128`
...
- `src/App.jsx` — این فایل `TelegramSettings.jsx` را import می‌کند (caller)
- `src/test/TelegramSettings.test.jsx` — این فایل `TelegramSettings.jsx` را import می‌کند (caller)

## 🔍 Context و وضعیت فعلی
The `addReminder` function is incomplete. It validates the input (`newReminder.text.trim()`) but then abruptly ends without updating the component's configuration state (the `patch` call to add the reminder is missing). This makes the 'add reminder' feature non-functional, indicating a significant logical gap in the implementation. While a linter might flag the undefined `p`, the core issue is the

📁 file: src/components/TelegramSettings.jsx (line 128)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 4 از 6
  id: 802f2537-3f0c-49d2-aa74-e59a56e6cb38
  عنوان اصلی: [منطق] Ambiguous Role of TelegramSettings.jsx
  اولویت اصلی: high
  وضعیت verify قبلی: error
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - هر دو طرف ناسازگاری شناسایی + فرض‌هایشان مستند شد [verify_method=static] [verify_plan={"grep_patterns": ["// CONFLICT_RESOLUTION: Ambiguous Role of TelegramSettings.jsx - Side A identified", "// CONFLICT_RESOLUTION: Ambiguous Role of TelegramSettings.jsx - Side B identified"], "files_h]
  - ground truth تعیین شد و طرف دیگر align شد [verify_method=static] [verify_plan={"grep_patterns": ["// GROUND_TRUTH: TelegramSettings.jsx role defined as:", "// ALIGNED: TelegramSettings.jsx now reflects ground truth"], "files_hint": ["src/components/TelegramSettings.jsx", "src/s]
  - integration test برای pipeline `notification` بدون شکست عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_notification_pipeline.py::test_telegram_settings_integration", "timeout_seconds": 60}]
  - PR description توضیح می‌دهد چرا این تصمیم گرفته شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
## 🎯 هدف (خلاصه ساختاریافته)
[منطق] Ambiguous Role of TelegramSettings.jsx
...
The purpose of `src/components/TelegramSettings.jsx` is marked as `?`, and its inputs/outputs are empty. While its name suggests it's for configuring Telegram settings, its specific role in managing user preferences for notifications (e.g., enabling/disabling specific notification types, setting default `silent` mode) is unclear. This component is crucial for user control over the notification experience.

## 💥 پیامد (impact)
Users may not be able to customize their notification preferences, leading to a poor user experience (e.g., receiving unwanted notifications, or missing important ones due to lack of configuration). The system might lack a clear source for user-defined notification parameters, making the 'silent' fl

## 🛠 پیشنهاد رفع اولیه
Clearly define the purpose of `TelegramSettings.jsx` to include managing user preferences for Telegram notifications (e.g., enabling/disabling notification types, setting default `silent` behavior). Specify its expected inputs (e.g., current user config) and outputs (e.g., updated config object) and interactions (e.g., with `src/lib/telegram.js` for config persistence).
...
🔧 مراحل remaining که در super-task باید انجام شوند:
  - تعریف purpose و scope برای TelegramSettings.jsx
  - تعریف ورودی‌های مورد انتظار TelegramSettings.jsx
  - تعریف تعاملات TelegramSettings.jsx با src/lib/telegram.js
  - بررسی وجود پیاده‌سازی قبلی در repo
  - اضافه کردن کامنت purpose به TelegramSettings.jsx
  - اضافه کردن کامنت ورودی‌ها به TelegramSettings.jsx
  - اضافه کردن کامنت خروجی‌ها به TelegramSettings.jsx
  - اضافه کردن کامنت تعاملات با src/lib/telegram.js به TelegramSettings.jsx
  - بررسی وابستگی‌های upstream برای TelegramSettings.jsx
  - ثبت کامیت نهایی با توضیحات کامل

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 5 از 6
  id: c9d4f5f1-0259-4be8-bb64-cc7e50afa1c4
  عنوان اصلی: [منطق] Missing Caption Generation for Proactive Notifications
  اولویت اصلی: high
  وضعیت verify قبلی: error
  فایل‌های دخیل: -
...
📋 acceptance_criteria کامل:
  ...
  - integration test برای pipeline `notification` بدون شکست عبور می‌کند [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_notification_pipeline.py::test_proactive_notification_caption_generation_integration", "timeout_seconds": 60}]
  - PR description توضیح می‌دهد چرا این تصمیم گرفته شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
While `src/lib/telegramCommands.js` is responsible for generating reply text (captions) for user commands, there is no explicit component or process defined for generating the content (caption) of *proactive* notifications (e.g., 'Your daily review is due,' 'Session completed summary'). The 'caption' step in the pipeline is only partially covered.

## 💥 پیامد (impact)
Without a clear component for generating proactive notification content, the system cannot send meaningful automated messages. This would either lead to generic, uninformative notifications or require ad-hoc content generation within the triggering logic, leading to code duplication and maintainabil

## 🛠 پیشنهاد رفع اولیه
The new notification triggering service (suggested in Issue 1) should either be responsible for generating these captions itself, or delegate to a dedicated formatting utility (similar to how `telegramCommands.js` uses `./format.js`) that can access `appState` and `dataset` (potentially via `appStateStore.js`) to create rich, context-aware notification messages.
...
🔧 مراحل remaining که در super-task باید انجام شوند:
  - تحلیل و مستندسازی ناسازگاری منطقی در pipeline notification
  - بررسی وابستگی‌های upstream برای تولید محتوای proactive notifications
  - طراحی راه‌حل: سرویس اختصاصی تولید caption برای proactive notifications
  - پیاده‌سازی سرویس تولید caption برای proactive notifications
  - یکپارچه‌سازی سرویس جدید با notification triggering service
  - نوشتن unit tests برای proactiveNotificationFormatter.js
  - نوشتن integration tests برای یکپارچه‌سازی با notification triggering service
  - بررسی backward compatibility و رفع مشکلات احتمالی
  - مستندسازی نهایی و بستن issue

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 6 از 6
  id: 0eb7aa00-c768-4ae2-9b7c-2894a0af0e1f
  عنوان اصلی: افزودن نوتیفیکیشن برای event 'scan_failed'
  اولویت اصلی: high
  وضعیت verify قبلی: pending
  فایل‌های دخیل: -

📋 acceptance_criteria کامل:
  - `notify_event("scan_failed", ...)` call در نقطه وقوع اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["notify_event\\(\"scan_failed\", .*"], "files_hint": ["backend/app/handlers/scan_failure_handler.py", "backend/app/tasks/scan_tasks.py"]}]
  ...
  - silent=False + priority="high" [verify_method=static] [verify_plan={"grep_patterns": ["notify_event\\(\"scan_failed\", .*silent=False, .*priority=\"high\".*\\)"], "files_hint": ["backend/app/handlers/scan_failure_handler.py", "backend/app/tasks/scan_tasks.py"]}]
  - تست: trigger مصنوعی → notification در Telegram دیده می‌شود [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}]
...
- در commit message: `merged-from: 9aa136b7-c5f1-4a19-bdb0-60bfb154e3b9, defeca98-2356-40ad-8212-03877bd94fc7, 1b3acfe3-9d19-4da8-968a-0d86628821c2, 802f2537-3f0c-49d2-aa74-e59a56e6cb38, c9d4f5f1-0259-4be8-bb64-cc7e50afa1c4, 0eb7aa00-c768-4ae2-9b7c-2894a0af0e1f`
...
7. تست edge case نوشته شد _(verify: manual_only)_
8. `notify_event("scan_failed", ...)` call در نقطه وقوع اضافه شد _(verify: static)_
9. message template فارسی و معنادار است _(verify: manual_only)_
10. silent=False + priority="high" _(verify: static)_
11. تست: trigger مصنوعی → notification در Telegram دیده می‌شود _(verify: manual_only)_
