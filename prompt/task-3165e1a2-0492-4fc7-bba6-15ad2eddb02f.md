---
task_id: 3165e1a2-0492-4fc7-bba6-15ad2eddb02f
title: '[منطق] Insufficient Content Validation for Non-Exam AI Responses'
type: logic_audit
priority: critical
...
created_at: '2026-06-12T19:10:51.132766+00:00'
updated_at: '2026-06-12T19:28:16.969052+00:00'
---

# [منطق] Insufficient Content Validation for Non-Exam AI Responses
...
While `aiTasks.js` includes `validateExamQuestions` for AI-generated exam questions, there is no explicit mention of content validation for responses from other prompts like `tafsirPrompt`, `hifzPrompt`, or `qaPrompt`. These responses are likely consumed directly by `UI components` without further checks on their relevance, accuracy, or safety.

## 💥 پیامد (impact)
Users might receive irrelevant, malformed, or nonsensical AI responses, leading to a poor user experience. For sensitive tasks like `tafsir` (religious interpretation) and `hifz` (memorization aid), unvalidated responses could lead to the dissemination of misinformation or inaccurate religious conte

## 🛠 پیشنهاد رفع اولیه
Implement specific content validation logic for each type of AI response (tafsir, hifz, QA). This could involve checking for keywords, structure, length, factual accuracy against a knowledge base, or even using a smaller, faster model to evaluate the quality/relevance of the primary model's output.
...
### Step 1: بررسی ساختار فعلی pipeline `ai_llm` و مکانیسم‌های موجود
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی دقیق pipeline `ai_llm` برای شناسایی نقاطی است که پاسخ‌های `tafsirPrompt`, `hifzPrompt`, و `qaPrompt` تولید و مصرف می‌شوند. هدف، درک چگونگی عملکرد `validateExamQuestions` در `aiTasks.js` و تعیین بهترین محل برای تزریق منطق اعتبارسنجی جدید است. این مرحله شامل پیاده‌سازی هیچ کد جدیدی نیست، بلکه صرفاً یک فاز تحقیقاتی و تحلیلی است.
**Excerpt:**
```
در pipeline `ai_llm` یک ناسازگاری منطقی پیدا شد: While `aiTasks.js` includes `validateExamQuestions` for AI-generated exam questions, there is no explicit mention of content validation for responses from other prompts like `tafsirPrompt`, `hifzPrompt`, or `qaPrompt`. این پرامپت بر اساس یک **بررسی اولیهٔ خودکار** از repo ساخته شده — ممکن است حاوی اشتباه، تشخیص نادرست، یا حذف موارد مهم باشد. پیش از شروع، با grep/search و خواندن فایل‌های مرتبط بررسی کن که چه چیزی **از قبل وجود دارد**.
```

### Step 2: پیاده‌سازی منطق اعتبارسنجی محتوا برای پاسخ‌های `tafsirPrompt`
**Status:** `pending` (0%)
**Scope:** این مرحله بر توسعه یک تابع یا ماژول اختصاصی برای اعتبارسنجی محتوای تولید شده توسط `tafsirPrompt` تمرکز دارد. منطق اعتبارسنجی باید شامل بررسی‌هایی مانند کلمات کلیدی مرتبط با تفسیر دینی، ساختار پاسخ، طول مناسب، و دقت واقعی در برابر یک پایگاه دانش (در صورت وجود) باشد. این تابع باید بتواند پاسخ‌های نامربوط، بدشکل، یا غیرمنطقی را شناسایی کند.
— [merged] این مرحله شامل توسعه یک تابع یا ماژول اختصاصی برای اعتبارسنجی محتوای تولید شده توسط `hifzPrompt` است. منطق اعتبارسنجی باید متناسب با نیازهای کمک به حفظ باشد، از جمله بررسی دقت متن، ساختار صحیح برای حفظ، و جلوگیری از پاسخ‌های نامربوط یا گمراه‌کننده. این تابع باید بتواند پاسخ‌های نامعتبر را شناسایی کند.
— [merged] این مرحله شامل توسعه یک تابع یا ماژول اختصاصی برای اعتبارسنجی محتوای تولید شده توسط `qaPrompt` است. منطق اعتبارسنجی باید بر روی مرتبط بودن پاسخ به سوال، دقت اطلاعات، و ساختار قابل فهم تمرکز کند. این تابع باید بتواند پاسخ‌های نامربوط، بدشکل، یا غیرمنطقی را شناسایی کند.
**Excerpt:**
```
Implement specific content validation logic for each type of AI response (tafsir, hifz, QA). This could involve checking for keywords, structure, length, factual accuracy against a knowledge base, or even using a smaller, faster model to evaluate the quality/relevance of the primary model's output. For sensitive tasks like `tafsir` (religious interpretation) and `hifz` (memorization aid), unvalidated responses could lead to the dissemination of misinformation or inaccurate religious conte
```

### Step 3: ادغام منطق اعتبارسنجی جدید در pipeline `ai_llm`
**Status:** `pending` (0%)
**Scope:** این مرحله شامل ادغام توابع اعتبارسنجی توسعه‌یافته برای `tafsirPrompt`, `hifzPrompt`, و `qaPrompt` در pipeline `ai_llm` است. این ادغام باید در نقطه‌ای انجام شود که پاسخ‌های AI تولید شده‌اند اما هنوز به `UI components` ارسال نشده‌اند. هدف این است که هر پاسخ قبل از مصرف توسط UI، از فیلتر اعتبارسنجی مربوطه عبور کند.
**Excerpt:**
```
Implement specific content validation logic for each type of AI response (tafsir, hifz, QA). These responses are likely consumed directly by `UI components` without further checks on their relevance, accuracy, or safety. In pipeline `ai_llm` a logical inconsistency was found: While `aiTasks.js` includes `validateExamQuestions` for AI-generated exam questions, there is no explicit mention of content validation for responses from other prompts like `tafsirPrompt`, `hifzPrompt`, or `qaPrompt`.
```

### Step 4: به‌روزرسانی `UI components` برای مدیریت پاسخ‌های اعتبارسنجی‌شده
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی و به‌روزرسانی `UI components` است که پاسخ‌های `tafsirPrompt`, `hifzPrompt`, و `qaPrompt` را مصرف می‌کنند. هدف این است که UI بتواند وضعیت اعتبارسنجی پاسخ‌ها را به درستی نمایش دهد (مثلاً پیام خطا برای پاسخ‌های نامعتبر، یا نمایش پاسخ‌های معتبر). این ممکن است شامل افزودن منطق برای نمایش پیام‌های کاربرپسند در صورت رد شدن پاسخ AI باشد.
**Excerpt:**
```
Users might receive irrelevant, malformed, or nonsensical AI responses, leading to a poor user experience. For sensitive tasks like `tafsir` (religious interpretation) and `hifz` (memorization aid), unvalidated responses could lead to the dissemination of misinformation or inaccurate religious conte. These responses are likely consumed directly by `UI components` without further checks on their relevance, accuracy, or safety.
```

### Step 5: نوشتن تست‌های واحد و یکپارچه‌سازی برای منطق اعتبارسنجی جدید
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد (unit tests) برای هر یک از توابع اعتبارسنجی (برای `tafsirPrompt`, `hifzPrompt`, و `qaPrompt`) و همچنین تست‌های یکپارچه‌سازی (integration tests) برای اطمینان از اینکه این توابع به درستی در pipeline `ai_llm` ادغام شده‌اند. تست‌ها باید سناریوهای مختلفی از جمله ورودی‌های معتبر، نامعتبر، لبه‌ای و موارد خاص را پوشش دهند.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند. پیش از شروع، با grep/search و خواندن فایل‌های مرتبط بررسی کن که چه چیزی **از قبل وجود دارد**. اگر معیارهای پذیرش (AC) مبهم/ناقص بودند، بهترین تفسیر را انتخاب کن و در commit message توضیح بده.
```

### Step 6: به‌روزرسانی مستندات و انجام ممیزی نهایی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل به‌روزرسانی هرگونه مستندات مرتبط با pipeline `ai_llm`، از جمله نحوه عملکرد اعتبارسنجی جدید، پارامترهای آن، و نحوه مدیریت پاسخ‌های نامعتبر است. همچنین شامل یک ممیزی نهایی برای اطمینان از اینکه تمامی تغییرات با استانداردهای کدنویسی و معماری پروژه مطابقت دارند و هیچ وابستگی شکسته یا مشکلی در همگام‌سازی ایجاد نشده است.
**Excerpt:**
```
📖 **خواندن کامل + اجرای مو-به-مو (بسیار مهم):** این پرامپت — از این یادداشت تا انتها — یک سند واحد است که هر بخشش حاوی الزام یا context منحصربه‌فرد است. 🔗 **وابستگی‌ها و همگام‌سازی (بسیار حیاتی — هرگز skip نکن):** این بخش از همهٔ بخش‌های دیگرِ این یادداشت **مهم‌تر** است. اگر نقض شود، نتیجهٔ کار ممکن است مشروع به‌نظر برسد ولی در عمل بخش‌های دیگر سیستم را عقب بیندازد، broken reference تولید کند، یا منجر به data corruption شود.
```
