---
task_id: 0ed2ef62-c96a-4c11-a5cf-b5dc285bbde6
title: '[منطق] Absence of Explicit Hallucination Guards'
type: logic_audit
priority: critical
...
created_at: '2026-06-12T19:11:13.703024+00:00'
updated_at: '2026-06-12T19:28:28.075390+00:00'
---

# [منطق] Absence of Explicit Hallucination Guards
...
The provided component descriptions do not mention any specific mechanisms or strategies implemented to guard against AI hallucinations across the pipeline. While `validateExamQuestions` might catch some egregious errors for exams, it is not a general hallucination guard. LLMs are inherently prone to generating factually incorrect or plausible-sounding but false information.

## 💥 پیامد (impact)
The application risks providing factually incorrect or fabricated information, especially for critical tasks like `tafsir` and `hifz`. This will lead to a significant erosion of user trust, potential misinformation, and severe reputational damage for the application.

## 🛠 پیشنهاد رفع اولیه
Implement multi-pronged hallucination mitigation strategies. This could include: Grounding responses using RAG (Retrieval Augmented Generation) with verified data, implementing fact-checking mechanisms (e.g., cross-referencing with external APIs or trusted sources), using self-correction prompts, and providing clear disclaimers to users about the experimental nature of AI responses.
...
### Step 1: پیاده‌سازی Grounding Responses با RAG (Retrieval Augmented Generation)
**Status:** `pending` (0%)
**Scope:** این مرحله بر طراحی و پیاده‌سازی یک سیستم Retrieval Augmented Generation (RAG) تمرکز دارد. هدف اصلی این است که قبل از تولید پاسخ توسط مدل زبانی بزرگ (LLM) در pipeline `ai_llm`، اطلاعات مرتبط و تأییدشده از یک پایگاه داده معتبر بازیابی شود. این کار به LLM کمک می‌کند تا پاسخ‌های خود را بر اساس حقایق موجود بنا کند و از تولید اطلاعات نادرست جلوگیری شود. این مرحله شامل ایجاد یا اتصال به یک دیتابیس دانش تأییدشده و اصلاح جریان کاری `ai_llm` برای استفاده از داده‌های بازیابی‌شده است، اما شامل مکانیسم‌های بررسی صحت پس از تولید یا تغییرات UI نمی‌شود.
**Excerpt:**
```
Implement multi-pronged hallucination mitigation strategies. This could include: Grounding responses using RAG (Retrieval Augmented Generation) with verified data, implementing fact-checking mechanisms (e.g., cross-referencing with external APIs or trusted sources), using self-correction prompts, and providing clear disclaimers to users about the experimental nature of AI responses.
```

### Step 2: پیاده‌سازی مکانیسم‌های Fact-Checking (بررسی صحت) پس از تولید پاسخ
**Status:** `pending` (0%)
**Scope:** این مرحله بر توسعه و ادغام مکانیسم‌های بررسی صحت (fact-checking) در pipeline `ai_llm` تمرکز دارد که پس از تولید اولیه پاسخ توسط LLM فعال می‌شوند. این مکانیسم‌ها باید قادر باشند پاسخ‌های تولیدشده را با منابع خارجی معتبر یا APIهای قابل اعتماد مقایسه کنند تا صحت اطلاعات را تأیید کنند. هدف این است که یک لایه تأیید اضافی برای شناسایی و پرچم‌گذاری اطلاعات نادرست احتمالی اضافه شود که ممکن است از مرحله RAG عبور کرده باشند. این مرحله شامل طراحی منطق اعتبارسنجی و ادغام آن با جریان `ai_llm` است.
**Excerpt:**
```
Implement multi-pronged hallucination mitigation strategies. This could include: Grounding responses using RAG (Retrieval Augmented Generation) with verified data, implementing fact-checking mechanisms (e.g., cross-referencing with external APIs or trusted sources), using self-correction prompts, and providing clear disclaimers to users about the experimental nature of AI responses.
```

### Step 3: پیاده‌سازی Self-Correction Prompts در تعامل با LLM
**Status:** `pending` (0%)
**Scope:** این مرحله بر اصلاح استراتژی پرامپت‌نویسی برای مدل زبانی بزرگ (LLM) در pipeline `ai_llm` تمرکز دارد. هدف این است که پرامپت‌ها به گونه‌ای طراحی شوند که LLM را تشویق به بازبینی و تصحیح خودکار پاسخ‌هایش برای دقت و انسجام کنند. این شامل افزودن دستورالعمل‌های صریح در پرامپت‌ها است که از LLM می‌خواهد قبل از ارائه پاسخ نهایی، آن را از نظر صحت و عدم وجود hallucination بررسی کند. این مرحله به مهندسی پرامپت و تنظیم دقیق تعامل با LLM می‌پردازد.
**Excerpt:**
```
Implement multi-pronged hallucination mitigation strategies. This could include: Grounding responses using RAG (Retrieval Augmented Generation) with verified data, implementing fact-checking mechanisms (e.g., cross-referencing with external APIs or trusted sources), using self-correction prompts, and providing clear disclaimers to users about the experimental nature of AI responses.
```

### Step 4: ارائه Disclaimers (سلب مسئولیت‌ها) شفاف به کاربران
**Status:** `pending` (0%)
**Scope:** این مرحله بر طراحی و پیاده‌سازی عناصر رابط کاربری (UI) یا پیام‌های متنی در برنامه تمرکز دارد که به وضوح به کاربران اطلاع می‌دهد که پاسخ‌های تولیدشده توسط هوش مصنوعی ماهیت آزمایشی دارند و ممکن است حاوی اطلاعات نادرست باشند. این شامل شناسایی مکان‌های مناسب در UI برای نمایش این سلب مسئولیت‌ها (مانند کنار پاسخ‌های AI، در بخش 'درباره ما' یا در هنگام اولین استفاده) و اطمینان از وضوح و قابل فهم بودن پیام است. این مرحله یک اقدام پیشگیرانه برای مدیریت انتظارات کاربر و کاهش پیامدهای منفی ناشی از خطاهای احتمالی باقی‌مانده است.
**Excerpt:**
```
Implement multi-pronged hallucination mitigation strategies. This could include: Grounding responses using RAG (Retrieval Augmented Generation) with verified data, implementing fact-checking mechanisms (e.g., cross-referencing with external APIs or trusted sources), using self-correction prompts, and providing clear disclaimers to users about the experimental nature of AI responses.
```
