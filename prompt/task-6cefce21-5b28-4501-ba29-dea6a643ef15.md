---
task_id: 6cefce21-5b28-4501-ba29-dea6a643ef15
title: 'UI option با نام مبهم: pid'
type: audit
priority: low
...
created_at: '2026-06-12T19:23:13.614731+00:00'
updated_at: '2026-06-12T19:41:32.948763+00:00'
target_files:
- src/components/AISettings.jsx
---

# UI option با نام مبهم: pid

## Raw Idea

## 📋 شرح
یک option/checkbox/toggle با نام `pid` در `src/components/AISettings.jsx` دیده شد که نام آن کاربردش را روشن نمی‌کند.

## 🤔 چرا مهم است
option های فراموش‌شده باعث می‌شوند کاربر یا (الف) نفهمد چی کار می‌کند و آن را خاموش بگذارد، یا (ب) به‌اشتباه روشن کند و رفتار غیرمنتظره ببیند.

## 🔍 جزئیات
- علت: UI option با field name مبهم — کاربر هر بار باید بپرسد این چیست
...
- [ ] option `pid` label واضح فارسی/انگلیسی دارد
- [ ] tooltip یا help text توضیح می‌دهد چه کاری می‌کند
- [ ] default value منطقی + در README مستند شده
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. گام ۱: کد را بخوان و کاربرد واقعی `pid` را بفهم.
گام ۲: label/title واضح در UI بگذار + tooltip توضیحی.
گام ۳: در README یا docs مستندسازی کن (یک خط).
...
تغییر default value می‌تواند رفتار کاربران فعلی را تغییر دهد. اگر default می‌خواهی عوض کنی، یک release note بنویس.
...
1. option `pid` label واضح فارسی/انگلیسی دارد _(verify: ui_interaction)_
2. tooltip یا help text توضیح می‌دهد چه کاری می‌کند _(verify: ui_interaction)_
3. default value منطقی + در README مستند شده _(verify: static)_
