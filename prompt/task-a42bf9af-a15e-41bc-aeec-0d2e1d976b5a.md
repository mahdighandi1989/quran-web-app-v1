---
task_id: a42bf9af-a15e-41bc-aeec-0d2e1d976b5a
title: پیکربندی .gitignore برای نادیده گرفتن فایل‌های حساس و build artifacts
type: security
priority: high
...
# پیکربندی .gitignore برای نادیده گرفتن فایل‌های حساس و build artifacts

## Raw Idea

فایل `.gitignore` موجود در ریشه ریپو ناقص است. این فایل شامل الگوهای رایج برای فایل‌های حساس مانند `.env`، `.env.local`، `credentials.json`، `service-account*.json`، `*.key`، `*.pem` نیست. همچنین، فایل‌ها و پوشه‌های تولید شده توسط ابزارهای توسعه مانند `node_modules`، `dist`، `build`، `logs`، و فایل‌های پیکربندی IDE (مثل `.vscode/`) باید نادیده گرفته شوند. عدم وجود `.gitignore` مناسب می‌تواند منجر به commit تصادفی credentials و secrets در مخزن git و همچنین commit شدن فایل‌های غیرضروری و حجیم شود.
...
- `.gitignore:all` — `(file content)`
  ```
  # Current .gitignore content (if any, to be replaced/augmented)
  ```
...
Git, Node.js, React, Vite

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.env.example` — متغیرهای محیطی حساس که باید نادیده گرفته شوند را مستند می‌کند.
- `package.json` — نشان‌دهنده `node_modules` و خروجی‌های build است.
- `vite.config.js` — پوشه خروجی build (معمولاً `dist`) را تعریف می‌کند.
- `tests/test_git_ignore.py` — این تست احتمالاً قوانین `.gitignore` را بررسی می‌کند و باید با قوانین جدید به‌روزرسانی یا تأیید شود.

## 🌐 نقشهٔ وابستگی‌ها
این تغییر روی مدیریت فایل‌ها در Git اثر می‌گذارد و از commit شدن ناخواسته فایل‌های حساس و غیرضروری جلوگیری می‌کند.
...
1. به‌روزرسانی فایل `.gitignore` با الگوهای استاندارد برای پروژه‌های React/Vite و فایل‌های حساس.

## 💡 نمونه‌های قبل/بعد
**نمونه .gitignore استاندارد**
...
pnpm-lock.yaml
yarn.lock
# package-lock.json # Keep this if you want to commit it, otherwise add it. Project has it.
...
- `pytest tests/test_git_ignore.py`

## ⚠️ ریسک‌ها و موارد احتیاط
احتمال نادیده گرفتن تصادفی فایل‌هایی که باید commit شوند (با الگوهای استاندارد بعید است).
