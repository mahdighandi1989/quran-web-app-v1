---
task_id: 744618af-52d2-4afc-b279-cb5209e75a9b
title: تست‌های Firebase در `App.test.jsx` از `vi.restoreAllMocks()` استفاده نمی‌کنند.
type: bug
priority: medium
...
created_at: '2026-06-12T19:19:53.786120+00:00'
updated_at: '2026-06-12T19:36:07.896610+00:00'
target_files:
- src/App.test.jsx
---

# تست‌های Firebase در `App.test.jsx` از `vi.restoreAllMocks()` استفاده نمی‌کنند.

## Raw Idea

در فایل `src/App.test.jsx`، تست‌های مربوط به Google Drive Sync از `vi.restoreAllMocks()` در `beforeEach` استفاده می‌کنند تا اطمینان حاصل شود که mockها بین تست‌ها پاک می‌شوند. اما تست‌های Firebase Auth که در ابتدای فایل تعریف شده‌اند، چنین مکانیزمی برای بازگرداندن mockها ندارند. این عدم بازگرداندن mockها می‌تواند منجر به تداخل حالت بین تست‌ها و ایجاد تست‌های ناپایدار (flaky tests) شود، به خصوص اگر mockهای Firebase در طول یک تست تغییر کنند و بر تست‌های بعدی تأثیر بگذارند.
...
- `src/App.test.jsx:145-170` — `describe('Firebase auth (firebase/auth mocked)')`
  ```jsx
...
Vitest, React Testing Library, Firebase Auth.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/App.jsx` — کامپوننت اصلی برنامه که تعاملات Firebase آن mock و تست می‌شوند.
- `src/lib/firebase.js` — حاوی منطق واقعی Firebase که mock می‌شود.
- `src/lib/arabic.js` — `App.test.jsx` این فایل را import می‌کند
- `src/lib/format.js` — `App.test.jsx` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این تغییر فقط بر مجموعه تست تأثیر می‌گذارد و پایداری تست را بهبود می‌بخشد. هیچ تأثیر مستقیمی بر زمان اجرای برنامه ندارد.
...
- [ ] تمام تست‌ها در `src/App.test.jsx` به طور پایدار و بدون تداخل حالت بین تست‌ها اجرا می‌شوند.
- [ ] بلوک `describe('Firebase auth (firebase/auth mocked)')` شامل یک هوک `beforeEach` است که `vi.restoreAllMocks()` را فراخوانی می‌کند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک بلوک `beforeEach` به `describe('Firebase auth (firebase/auth mocked)', ...)` اضافه کنید تا `vi.restoreAllMocks()` را قبل از هر تست در این مجموعه اجرا کند و از ایزوله‌سازی مناسب تست‌ها اطمینان حاصل شود.
...
هیچ، این تغییر پایداری تست را بهبود می‌بخشد.
...
1. تمام تست‌ها در `src/App.test.jsx` به طور پایدار و بدون تداخل حالت بین تست‌ها اجرا می‌شوند. _(verify: backend_test)_
2. بلوک `describe('Firebase auth (firebase/auth mocked)')` شامل یک هوک `beforeEach` است که `vi.restoreAllMocks()` را فراخوانی می‌کند. _(verify: static)_
