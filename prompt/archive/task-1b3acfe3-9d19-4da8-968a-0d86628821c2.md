---
task_id: 1b3acfe3-9d19-4da8-968a-0d86628821c2
title: 'Resolve Anti-pattern: Under-engineering'
type: bug_fix
priority: high
...
created_at: '2026-06-05T18:43:58.234358+00:00'
...
# Resolve Anti-pattern: Under-engineering
...
---
[scan #2 at 2026-06-05T18:46:34.871386+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/TelegramSettings.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `{ patch({ primaryChatId: d.id }); setDetected(null); ok(`Chat ID = ${d.
---
[scan #3 at 2026-06-05T18:46:34.879526+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/TelegramSettings.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `removeDevice(d.id)}>حذف`
- فایل: `src/components/TelegramSettings.jsx`

---
[scan #4 at 2026-06-05T18:46:34.888401+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/TelegramSettings.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `removeReminder(r.id)}>حذف`
- فایل: `src/components/TelegramSettings.jsx
...
3. تست edge case نوشته شد _(verify: manual_only)_
