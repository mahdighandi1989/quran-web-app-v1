---
task_id: task_8f403a6c6e4a
title: بهینه‌سازی ابزارهای ساخت و مدیریت وابستگی‌ها
type: other
priority: medium
...
updated_at: '2026-06-05T04:55:12.666838+00:00'
archived: true
archived_at: '2026-06-05T04:54:55.379863+00:00'
tags:
- consolidated
- post_verify_merge
---

# بهینه‌سازی ابزارهای ساخت و مدیریت وابستگی‌ها

## Raw Idea

🧬 این یک تسک تلفیقی است — از 2 تسک منفرد ساخته شده.
📌 دلیل تلفیق (rationale توسط AI): این تسک‌ها بر بهبود فرآیند ساخت پروژه از طریق پیکربندی Vite و همچنین بازآرایی کد برای رفع پیچیدگی‌های غیرضروری و بهبود کیفیت کلی کد تمرکز دارند.
🎯 theme: بهینه‌سازی ابزارهای ساخت و بازآرایی عمومی کد
💎 estimated_difficulty: medium

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 1 از 2
  id: 5de6420d-461a-4905-a3b9-de919b87de97
  عنوان اصلی: ایجاد فایل vite.config.js برای پیکربندی Vite
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: vite.config.js

📋 acceptance_criteria کامل:
  - فایل vite.config.js در ریشه پروژه ایجاد شود [verify_method=static] [verify_plan={"grep_patterns": ["// vite.config.js"], "files_hint": ["vite.config.js"]}]
  - برنامه با npm run dev و npm run build بدون خطا کار کند [verify_method=backend_test] [verify_plan={"test_node": "tests/system/test_vite_config.py::test_npm_commands_run_without_errors", "timeout_seconds": 120}]
...
عدم وجود فایل vite.config.js برای پیکربندی Vite

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `vite.config.js:1-15` — `N/A` — فایل جدید باید ایجاد شود
  ```jsx
...
- `package.json` (سطر 16) — برای اطمینان از وجود @vitejs/plugin-react در dependencies
- `index.html` (سطر 1) — ورودی اصلی که توسط Vite پردازش می‌شود

## 🌐 نقشهٔ وابستگی‌ها
این تغییر نیاز به نصب @vitejs/plugin-react دارد (اگر نصب نیست).

## 🔍 Context و وضعیت فعلی
پروژه از Vite به عنوان build tool استفاده می‌کند (بر اساس package.json و index.html که مستقیماً به src/main.jsx اشاره دارد)، اما هیچ فایل vite.config.js در ریشه پروژه وجود ندارد. اگرچه Vite بدون پیکربندی کار می‌کند، اما برای تنظیمات پیشرفته (مانند resolve aliases, proxy, plugins) نیاز به این فایل است. عدم وجود آن می‌تواند در آینده باعث مشکلاتی در build یا development شود.

## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
verify می‌تواند پیاده‌سازی متفاوت ولی هم‌ارز را قبول کند.

- [ ] فایل vite.config.js در ریشه پروژه ایجاد شود
- [ ] برنامه با npm run dev و npm run build بدون خطا کار کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل vite.config.js در ریشه پروژه ایجاد کنید. حداقل پیکربندی شامل react plugin (اگر از @vitejs/plugin-react استفاده می‌شود) و تنظیمات base path باشد. همچنین می‌توانید resolve alias برای مسیرهای src اضافه کنید.
...
اگر @vitejs/plugin-react نصب نباشد، باید نصب شود. در غیر این صورت build با خطا مواجه می‌شود.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 2
  id: 5e99a172-2dd9-406c-b786-2626e943ea60
  عنوان اصلی: Refactor over-engineered code
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: package-lock.json
...
📋 acceptance_criteria کامل:
  ...
  - یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["This package-lock.json verbosity is expected due to lockfileVersion: 3", "npm lockfileVersion 3 includes all optional dependencies for cross-platform reproducibility"], "files_hint]
  - تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_anti_pattern_edge_case.py::test_oversized_lockfile_handling", "timeout_seconds": 60}]
...
## 🔍 Context و وضعیت فعلی
The `package-lock.json` file includes an exhaustive list of all platform-specific optional dependencies for packages like `esbuild` (e.g., `@esbuild/aix-ppc64`, `@esbuild/android-arm`, etc.). While this is the standard behavior for `lockfileVersion: 3` and `npm` to ensure maximum cross-platform reproducibility of the lock file itself, it significantly increases the file's size and verbosity. This 

📁 file: package-lock.json (line 15)
...
## 🧪 دستورات اعتبارسنجی
- `python -c "import json; json.load(open('package-lock.json'))"`
...
- در commit message: `merged-from: 5de6420d-461a-4905-a3b9-de919b87de97, 5e99a172-2dd9-406c-b786-2626e943ea60`
...
1. فایل vite.config.js در ریشه پروژه ایجاد شود _(verify: static)_
2. برنامه با npm run dev و npm run build بدون خطا کار کند _(verify: backend_test)_
...
### Step 1: ایجاد فایل vite.config.js در ریشه پروژه با پیکربندی پایه Vite + React
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد فایل vite.config.js در ریشه پروژه با پیکربندی پایه شامل پلاگین React و resolve alias برای '@' به '/src' است. خارج از این مرحله: نصب وابستگی‌ها، تست، یا تغییر فایل‌های دیگر. نکته حیاتی: فایل باید دقیقاً در ریشه پروژه ایجاد شود و محتوای آن مطابق نمونه ارائه‌شده در پرامپت باشد.
**Excerpt:**
```
فایل vite.config.js در ریشه پروژه ایجاد شود [verify_method=static] [verify_plan={"grep_patterns": ["// vite.config.js"], "files_hint": ["vite.config.js"]}] ... import { defineConfig } from 'vite'; import react from '@vitejs/plugin-react'; export default defineConfig({ plugins: [react()], resolve: { alias: { '@': '/src', }, }, });
```

### Step 2: نصب @vitejs/plugin-react در صورت عدم وجود در dependencies
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی وجود @vitejs/plugin-react در package.json و نصب آن در صورت عدم وجود است. خارج از این مرحله: ایجاد فایل vite.config.js یا تست. نکته حیاتی: اگر وابستگی از قبل وجود دارد، مرحله را رد کن.
**Excerpt:**
```
اگر @vitejs/plugin-react نصب نباشد، باید نصب شود. در غیر این صورت build با خطا مواجه می‌شود. ... package.json (سطر 16) — برای اطمینان از وجود @vitejs/plugin-react در dependencies
```

### Step 3: اجرای npm run dev و npm run build برای اعتبارسنجی پیکربندی Vite
**Status:** `done` (100%)
**Scope:** این مرحله شامل اجرای دستورات npm run dev و npm run build برای اطمینان از کارکرد صحیح پیکربندی Vite است. خارج از این مرحله: تست‌های واحد یا edge case. نکته حیاتی: هر دو دستور باید بدون خطا اجرا شوند.
**Excerpt:**
```
برنامه با npm run dev و npm run build بدون خطا کار کند [verify_method=backend_test] [verify_plan={"test_node": "tests/system/test_vite_config.py::test_npm_commands_run_without_errors", "timeout_seconds": 120}] ... npm run dev ... npm run build
```

### Step 4: تشخیص ریشه anti-pattern در package-lock.json (over-engineering)
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی package-lock.json برای تشخیص anti-pattern مربوط به over-engineering (لیست کامل وابستگی‌های اختیاری پلتفرم-خاص) است. خارج از این مرحله: اصلاح کد یا نوشتن تست. نکته حیاتی: این مرحله صرفاً تشخیص است و نیاز به بازبینی دستی دارد.
**Excerpt:**
```
ریشه anti-pattern تشخیص داده شد [verify_method=manual_only] [verify_plan={"reason": "نیاز به بازبینی دستی", "grep_patterns": [], "files_hint": []}] ... Anti-pattern: Over-engineering ... The `package-lock.json` file includes an exhaustive list of all platform-specific optional dependencies for packages like `esbuild` (e.g., `@esbuild/aix-ppc64`, `@esbuild/android-arm`, etc.).
```

### Step 5: اضافه کردن کامنت توجیهی در package-lock.json یا فایل مرتبط
**Status:** `done` (100%)
**Scope:** این مرحله شامل اضافه کردن کامنت توجیهی در package-lock.json یا فایل مرتبط (مانند README) برای توضیح دلیل verbosity بالای فایل است. خارج از این مرحله: اصلاح کد یا نوشتن تست. نکته حیاتی: کامنت باید حاوی یکی از الگوهای مشخص‌شده در AC باشد.
**Excerpt:**
```
یا کد اصلاح شد، یا کامنت توجیهی اضافه شد [verify_method=static] [verify_plan={"grep_patterns": ["This package-lock.json verbosity is expected due to lockfileVersion: 3", "npm lockfileVersion 3 includes all optional dependencies for cross-platform reproducibility"], "files_hint": []}]
```

### Step 6: نوشتن تست edge case برای handling oversized lockfile
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن تست edge case در فایل tests/test_anti_pattern_edge_case.py برای بررسی handling فایل‌های lockfile بزرگ است. خارج از این مرحله: اصلاح کد یا تشخیص anti-pattern. نکته حیاتی: تست باید در فایل مشخص‌شده و با نام تابع مشخص‌شده نوشته شود.
**Excerpt:**
```
تست edge case نوشته شد [verify_method=backend_test] [verify_plan={"test_node": "tests/test_anti_pattern_edge_case.py::test_oversized_lockfile_handling", "timeout_seconds": 60}]
```

### Step 7: بررسی و همگام‌سازی وابستگی‌های upstream برای تسک 1 (ایجاد vite.config.js)
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی وابستگی‌های upstream برای تسک 1 است: فایل‌ها، توابع، API endpoint ها، schema های دیتابیس، env vars، یا config هایی که این تسک نیاز دارد. خارج از این مرحله: پیاده‌سازی کد. نکته حیاتی: اگر dependency جدیدی اضافه می‌شود، تداخل با dependencyهای موجود بررسی شود.
— [merged] این مرحله شامل بررسی وابستگی‌های downstream برای تسک 1 است: فایل‌ها، توابع، تست‌ها، migrations، docs، یا UI component هایی که از کد جدید استفاده می‌کنند. خارج از این مرحله: پیاده‌سازی کد. نکته حیاتی: با grep و reference search همه call sites و importها پیدا شوند.
— [merged] این مرحله شامل بررسی وابستگی‌های cross-tier برای تسک 1 است: backend، frontend، database، worker، و سایر tierها. خارج از این مرحله: پیاده‌سازی کد. نکته حیاتی: مستقل از اینکه تسک از کدام tier است، چک‌های دو طرفه انجام شود.
— [merged] این مرحله شامل بررسی و همگام‌سازی وابستگی‌های جانبی برای تسک 1 است: مستندات، تست‌ها، type definitions، infrastructure، monitoring، security، caches. خارج از این مرحله: پیاده‌سازی کد. نکته حیاتی: همه artifact ها بررسی و به‌روز شوند.
**Excerpt:**
```
وابستگی‌های upstream (این تسک به چه چیزهایی متکی است): - چه فایل‌ها، توابع، کلاس‌ها، API endpoint ها، schema های دیتابیس، env vars، یا config هایی که این تسک نیاز دارد؟ - آیا قرار است چیزی را ویرایش/حذف کنی که جای دیگر (signature، رفتار، return type، side effect) از آن انتظار خاصی می‌رود؟ - اگر dependency جدیدی اضافه می‌کنی، آیا با dependencyهای موجود تداخل دارد (نسخه، compat، lock file)؟
```

### Step 8: بررسی و همگام‌سازی وابستگی‌ها برای تسک 2 (Refactor over-engineered code)
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی و همگام‌سازی همه وابستگی‌ها (upstream, downstream, cross-tier, side) برای تسک 2 است. خارج از این مرحله: پیاده‌سازی کد. نکته حیاتی: همه چهار جهت وابستگی بررسی شوند.
**Excerpt:**
```
پیش از و حین تغییر، تمام وابستگی‌ها را در چهار جهت به‌طور کامل و بدون هیچ خلاصه‌سازی شناسایی و همگام کن: ۱. وابستگی‌های upstream ۲. وابستگی‌های downstream ۳. وابستگی‌های cross-tier ۴. وابستگی‌های جانبی
```
