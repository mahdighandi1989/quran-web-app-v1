...
updated_at: '2026-06-05T05:32:55.636350+00:00'
archived: true
archived_at: '2026-06-05T05:32:55.636326+00:00'
tags:
- consolidated
- post_verify_merge
---
...
پروژه فاقد هرگونه فایل تست و پیکربندی تست است

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `package.json:1-17` — `scripts و dependencies` — فقدان وابستگی‌های تست و اسکریپت test
  ```json
  {
    "name": "quran-web-app",
    "private": true,
...
- `src/App.jsx` (سطر 1) — فایل اصلی که باید تست شود
- `src/main.jsx` (سطر 1) — فایل ورودی که باید تست شود

## 🌐 نقشهٔ وابستگی‌ها
نیاز به نصب vitest, @testing-library/react, jsdom, و سایر وابستگی‌های تست

## 🔍 Context و وضعیت فعلی
بررسی کامل ساختار پروژه نشان می‌دهد که هیچ فایل تستی (با الگوهای *.test.*, *.spec.*, __tests__/, tests/) وجود ندارد. همچنین فایل پیکربندی تست (vitest.config.js یا jest.config.js) در پروژه وجود ندارد. با وجود اینکه پروژه از Vite استفاده می‌کند و به راحتی می‌تواند از Vitest استفاده کند، هیچ زیرساخت تستی راه‌اندازی نشده است. این یک شکاف اساسی در کیفیت کد است.
...
1. راه‌اندازی Vitest به عنوان فریمورک تست با نصب وابستگی‌های لازم و ایجاد فایل پیکربندی. سپس ایجاد تست‌های پایه برای فایل‌های اصلی.

## 💡 نمونه‌های قبل/بعد
**اضافه کردن Vitest به package.json**
...
نیاز به نصب وابستگی‌های جدید که ممکن است با نسخه‌های فعلی Vite ناسازگار باشد
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 2
  id: a3e9a63e-155a-42ee-8566-56f81c55702e
  عنوان اصلی: ایجاد تست رندر برای main.jsx
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: src/main.jsx

📋 acceptance_criteria کامل:
  - فایل src/main.test.jsx ایجاد شود [verify_method=static] [verify_plan={"grep_patterns": ["src/main.test.jsx"], "files_hint": ["src/main.test.jsx"]}]
  - تست بررسی کند که رندر App بدون خطا انجام می‌شود [verify_method=backend_test] [verify_plan={"test_node": "src/main.test.jsx::test_app_renders_without_crashing", "timeout_seconds": 30}]
  - تست با npm test عبور کند [verify_method=backend_test] [verify_plan={"test_node": "src/main.test.jsx", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
فایل main.jsx فاقد تست است و نقطه ورود برنامه است

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/main.jsx:1-10` — `کل فایل` — نقطه ورود برنامه بدون تست
  ```jsx
...
- `src/App.jsx` (سطر 1) — کامپوننتی که رندر می‌شود
- `index.html` (سطر 1) — فایل HTML که حاوی div#root است

## 🌐 نقشهٔ وابستگی‌ها
وابسته به react-dom و App.jsx

## 🔍 Context و وضعیت فعلی
فایل src/main.jsx نقطه ورود برنامه React است که App.jsx را رندر می‌کند. اگرچه منطق کمی دارد، اما هرگونه خطا در رندر اولیه می‌تواند کل برنامه را از کار بیندازد. هیچ فایل تستی برای آن وجود ندارد.
...
## 🪜 مراحل اجرایی پیشنهادی
1. ایجاد فایل تست src/main.test.jsx برای بررسی رندر صحیح کامپوننت App و عدم وجود خطا در mount شدن.
...
## 🧪 دستورات اعتبارسنجی
- `npx vitest run src/main.test.jsx`
- `npm test`

## ⚠️ ریسک‌ها و موارد احتیاط
نیاز به mock کردن Firebase و سایر وابستگی‌های App.jsx
...
- در commit message: `merged-from: 6eba8818-3989-4526-9d4c-d8277cbe68e6, a3e9a63e-155a-42ee-8566-56f81c55702e`
...
1. vitest و وابستگی‌های لازم در package.json اضافه شوند _(verify: static)_
2. فایل vitest.config.js ایجاد شود _(verify: static)_
3. اسکریپت npm test کار کند _(verify: static)_
4. حداقل یک تست ساده (مثلاً 1+1=2) با npm test عبور کند _(verify: backend_test)_
5. فایل src/test/setup.js برای تنظیمات global ایجاد شود _(verify: static)_
6. فایل src/main.test.jsx ایجاد شود _(verify: static)_
7. تست بررسی کند که رندر App بدون خطا انجام می‌شود _(verify: backend_test)_
8. تست با npm test عبور کند _(verify: backend_test)_
