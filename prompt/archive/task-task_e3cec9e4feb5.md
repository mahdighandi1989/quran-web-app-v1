...
title: تقویت امنیت و پیکربندی پایه پروژه
type: other
priority: critical
...
created_at: '2026-06-02T10:59:34.423376+00:00'
updated_at: '2026-06-05T05:17:36.494808+00:00'
archived: true
archived_at: '2026-06-05T05:17:36.494772+00:00'
tags:
- consolidated
- post_verify_merge
---
...
عدم اعتبارسنجی ورودی در تابع handleInspectorCommand (XSS و Command Injection)

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `index.html:165-186` — `handleInspectorCommand` — ورودی بدون sanitization
  ```
...
- `index.html` (سطر 165) — همان فایل

## 🌐 نقشهٔ وابستگی‌ها
این تغییر فقط روی اسکریپت bridge تأثیر می‌گذارد.

## 🔍 Context و وضعیت فعلی
تابع `handleInspectorCommand` در `index.html` ورودی‌های `msg.selector` و `msg.url` را بدون هیچ sanitization یا validation به `document.querySelector` و `window.location.href` می‌دهد. این می‌تواند منجر به XSS (از طریق selectorهای مخرب) و Open Redirect (از طریق URLهای دلخواه) شود. همچنین `msg.url` می‌تواند حاوی `javascript:` protocol باشد که منجر به XSS می‌شود.
...
1. selector را با regex محدود به الگوهای CSS selector معتبر کنید. URL را با allowlist محدود کنید و از `javascript:` و `data:` protocols جلوگیری کنید. از `URL` constructor برای validation استفاده کنید.

## 💡 نمونه‌های قبل/بعد
**sanitize selector و URL**
...
_بعد:_
```
var sanitizedSelector = msg.selector.replace(/[^\w\s#\.\-\[\]=:]/g, '');
var el = document.querySelector(sanitizedSelector);
...
## 🧪 دستورات اعتبارسنجی
- `grep -E "document\.querySelector\(msg\.selector\)" index.html`
- `grep -E "window\.location\.href = msg\.url" index.html`

## ⚠️ ریسک‌ها و موارد احتیاط
کم - فقط نیاز به اضافه کردن validation دارد
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 2 از 4
  id: ca2ce8f6-5c53-4bd5-8492-59dce6bfd69b
  عنوان اصلی: افزودن اطلاعات مجوز به پروژه
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: LICENSE

📋 acceptance_criteria کامل:
  - فایل LICENSE در ریشه پروژه وجود داشته باشد [verify_method=static] [verify_plan={"grep_patterns": ["LICENSE"], "files_hint": ["LICENSE"]}]
  - مجوز در README.md ذکر شده باشد [verify_method=static] [verify_plan={"grep_patterns": ["License", "MIT License", "Apache-2.0 License"], "files_hint": ["README.md"]}]
  - package.json شامل فیلد license باشد [verify_method=static] [verify_plan={"grep_patterns": ["\"license\":"], "files_hint": ["package.json"]}]
...
## 🎯 هدف (خلاصه ساختاریافته)
عدم وجود فایل LICENSE در پروژه

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `LICENSE:1-21` — `N/A` — فایل مجوز باید در ریشه پروژه ایجاد شود
  ```
...
- `README.md` (سطر 1) — باید مجوز پروژه را ذکر کند
- `package.json` (سطر 3) — می‌توان فیلد license را اضافه کرد

## 🌐 نقشهٔ وابستگی‌ها
این تغییر فقط روی metadata پروژه تأثیر می‌گذارد.

## 🔍 Context و وضعیت فعلی
پروژه فایل LICENSE ندارد. با توجه به اینکه از dependencies با مجوزهای مختلف (MIT برای React و Vite، Apache-2.0 برای Firebase) استفاده می‌کند، عدم وجود مجوز مشخص برای پروژه می‌تواند مسائل قانونی ایجاد کند. همچنین اگر پروژه قرار است به صورت commercial استفاده شود، باید مجوز مناسب انتخاب شود.
...
## 🪜 مراحل اجرایی پیشنهادی
1. یک فایل LICENSE با مجوز مناسب (مثلاً MIT برای open source یا Apache-2.0 برای compatibility با Firebase) ایجاد کنید. مجوز را در README.md نیز ذکر کنید.
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 3 از 4
  id: 39692dce-68f1-435f-ba78-c2e24200ff28
  عنوان اصلی: پیکربندی .gitignore برای نادیده گرفتن فایل‌های حساس
  اولویت اصلی: high
  وضعیت verify قبلی: partial
  فایل‌های دخیل: .gitignore

📋 acceptance_criteria کامل:
  - الگوهای امنیتی به .gitignore اضافه شوند [verify_method=static] [verify_plan={"grep_patterns": [".env", ".env.local", "credentials.json", "service-account*.json", "*.key", "*.pem"], "files_hint": [".gitignore"]}]
  - فایل‌های env و credentials از git ignore شوند [verify_method=backend_test] [verify_plan={"test_node": "tests/test_git_ignore.py::test_sensitive_files_are_ignored", "timeout_seconds": 30}]
...
## 🎯 هدف (خلاصه ساختاریافته)
عدم وجود فایل .gitignore برای فایل‌های حساس

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `.gitignore:1-10` — `N/A` — الگوهای امنیتی باید اضافه شوند
  ```
  # فایل .gitignore فعلی
  # (محتوای فعلی دیده نشد، اما الگوهای امنیتی وجود ندارد)
  ```
...
- `.env.example` (سطر 1) — فایل نمونه برای env variables

## 🌐 نقشهٔ وابستگی‌ها
این تغییر روی کل پروژه تأثیر می‌گذارد.

## 🔍 Context و وضعیت فعلی
فایل `.gitignore` وجود دارد اما شامل الگوهای رایج برای فایل‌های حساس مانند `.env`، `.env.local`، `credentials.json`، `service-account*.json`، `*.key`، `*.pem` نیست. این می‌تواند منجر به commit تصادفی credentials و secrets در مخزن git شود.
...
## 🪜 مراحل اجرایی پیشنهادی
1. الگوهای زیر را به `.gitignore` اضافه کنید: `.env`, `.env.local`, `.env.*.local`, `credentials.json`, `service-account*.json`, `*.key`, `*.pem`, `secrets.yml`, `htpasswd`

## 💡 نمونه‌های قبل/بعد
**اضافه کردن الگوهای امنیتی**
...
## 🧪 دستورات اعتبارسنجی
- `grep -E "^\.env|credentials|service-account|\\.key|\\.pem" .gitignore`
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
تسک 4 از 4
  id: 0740d77f-162f-4a75-a23b-bd9482f31b0e
  عنوان اصلی: ایجاد .gitignore مناسب برای React + Vite
  اولویت اصلی: medium
  وضعیت verify قبلی: partial
  فایل‌های دخیل: .gitignore

📋 acceptance_criteria کامل:
  - فایل .gitignore شامل node_modules, dist, .env باشد [verify_method=static] [verify_plan={"grep_patterns": ["node_modules", "dist", ".env"], "files_hint": [".gitignore"]}]
  - فایل‌های غیرضروری در git status نمایش داده نشوند [verify_method=backend_test] [verify_plan={"test_node": "tests/integration/test_git_ignore.py::test_unnecessary_files_ignored", "timeout_seconds": 60}]
...
## 🎯 هدف (خلاصه ساختاریافته)
عدم وجود فایل .gitignore مناسب برای پروژه React + Vite

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `.gitignore:1-20` — `N/A` — فایل موجود باید بازنویسی شود
  ```
  # Dependencies
  node_modules
...
## 🌐 نقشهٔ وابستگی‌ها
فقط فایل .gitignore تحت تأثیر قرار می‌گیرد.

## 🔍 Context و وضعیت فعلی
فایل .gitignore موجود (که در لیست فایل‌ها به عنوان 'other' ذکر شده) احتمالاً کامل نیست. برای پروژه React + Vite، باید شامل پوشه‌های node_modules, dist, .env, فایل‌های لاگ و ... باشد. عدم وجود .gitignore مناسب می‌تواند منجر به commit شدن فایل‌های غیرضروری (مانند node_modules) یا اطلاعات حساس (مانند .env) شود.
...
## 🪜 مراحل اجرایی پیشنهادی
1. فایل .gitignore را با محتوای استاندارد برای React + Vite به‌روز کنید. شامل node_modules, dist, .env, .env.local, *.log, .DS_Store و ...
...
_قبل:_
```
محتوای فعلی نامشخص (احتمالاً ناقص)
```
...
## 🧪 دستورات اعتبارسنجی
- `cat .gitignore`
- `git status (بعد از تغییر)`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر کم‌خطر. فقط باید مطمئن شوید فایل‌های مهم از قبل commit نشده باشند.
...
- در commit message: `merged-from: 2fdbbff9-ff50-450a-87bb-6361f098e2b7, ca2ce8f6-5c53-4bd5-8492-59dce6bfd69b, 39692dce-68f1-435f-ba78-c2e24200ff28, 0740d77f-162f-4a75-a23b-bd9482f31b0e`
...
1. selectorها محدود به کاراکترهای مجاز شوند _(verify: static)_
2. URLها فقط به https:// محدود شوند _(verify: ui_interaction)_
3. از javascript: protocol جلوگیری شود _(verify: ui_interaction)_
4. فایل LICENSE در ریشه پروژه وجود داشته باشد _(verify: static)_
5. مجوز در README.md ذکر شده باشد _(verify: static)_
6. package.json شامل فیلد license باشد _(verify: static)_
7. الگوهای امنیتی به .gitignore اضافه شوند _(verify: static)_
8. فایل‌های env و credentials از git ignore شوند _(verify: backend_test)_
9. فایل .gitignore شامل node_modules, dist, .env باشد _(verify: static)_
10. فایل‌های غیرضروری در git status نمایش داده نشوند _(verify: backend_test)_

## Task Steps

### Step 1: یادداشت مهم برای مدل اجراکننده — قبل از شروع بخوان
**Status:** `done` (100%)
**Scope:** این بخش یک یادداشت متا-پرامپت است که نحوه اجرای کل درخواست را تعیین می‌کند. شامل قوانین خواندن دقیق، ممنوعات، بررسی وابستگی‌های چهارجهته، مدیریت TO-DO، و قوانین commit/push است. این بخش خودش یک مرحله اجرایی نیست، بلکه دستورالعمل اجرای سایر مراحل است.
**Excerpt:**
```
## ⚠️ یادداشت مهم برای مدل اجراکننده — قبل از شروع بخوان
...
### Step 2: اعتبارسنجی و پاک‌سازی ورودی‌های تابع handleInspectorCommand برای جلوگیری از XSS و Command Injection
**Status:** `partial` (60%)
**Scope:** این مرحله شامل اصلاح تابع handleInspectorCommand در فایل index.html (سطرهای 165-186) است. ورودی‌های msg.selector و msg.url باید قبل از استفاده در document.querySelector و window.location.href پاک‌سازی و اعتبارسنجی شوند. محدوده تغییرات فقط به این تابع محدود می‌شود و شامل سایر بخش‌های پروژه نیست. نکته حیاتی: باید از اجرای کدهای مخرب از طریق selectorهای خاص (مانند تزریق HTML) و URLهای حاوی پروتکل javascript: جلوگیری شود.
**Excerpt:**
```
تابع `handleInspectorCommand` در `index.html` ورودی‌های `msg.selector` و `msg.url` را بدون هیچ sanitization یا validation به `document.querySelector` و `window.location.href` می‌دهد. این می‌تواند منجر به XSS (از طریق selectorهای مخرب) و Open Redirect (از طریق URLهای دلخواه) شود. همچنین `msg.url` می‌تواند حاوی `javascript:` protocol باشد که منجر به XSS می‌شود.
...
### Step 3: اعتبارسنجی و محدودسازی selectorها، URLها و پروتکل‌های ورودی
**Status:** `partial` (60%)
**Scope:** این بخش شامل پیاده‌سازی اعتبارسنجی برای selectorهای CSS (محدود به کاراکترهای مجاز)، URLها (محدود به پروتکل https و جلوگیری از javascript: و data:) و اطمینان از عبور تست‌ها، linter و type-check است. خارج از scope: پیاده‌سازی کامل امنیت پروژه، تغییرات در فایل‌های تست موجود (tests/test_git_ignore.py و tests/test_git_ignore.py) و هرگونه refactoring غیرمرتبط.
**Excerpt:**
```
## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
...
### Step 4: Sanitize selector و URL برای جلوگیری از حملات XSS و open redirect
**Status:** `partial` (60%)
**Scope:** این بخش شامل تغییر کد جاوااسکریپت برای sanitize کردن selector قبل از استفاده در querySelector و اعتبارسنجی URL با بررسی شروع با https:// قبل از تغییر location.href است. خارج از scope: تغییرات در فایل‌های tests/test_git_ignore.py یا tests/test_git_ignore.py (این فایل‌ها برای تست gitignore هستند و ربطی به sanitize ندارند). نکته حیاتی: فایل دقیق حاوی کد مشخص نشده، اما باید در فایل‌های جاوااسکریپت پروژه اعمال شود.
**Excerpt:**
```
## 💡 نمونه‌های قبل/بعد
**sanitize selector و URL**
...
### Step 5: افزودن فایل LICENSE و ارجاع به آن در README.md و package.json
**Status:** `not_done` (0%)
**Scope:** این بخش شامل ایجاد فایل LICENSE در ریشه پروژه، ذکر نوع مجوز در README.md و افزودن فیلد license به package.json است. خارج از این بخش: تغییر محتوای LICENSE، بررسی تطابق مجوز با سایر فایل‌ها، یا هرگونه تغییر در کد منبع. نکته حیاتی: acceptance_criteria صراحتاً وجود فایل LICENSE، ذکر مجوز در README.md و فیلد license در package.json را الزامی کرده است.
**Excerpt:**
```
تسک 2 از 4
  id: ca2ce8f6-5c53-4bd5-8492-59dce6bfd69b
...
### Step 6: تبدیل یادداشت‌های هشداردهنده و قواعد اجرایی به یک مرحله اجرایی واحد
**Status:** `done` (100%)
**Scope:** این بخش شامل تمام یادداشت‌های هشداردهنده (⚠️) و قواعد اجرایی (از جمله خواندن کامل، ممنوعیت خلاصه‌سازی، همگام‌سازی وابستگی‌ها، مدیریت TO-DO، و commit/push فوری) است. هدف این مرحله اطمینان از اجرای دقیق و کامل تمام دستورالعمل‌های این بخش در طول اجرای پروژه است. این بخش هیچ کد یا تغییری در repo ایجاد نمی‌کند، بلکه یک چارچوب اجرایی برای تمام مراحل بعدی تعریف می‌کند.
— [merged] این بخش شامل کل یادداشت مهم برای مدل اجراکننده است که قبل از شروع هر کاری باید خوانده شود. شامل قواعد سخت مانند خواندن سطر به سطر، ممنوعیت خلاصه‌سازی، بررسی وابستگی‌های چهارجهته (upstream, downstream, cross-tier, side artifacts)، مدیریت TO-DO با قوانین سختگیرانه، و الزام commit+push فوری per-task. این بخش یک مرحله اجرایی واحد است که تمام این دستورالعمل‌ها را به عنوان یک precondition برای اجرای کل پروژه تعریف می‌کند. هیچ کد یا تغییری در این مرحله تولید نمی‌شود، بلکه چارچوب اجرای مراحل بعدی تعیین می‌گردد.
**Excerpt:**
```
## ⚠️ یادداشت مهم برای مدل اجراکننده — قبل از شروع بخوان
این پرامپت بر اساس یک **بررسی اولیهٔ خودکار** از repo ساخته شده — ممکن است حاوی اشتباه، تشخیص نادرست، یا حذف موارد مهم باشد. به‌عنوان منبع نهایی به آن استناد نکن.
...
⛔ **ممنوعات مطلق TO-DO**:
  ❌ ساختن TO-DO برای کاری که می‌توانستی خودت انجام دهی (شلوغی فولدر)
  ❌ overwrite کردن `TO-DO/_in
```

### Step 7: ایجاد فایل LICENSE در ریشه پروژه با مجوز مناسب
**Status:** `not_done` (0%)
**Scope:** این بخش شامل ایجاد فایل LICENSE در ریشه پروژه است. مجوز باید با توجه به استفاده از dependencies با مجوزهای MIT (React, Vite) و Apache-2.0 (Firebase) و همچنین هدف تجاری احتمالی پروژه انتخاب شود. همچنین باید فیلد license در package.json به‌روزرسانی شود و README.md به مجوز پروژه اشاره کند. این بخش شامل انتخاب نوع مجوز نمی‌شود (کاربر باید تصمیم بگیرد)، بلکه فقط ایجاد فایل و به‌روزرسانی metadata است.
**Excerpt:**
```
## 🎯 هدف (خلاصه ساختاریافته)
عدم وجود فایل LICENSE در پروژه
...
### Step 8: افزودن فایل LICENSE و ذکر مجوز در README.md و package.json
**Status:** `not_done` (0%)
**Scope:** این بخش شامل ایجاد فایل LICENSE با مجوز مناسب (مثلاً MIT یا Apache-2.0)، ذکر مجوز در README.md، و افزودن فیلد license به package.json است. خارج از scope: تغییر محتوای README فراتر از ذکر مجوز، تغییر سایر فیلدهای package.json، اجرای تست‌ها یا linter. نکته حیاتی: مجوز باید با نیازهای پروژه (مثلاً compatibility با Firebase برای Apache-2.0) هماهنگ باشد.
**Excerpt:**
```
## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
...
### Step 9: اضافه کردن فایل LICENSE با مجوز MIT یا Apache-2.0
**Status:** `not_done` (0%)
**Scope:** این بخش شامل ایجاد فایل LICENSE در ریشه پروژه با متن یکی از مجوزهای استاندارد MIT یا Apache-2.0 است. خارج از scope: تغییر هیچ فایل کد، commit یا PR (این بخش صرفاً ایجاد فایل LICENSE را پوشش می‌دهد). نکته حیاتی: انتخاب بین MIT و Apache-2.0 باید بر اساس نیاز پروژه انجام شود.
**Excerpt:**
```
## 💡 نمونه‌های قبل/بعد
**اضافه کردن LICENSE**
...
### Step 10: پیکربندی .gitignore برای نادیده گرفتن فایل‌های حساس
**Status:** `partial` (50%)
**Scope:** این بخش شامل افزودن الگوهای امنیتی به فایل .gitignore برای جلوگیری از commit فایل‌های حساس مانند .env، credentials.json و کلیدهای خصوصی است. همچنین شامل اجرای تست backend برای اطمینان از نادیده گرفته شدن این فایل‌ها می‌شود. خارج از scope: تغییرات در فایل‌های دیگر، پیکربندی CI/CD، یا اصلاح فایل‌های env موجود.
**Excerpt:**
```
تسک 3 از 4
  id: 39692dce-68f1-435f-ba78-c2e24200ff28
...
### Step 11: افزودن الگوهای امنیتی به فایل .gitignore برای جلوگیری از commit فایل‌های حساس
**Status:** `partial` (50%)
**Scope:** این مرحله شامل افزودن الگوهای رایج برای فایل‌های حساس (مانند .env، .env.local، credentials.json، service-account*.json، *.key، *.pem) به فایل .gitignore موجود در ریشه پروژه است. خارج از این مرحله: ایجاد فایل .gitignore از صفر، تغییر ساختار پروژه، یا افزودن الگوهای غیرمرتبط با امنیت. نکته حیاتی: الگوها باید با دقت به انتهای فایل فعلی اضافه شوند تا الگوهای موجود را بازنویسی نکنند.
**Excerpt:**
```
عدم وجود فایل .gitignore برای فایل‌های حساس
...
### Step 12: افزودن الگوهای امنیتی به .gitignore و اطمینان از عبور تست‌ها، linter و type-check
**Status:** `partial` (50%)
**Scope:** این بخش شامل افزودن الگوهای امنیتی مشخص (مانند .env، credentials.json، فایل‌های کلید و گواهی) به فایل .gitignore است. همچنین باید اطمینان حاصل شود که پس از این تغییرات، تست‌ها (npm run test / pytest) fail نشوند، linter بدون warning عبور کند و type-check (tsc --noEmit / mypy) موفق باشد. خارج از scope این بخش، تغییر در منطق برنامه، اضافه کردن فایل‌های جدید غیر از .gitignore، یا پیکربندی ابزارهای CI/CD است.
**Excerpt:**
```
## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
...
### Step 13: به‌روزرسانی .gitignore با الگوهای امنیتی
**Status:** `partial` (50%)
**Scope:** این بخش شامل به‌روزرسانی فایل .gitignore برای افزودن الگوهای امنیتی مانند فایل‌های .env، credentials.json، service-account*.json، کلیدها و فایل‌های secrets است. خارج از این بخش: تغییرات در کد برنامه، تست‌ها، یا هر فایل دیگری غیر از .gitignore. نکته حیاتی: فایل‌های .env.*.local و htpasswd نیز باید اضافه شوند.
**Excerpt:**
```
**اضافه کردن الگوهای امنیتی**
...
### Step 14: ایجاد .gitignore مناسب برای React + Vite
**Status:** `partial` (50%)
**Scope:** این مرحله شامل ایجاد یا اصلاح فایل .gitignore در ریشه پروژه است به گونه‌ای که حداقل شامل node_modules، dist و .env باشد. همچنین باید اطمینان حاصل شود که فایل‌های غیرضروری در خروجی git status نمایش داده نشوند. این مرحله صرفاً بر روی فایل .gitignore تمرکز دارد و شامل تغییرات دیگر در پیکربندی پروژه نمی‌شود.
**Excerpt:**
```
تسک 4 از 4
  id: 0740d77f-162f-4a75-a23b-bd9482f31b0e
...
📋 acceptance_criteria کامل:
  - فایل .gitignore شامل node_modules, dist, .env باشد [verify_method=static] [verify_plan={"grep_patterns": ["node_modules", "dist", ".env"], "files_hint": [".gitignore"]}]
  - فایل‌های غیرضروری در git status نمایش داده نشوند [verify_method=backend_test] [verify_plan={"test_node": "tests/test_git_ignore.py::test_unnecessary_files_ignored", "timeout_seconds": 60}]
```

### Step 15: تبدیل یادداشت مهم به مرحله اجرایی — بررسی و همگام‌سازی وابستگی‌ها
**Status:** `done` (100%)
**Scope:** این بخش یک یادداشت متا-پرامپتی است که نحوه اجرای کل درخواست را تعیین می‌کند. شامل قوانین خواندن دقیق، ممنوعیت خلاصه‌سازی، بررسی وجود پیاده‌سازی قبلی، همگام‌سازی وابستگی‌ها در چهار جهت (upstream, downstream, cross-tier, side artifacts)، مدیریت TO-DO با آستانه ۲۰٪، و الزام commit+push فوری per-task است. این یادداشت خودش یک مرحله اجرایی نیست بلکه چارچوب اجرای تمام مراحل بعدی را تعریف می‌کند.
**Excerpt:**
```
این پرامپت بر اساس یک **بررسی اولیهٔ خودکار** از repo ساخته شده — ممکن است حاوی اشتباه، تشخیص نادرست، یا حذف موارد مهم باشد. به‌عنوان منبع نهایی به آن استناد نکن.
...
⛔ **ممنوعات مطلق TO-DO**:
  ❌ ساختن TO-DO برای کاری که می‌توانستی خودت انجام دهی (شلوغی فولدر)
  ❌ overwrite کردن `TO-DO/_index.json` ب
```

### Step 16: بازنویسی فایل .gitignore برای پروژه React + Vite
**Status:** `partial` (40%)
**Scope:** این مرحله شامل بازنویسی کامل فایل .gitignore در ریشه پروژه است. محتوای جدید باید شامل پوشه‌های node_modules, dist, build, فایل‌های محیطی (.env, .env.local, .env.*.local), فایل‌های لاگ (*.log, npm-debug.log*), فایل‌های سیستم‌عامل (.DS_Store, Thumbs.db) و فایل‌های IDE (.vscode/, .idea/, *.swp, *.swo) باشد. خارج از scope: تغییر سایر فایل‌ها، تغییر ساختار پروژه، یا اعمال تنظیمات اضافی امنیتی.
**Excerpt:**
```
## 🎯 هدف (خلاصه ساختاریافته)
عدم وجود فایل .gitignore مناسب برای پروژه React + Vite
...
### Step 17: به‌روزرسانی فایل .gitignore با محتوای استاندارد برای React + Vite
**Status:** `partial` (50%)
**Scope:** این بخش شامل به‌روزرسانی فایل .gitignore پروژه با محتوای استاندارد برای React + Vite است. مواردی مانند node_modules, dist, .env, .env.local, *.log, .DS_Store و سایر فایل‌های غیرضروری باید در این فایل گنجانده شوند. هدف این است که فایل‌های غیرضروری در git status نمایش داده نشوند. تست‌های مربوطه در tests/test_git_ignore.py و tests/test_git_ignore.py قرار دارند.
**Excerpt:**
```
## ✅ معیار پذیرش (Acceptance Criteria) — رفتار-محور
**مهم:** هر AC رفتار قابل مشاهده را تعریف می‌کند، نه نام فایل/کلاس.
...
### Step 18: به‌روزرسانی فایل .gitignore برای پروژه
**Status:** `partial` (50%)
**Scope:** این بخش شامل به‌روزرسانی محتوای فایل .gitignore در ریشه پروژه است. محتوای فعلی (که احتمالاً ناقص است) باید با محتوای مشخص‌شده شامل node_modules, dist, .env, *.log, .DS_Store جایگزین شود. این بخش شامل ایجاد یا اصلاح فایل‌های تست (tests/test_git_ignore.py و tests/test_git_ignore.py) نیست و صرفاً بر تغییر فایل .gitignore تمرکز دارد.
**Excerpt:**
```
## 💡 نمونه‌های قبل/بعد
**به‌روزرسانی .gitignore**
...
### Step 19: بررسی عدم commit فایل‌های مهم پیش از تغییر gitignore
**Status:** `done` (100%)
**Scope:** این بخش شامل بررسی و اطمینان از اینکه فایل‌های مهم پروژه (مانند فایل‌های کد، کانفیگ، و تست) قبلاً در git commit نشده‌اند، قبل از اعمال تغییرات در .gitignore است. این یک مرحله احتیاطی و پیش‌نیاز برای تغییرات بعدی است. هیچ تغییری در کد یا فایل‌ها انجام نمی‌شود، فقط یک بررسی وضعیت است.
**Excerpt:**
```
## ⚠️ ریسک‌ها و موارد احتیاط
تغییر کم‌خطر. فقط باید مطمئن شوید فایل‌های مهم از قبل commit نشده باشند.
