---
task_id: 9aa136b7-c5f1-4a19-bdb0-60bfb154e3b9
title: Develop Proactive Notification Triggering Logic
type: logic_audit
priority: critical
...
created_at: '2026-06-05T18:43:11.477966+00:00'
updated_at: '2026-06-06T05:28:53.781500+00:00'
archived: true
...
# Develop Proactive Notification Triggering Logic
...
### Step 1: ایجاد سرویس جدید notificationScheduler.js برای مانیتورینگ وضعیت اپلیکیشن
**Status:** `done` (100%)
**Scope:** این مرحله شامل ایجاد فایل جدید src/lib/notificationScheduler.js است که وظیفه مانیتورینگ تغییرات وضعیت اپلیکیشن (مانند تکمیل جلسه، پیشرفت روزانه، یادآوری‌ها) را بر عهده دارد. این سرویس باید منطق تصمیم‌گیری برای زمان ارسال نوتیفیکیشن را پیاده‌سازی کند. خارج از این مرحله: تغییر در فایل‌های موجود appStateStore.js یا telegramCommands.js انجام نمی‌شود. نکته حیاتی: این سرویس باید قابلیت خواندن داده از appStateStore.js را داشته باشد و بتواند notify_event را فراخوانی کند.
**Excerpt:**
```
Introduce a new service (e.g., `src/lib/notificationScheduler.js` or a server-side component) responsible for monitoring application state changes, evaluating notification conditions, and triggering `src/lib/telegram.js` to send messages. This service would embody the 'event → notify_event' logic.
```

### Step 2: پیاده‌سازی منطق تشخیص رویدادهای اپلیکیشن (session completion, daily progress, reminders)
**Status:** `done` (100%)
**Scope:** این مرحله شامل پیاده‌سازی توابعی در notificationScheduler.js است که رویدادهای مختلف اپلیکیشن مانند تکمیل جلسه، پیشرفت روزانه و یادآوری‌ها را تشخیص می‌دهند. این توابع باید داده‌های ذخیره شده در appStateStore.js را بخوانند و شرایط هر رویداد را ارزیابی کنند. خارج از این مرحله: پیاده‌سازی مکانیزم ارسال نوتیفیکیشن یا زمان‌بندی انجام نمی‌شود. نکته حیاتی: هر رویداد باید یک تابع مجزا داشته باشد که شرایط آن را بررسی کند.
**Excerpt:**
```
The provided components lack a clear mechanism for detecting application events (e.g., session completion, daily progress, reminders) and initiating a proactive notification flow. `src/lib/appStateStore.js` stores relevant data, but its purpose is persistence, not event triggering.
```

### Step 3: پیاده‌سازی منطق ارزیابی شرایط نوتیفیکیشن (notification conditions)
**Status:** `done` (100%)
**Scope:** این مرحله شامل پیاده‌سازی منطق ارزیابی شرایط در notificationScheduler.js است که مشخص می‌کند آیا یک رویداد خاص باید منجر به ارسال نوتیفیکیشن شود یا خیر. این شرایط می‌تواند شامل فاصله زمانی از آخرین نوتیفیکیشن، اولویت کاربر، یا تنظیمات خاص باشد. خارج از این مرحله: فراخوانی واقعی تابع ارسال نوتیفیکیشن انجام نمی‌شود. نکته حیاتی: شرایط باید قابل تنظیم و توسعه باشند.
**Excerpt:**
```
responsible for monitoring application state changes, evaluating notification conditions, and triggering `src/lib/telegram.js` to send messages.
```

### Step 4: اتصال notificationScheduler به telegram.js برای ارسال نوتیفیکیشن
**Status:** `done` (100%)
**Scope:** این مرحله شامل اضافه کردن فراخوانی تابع ارسال پیام از src/lib/telegram.js در notificationScheduler.js است. زمانی که یک رویداد تشخیص داده شد و شرایط ارزیابی شد، notificationScheduler باید تابع مناسب از telegram.js را برای ارسال پیام به کاربر فراخوانی کند. خارج از این مرحله: تغییر در telegram.js انجام نمی‌شود. نکته حیاتی: باید از همان تابع موجود در telegram.js استفاده شود، نه ایجاد تابع جدید.
**Excerpt:**
```
triggering `src/lib/telegram.js` to send messages. This service would embody the 'event → notify_event' logic.
```

### Step 5: بررسی وابستگی‌های upstream: اطمینان از سازگاری با appStateStore.js
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی کامل فایل src/lib/appStateStore.js برای اطمینان از اینکه notificationScheduler.js می‌تواند داده‌های مورد نیاز را از آن بخواند. باید مشخص شود که appStateStore.js چه داده‌هایی را ذخیره می‌کند، ساختار آن چگونه است، و آیا نیاز به تغییر در آن برای پشتیبانی از notificationScheduler وجود دارد. خارج از این مرحله: تغییر در appStateStore.js انجام نمی‌شود مگر اینکه ضروری تشخیص داده شود. نکته حیاتی: اگر appStateStore.js نیاز به تغییر دارد، این مرحله باید آن را مستند کند.
**Excerpt:**
```
`src/lib/appStateStore.js` stores relevant data, but its purpose is persistence, not event triggering.
```

### Step 6: بررسی وابستگی‌های downstream: اطمینان از عدم شکستن telegramCommands.js
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی فایل src/lib/telegramCommands.js برای اطمینان از اینکه تغییرات ایجاد شده در notificationScheduler.js و telegram.js باعث شکستن عملکرد موجود telegramCommands.js نمی‌شود. telegramCommands.js مسئول پاسخ‌های واکنشی به دستورات کاربر است و نباید تحت تأثیر قرار گیرد. خارج از این مرحله: تغییر در telegramCommands.js انجام نمی‌شود. نکته حیاتی: باید اطمینان حاصل شود که telegramCommands.js و notificationScheduler.js از یک کانال ارتباطی مجزا استفاده می‌کنند.
**Excerpt:**
```
`src/lib/telegramCommands.js` handles reactive command responses, not proactive system-generated notifications.
```

### Step 7: نوشتن تست‌های واحد برای notificationScheduler.js
**Status:** `done` (100%)
**Scope:** این مرحله شامل نوشتن تست‌های واحد برای تمام توابع موجود در src/lib/notificationScheduler.js است. تست‌ها باید تشخیص رویدادها، ارزیابی شرایط، و فراخوانی telegram.js را پوشش دهند. خارج از این مرحله: تست‌های یکپارچه‌سازی یا end-to-end نوشته نمی‌شود. نکته حیاتی: تست‌ها باید در فایل مجزا (مانند tests/notificationScheduler.test.js) قرار گیرند و تمام edge cases را پوشش دهند.
**Excerpt:**
```
This class of bugs are not found in typical tests because unit tests run in silos.
```

### Step 8: نوشتن تست‌های یکپارچه‌سازی برای pipeline نوتیفیکیشن
**Status:** `partial` (80%)
**Scope:** این مرحله شامل نوشتن تست‌های یکپارچه‌سازی است که کل pipeline نوتیفیکیشن را از تشخیص رویداد تا ارسال پیام آزمایش می‌کند. این تست‌ها باید تعامل بین notificationScheduler.js، appStateStore.js و telegram.js را پوشش دهند. خارج از این مرحله: تست‌های end-to-end با کاربر واقعی نوشته نمی‌شود. نکته حیاتی: تست‌ها باید در فایل مجزا (مانند tests/notificationPipeline.test.js) قرار گیرند.
**Excerpt:**
```
coherence issue means two parts of code have incompatible assumptions — usually a sign of unfinished refactor or feature flag rot.
```

### Step 9: بررسی نهایی و مستندسازی تغییرات
**Status:** `done` (100%)
**Scope:** این مرحله شامل بررسی نهایی تمام تغییرات ایجاد شده، اطمینان از عدم وجود broken reference، و مستندسازی معماری جدید pipeline نوتیفیکیشن است. باید اطمینان حاصل شود که notificationScheduler.js به درستی با سایر اجزا کار می‌کند و هیچ وابستگی از قلم نیفتاده است. خارج از این مرحله: تغییرات جدید در کد انجام نمی‌شود. نکته حیاتی: مستندسازی باید شامل نحوه عملکرد pipeline و نحوه اضافه کردن رویدادهای جدید در آینده باشد.
**Excerpt:**
```
Before and during the change, identify and synchronize all dependencies in four directions completely and without any summarization.
```
