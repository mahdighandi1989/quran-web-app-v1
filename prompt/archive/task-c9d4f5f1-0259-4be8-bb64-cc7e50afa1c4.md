---
task_id: c9d4f5f1-0259-4be8-bb64-cc7e50afa1c4
title: '[منطق] Missing Caption Generation for Proactive Notifications'
type: logic_audit
priority: high
...
### Step 1: تحلیل و مستندسازی ناسازگاری منطقی در pipeline notification
**Status:** `pending` (0%)
**Scope:** این مرحله شامل تحلیل کامل ناسازگاری منطقی شناسایی‌شده در pipeline notification است. باید مشخص شود که دقیقاً کدام بخش‌های کد فرض‌های ناسازگار دارند. این مرحله شامل: (1) بررسی src/lib/telegramCommands.js برای درک نحوه تولید reply text، (2) جستجوی هرگونه component موجود برای تولید محتوای proactive notifications، (3) مستندسازی شکاف بین تولید reply text و proactive caption. خارج از این مرحله: پیاده‌سازی راه‌حل، تغییر کد، یا refactoring. نکته حیاتی: این مرحله صرفاً تحلیلی است و هیچ تغییری در کد ایجاد نمی‌کند.
**Excerpt:**
```
While `src/lib/telegramCommands.js` is responsible for generating reply text (captions) for user commands, there is no explicit component or process defined for generating the content (caption) of *proactive* notifications (e.g., 'Your daily review is due,' 'Session completed summary'). The 'caption' step in the pipeline is only partially covered.
```

### Step 2: بررسی وابستگی‌های upstream برای تولید محتوای proactive notifications
**Status:** `pending` (0%)
**Scope:** این مرحله شامل شناسایی و مستندسازی تمام وابستگی‌های upstream مورد نیاز برای تولید محتوای proactive notifications است. باید مشخص شود: (1) چه داده‌هایی از appState و dataset مورد نیاز است، (2) چگونه appStateStore.js این داده‌ها را فراهم می‌کند، (3) چه توابع یا سرویس‌هایی در حال حاضر به این داده‌ها دسترسی دارند. خارج از این مرحله: تغییر در appStateStore.js یا هر فایل دیگر. نکته حیاتی: این مرحله صرفاً شناسایی وابستگی‌هاست و شامل پیاده‌سازی نیست.
— [merged] این مرحله شامل شناسایی و مستندسازی تمام وابستگی‌های downstream است که به قابلیت تولید محتوای proactive notifications متکی هستند. باید مشخص شود: (1) چه سرویس‌ها یا کامپوننت‌هایی در حال حاضر notification ارسال می‌کنند، (2) آیا این سرویس‌ها از caption استفاده می‌کنند یا خالی/پیش‌فرض ارسال می‌کنند، (3) چه بخش‌هایی از کد ممکن است با تغییر در نحوه تولید caption بشکنند. خارج از این مرحله: تغییر در سرویس‌های downstream. نکته حیاتی: این مرحله صرفاً شناسایی وابستگی‌هاست.
**Excerpt:**
```
The new notification triggering service (suggested in Issue 1) should either be responsible for generating these captions itself, or delegate to a dedicated formatting utility (similar to how `telegramCommands.js` uses `./format.js`) that can access `appState` and `dataset` (potentially via `appStateStore.js`) to create rich, context-aware notification messages.
```

### Step 3: طراحی راه‌حل: سرویس اختصاصی تولید caption برای proactive notifications
**Status:** `pending` (0%)
**Scope:** این مرحله شامل طراحی معماری راه‌حل برای تولید محتوای proactive notifications است. باید مشخص شود: (1) آیا یک سرویس جدید (notification caption service) ایجاد شود یا از یک formatting utility موجود (مشابه format.js) استفاده شود، (2) این سرویس چگونه به appState و dataset دسترسی پیدا کند، (3) چه interface/API برای این سرویس تعریف شود. خارج از این مرحله: پیاده‌سازی کد. نکته حیاتی: این مرحله صرفاً طراحی است و شامل نوشتن کد نمی‌شود.
**Excerpt:**
```
The new notification triggering service (suggested in Issue 1) should either be responsible for generating these captions itself, or delegate to a dedicated formatting utility (similar to how `telegramCommands.js` uses `./format.js`) that can access `appState` and `dataset` (potentially via `appStateStore.js`) to create rich, context-aware notification messages.
```

### Step 4: پیاده‌سازی سرویس تولید caption برای proactive notifications
**Status:** `pending` (0%)
**Scope:** این مرحله شامل پیاده‌سازی سرویس یا utility اختصاصی برای تولید محتوای proactive notifications است. باید: (1) فایل جدیدی (مثلاً src/lib/proactiveNotificationFormatter.js) ایجاد شود، (2) توابعی برای تولید caption انواع مختلف notifications (daily review, session completed summary و غیره) پیاده‌سازی شود، (3) دسترسی به appState و dataset از طریق appStateStore.js فراهم شود. خارج از این مرحله: تغییر در telegramCommands.js یا سایر فایل‌های موجود. نکته حیاتی: این سرویس باید مشابه format.js عمل کند اما برای proactive notifications.
**Excerpt:**
```
The new notification triggering service (suggested in Issue 1) should either be responsible for generating these captions itself, or delegate to a dedicated formatting utility (similar to how `telegramCommands.js` uses `./format.js`) that can access `appState` and `dataset` (potentially via `appStateStore.js`) to create rich, context-aware notification messages.
```

### Step 5: یکپارچه‌سازی سرویس جدید با notification triggering service
**Status:** `pending` (0%)
**Scope:** این مرحله شامل یکپارچه‌سازی سرویس تولید caption (proactiveNotificationFormatter.js) با notification triggering service است. باید: (1) notification triggering service را طوری تغییر دهد که از سرویس جدید برای تولید caption استفاده کند، (2) اطمینان حاصل شود که caption به‌درستی به payload notification اضافه می‌شود، (3) backward compatibility حفظ شود. خارج از این مرحله: تغییر در telegramCommands.js یا سایر بخش‌های غیرمرتبط. نکته حیاتی: notification triggering service باید از سرویس جدید استفاده کند نه اینکه خودش caption تولید کند.
**Excerpt:**
```
The new notification triggering service (suggested in Issue 1) should either be responsible for generating these captions itself, or delegate to a dedicated formatting utility (similar to how `telegramCommands.js` uses `./format.js`) that can access `appState` and `dataset` (potentially via `appStateStore.js`) to create rich, context-aware notification messages.
```

### Step 6: نوشتن unit tests برای proactiveNotificationFormatter.js
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن unit tests برای سرویس جدید proactiveNotificationFormatter.js است. باید: (1) تست‌هایی برای هر تابع (generateDailyReviewCaption, generateSessionCompletedCaption و غیره) نوشته شود، (2) mock داده‌های appState و dataset برای تست‌ها فراهم شود، (3) edge cases (داده‌های خالی، کاربر بدون review و غیره) پوشش داده شود. خارج از این مرحله: integration tests یا end-to-end tests. نکته حیاتی: تست‌ها باید در isolation اجرا شوند و به سرویس‌های واقعی وابسته نباشند.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 7: نوشتن integration tests برای یکپارچه‌سازی با notification triggering service
**Status:** `pending` (0%)
**Scope:** این مرحله شامل نوشتن integration tests برای بررسی یکپارچه‌سازی صحیح proactiveNotificationFormatter.js با notification triggering service است. باید: (1) سناریوهای مختلف trigger notification (daily review, session completed و غیره) تست شود، (2) بررسی شود که caption به‌درستی در payload notification قرار می‌گیرد، (3) بررسی شود که backward compatibility حفظ شده است. خارج از این مرحله: unit tests (مرحله قبل) یا end-to-end tests. نکته حیاتی: این تست‌ها باید تعامل بین سرویس‌ها را بررسی کنند.
**Excerpt:**
```
Without a clear component for generating proactive notification content, the system cannot send meaningful automated messages. This would either lead to generic, uninformative notifications or require ad-hoc content generation within the triggering logic, leading to code duplication and maintainability issues.
```

### Step 8: بررسی backward compatibility و رفع مشکلات احتمالی
**Status:** `pending` (0%)
**Scope:** این مرحله شامل بررسی backward compatibility پس از تغییرات است. باید: (1) اطمینان حاصل شود که سرویس‌های موجود که از notification استفاده می‌کنند (بدون caption یا با caption پیش‌فرض) همچنان کار می‌کنند، (2) بررسی شود که هیچ API endpoint یا interface ای تغییر signature نداده است، (3) در صورت وجود مشکل، رفع شود. خارج از این مرحله: اضافه کردن feature جدید. نکته حیاتی: هیچ سرویس موجود نباید به دلیل این تغییرات بشکند.
**Excerpt:**
```
coherence issue یعنی دو بخش کد فرض‌های ناسازگار دارند — معمولاً نشانه‌ی refactor ناتمام یا feature flag rot است. این کلاس bug ها در test معمولی پیدا نمی‌شوند چون unit test ها در silo اجرا می‌شوند.
```

### Step 9: مستندسازی نهایی و بستن issue
**Status:** `pending` (0%)
**Scope:** این مرحله شامل مستندسازی نهایی تغییرات و بستن issue است. باید: (1) مستندات فنی (ADR یا docs) به‌روزرسانی شود، (2) تغییرات در CHANGELOG ثبت شود، (3) issue مربوطه بسته شود. خارج از این مرحله: تغییرات کد جدید. نکته حیاتی: همه مستندات باید دقیق و کامل باشند.
**Excerpt:**
```
The new notification triggering service (suggested in Issue 1) should either be responsible for generating these captions itself, or delegate to a dedicated formatting utility (similar to how `telegramCommands.js` uses `./format.js`) that can access `appState` and `dataset` (potentially via `appStateStore.js`) to create rich, context-aware notification messages.
```
