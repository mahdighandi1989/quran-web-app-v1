...
updated_at: '2026-05-30T10:42:00.196138+00:00'
archived: true
archived_at: '2026-05-30T10:42:00.196131+00:00'
target_files:
- src/App.jsx
---
...
فایل `src/App.jsx` شامل Firebase config کامل با apiKey، authDomain، projectId، storageBucket، messagingSenderId، appId و measurementId به صورت plain text است (خطوط 12-20). این اطلاعات در build نهایی Vite به صورت کامل در bundle فرانت‌اند قرار می‌گیرد و هر کاربری می‌تواند آنها را از source maps یا bundle استخراج کند. اگرچه Firebase API Key ذاتاً برای استفاده در سمت کلاینت طراحی شده، اما قرار دادن تمام config به صورت plain text در کد منبع ریسک امنیتی ایجاد می‌کند و امکان سوءاستفاده از سرویس‌های Firebase (مانند Firestore یا Storage) را فراهم می‌کند.
...
- `src/App.jsx:12-20` — `firebaseConfig` — کل Firebase config به صورت plain text در کد منبع
  ```jsx
...
- `.gitignore` (سطر 1) — باید .env به این فایل اضافه شود
- `src/App.jsx` (سطر 21) — محل استفاده از متغیرهای محیطی

## 🌐 نقشهٔ وابستگی‌ها
این تغییر فقط روی فایل App.jsx تأثیر می‌گذارد و نیاز به تغییر در هیچ وابستگی دیگری ندارد.
...
- [ ] Firebase config در کد منبع با import.meta.env جایگزین شده باشد
- [ ] فایل .env.example با placeholder values ایجاد شده باشد
- [ ] .env به .gitignore اضافه شده باشد
- [ ] برنامه با متغیرهای محیطی به درستی کار کند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. Firebase config را به متغیرهای محیطی (environment variables) منتقل کنید. یک فایل `.env` در ریشه پروژه ایجاد کنید و مقادیر را در `VITE_FIREBASE_API_KEY` و سایر متغیرهای مشابه قرار دهید. سپس در `src/App.jsx` از `import.meta.env.VITE_FIREBASE_API_KEY` استفاده کنید. همچنین فایل `.env` را به `.gitignore` اضافه کنید.
...
## 🧪 دستورات اعتبارسنجی
- `grep -r "AIzaSyBGVf6Ep5JIwg61pNvml8XqdzfDazZ2MT0" src/`
- `cat .gitignore | grep ".env"`
- `npm run build && grep -r "AIzaSyBGVf6Ep5JIwg61pNvml8XqdzfDazZ2MT0" dist/`

## ⚠️ ریسک‌ها و موارد احتیاط
بدون ریسک خاصی، فقط نیاز به تنظیم مجدد متغیرهای محیطی در محیط‌های مختلف
...
**Excerpt:**
```
فایل `src/App.jsx` شامل Firebase config کامل با apiKey، authDomain، projectId، storageBucket، messagingSenderId، appId و measurementId به صورت plain text است (خطوط 12-20). این اطلاعات در build نهایی Vite به صورت کامل در bundle فرانت‌اند قرار می‌گیرد و هر کاربری می‌تواند آنها را از source maps یا bundle استخراج کند.
```
...
**Excerpt:**
```
فایل `src/App.jsx` شامل Firebase config کامل با apiKey، authDomain، projectId، storageBucket، messagingSenderId، appId و measurementId به صورت plain text است (خطوط 12-20).
```
...
**Excerpt:**
```
این اطلاعات در build نهایی Vite به صورت کامل در bundle فرانت‌اند قرار می‌گیرد و هر کاربری می‌تواند آنها را از source maps یا bundle استخراج کند.
```
