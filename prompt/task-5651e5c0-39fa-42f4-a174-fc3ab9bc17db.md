---
task_id: 5651e5c0-39fa-42f4-a174-fc3ab9bc17db
title: "دکمه‌ی UI بدون handler: setAnswers((a) => ({ ...a, [i]: ci }))}>\n                        {c}{correct ? '"
type: bug_fix
priority: medium
...
created_at: '2026-06-12T19:21:38.553948+00:00'
updated_at: '2026-06-12T19:21:42.625120+00:00'
target_files:
...
---
[scan #2 at 2026-06-12T19:21:39.236777+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/AIExamGenerator.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setRevealed(true)}>نمایش پاسخ‌ها`
- فایل: `src/components/AIExamGenerato
---
[scan #3 at 2026-06-12T19:21:39.568762+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/AISettings.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `setShowKey((s) => !s)}>{showKey ? 'پنهان' : 'نمایش'}`
- فایل: `src/components
---
[scan #4 at 2026-06-12T19:21:39.904350+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/AISettings.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `removeProvider(active.id)}>حذف پروایدر`
- فایل: `src/components/AISettings.js
---
[scan #5 at 2026-06-12T19:21:40.243676+00:00]
## 📋 شرح
یک دکمه/کنترل UI در فایل `src/components/AIWidgets.jsx` پیدا شد که هیچ event handler معنادار به آن متصل نیست (onClick، onChange، form submit، router push، یا API call شناسایی نشد).

## 🔍 جزئیات
- label/متن دکمه: `{ if (!open) run(); else setOpen(false); }}>
        {compact ? '✨' : label}`
...
1. git blame مشخص می‌کند چرا این دکمه `setAnswers((a) => ({ ...a, [i]: ci }))}>
                        {c}{correct ? '` فاقد handler است
2. یکی از این سه حالت تعیین شده: (a) handler restore شده + کار می‌کند، (b) دکمه حذف شده، (c) به‌صورت decorative علامت‌گذاری شده
3. اگر دکمه باقی ماند، تست end-to-end (Playwright یا cypress) برای کلیک و تأیید رفتار اضافه شده
