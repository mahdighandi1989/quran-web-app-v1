...
updated_at: '2026-06-12T19:16:36.632241+00:00'
target_files:
- src/lib/aiClient.js
...
1. `chatComplete`, `validateProviderKey`, and `validateModel` functions include a default timeout for network requests.
2. Error messages returned by these functions are more specific (e.g., 'Invalid API Key', 'Rate Limit Exceeded', 'Network Unreachable') based on HTTP status codes or network errors.
3. The UI components (`AIExamGenerator`, `AIWidgets`, `AISettings`) display these more specific error messages to the user.
4. New unit/integration tests are added to `src/test/aiClient.test.js` and `src/test/aiProviders.test.js` to cover timeout and various API error scenarios.
