---
task_id: 8a4b57e3-9e0b-4851-a5d4-01c9f9e03d26
title: اعتبارسنجی payload در driveDownload
type: bug
priority: high
...
created_at: '2026-06-05T18:43:58.170169+00:00'
updated_at: '2026-06-06T04:56:13.161475+00:00'
target_files:
- src/lib/drive.js
---

# اعتبارسنجی payload در driveDownload

## Raw Idea

تابع `driveDownload` در `src/lib/drive.js` نتیجهٔ `res.json()` را مستقیماً و بدون هیچ validation به caller برمی‌گرداند. اگر فایل Google Drive خراب، خالی، یا حاوی JSON نامعتبر باشد، برنامه در زمان parse یا استفاده از داده crash می‌کند. شواهد: در `src/App.test.jsx` خطوط 282-295 تست‌هایی برای این سناریو نوشته شده که نشان می‌دهد تابع باید validation داشته باشد ولی خود تابع هنوز آن را ندارد. همچنین `validateDrivePayload` در همان فایل تست وجود دارد ولی در `driveDownload` صدا زده نمی‌شود.
...
عدم اعتبارسنجی payload در driveDownload — ریسک crash در صورت خرابی فایل

## 📍 موقعیت دقیق در پروژه
_(file:line — symbol — snippet)_

- `src/lib/drive.js:~50-70` — `driveDownload` — باید validateDrivePayload(data) صدا زده شود
  ```jsx
  const data = await res.json();
  return data;  // ⚠️ بدون validation
  ```
...
Vanilla JS, Google Drive API v3, fetch API

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `src/App.test.jsx` (سطر 266) — تست‌های validateDrivePayload و driveDownload در این فایل هستند
- `src/App.jsx` — App.jsx از driveDownload استفاده می‌کند

## 🌐 نقشهٔ وابستگی‌ها
این تابع توسط App.jsx برای بازیابی بکاپ از Google Drive استفاده می‌شود؛ عدم validation می‌تواند کل برنامه را از کار بیندازد.
...
- [ ] driveDownload پس از دریافت پاسخ، validateDrivePayload را صدا می‌زند
- [ ] اگر payload یک primitive باشد (مثلاً عدد 12345)، خطای 'payload نامعتبر' پرتاب می‌کند
- [ ] اگر JSON قابل parse نباشد، خطای 'فایل قابل تجزیه نیست' پرتاب می‌کند
- [ ] تست‌های موجود در App.test.jsx پاس می‌شوند
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. در تابع `driveDownload`، پس از `res.json()`، نتیجه را به `validateDrivePayload` پاس بده و اگر معتبر نبود، یک خطای مشخص با پیام فارسی پرتاب کن. همچنین catch خطای JSON.parse (SyntaxError) را اضافه کن.

## 💡 نمونه‌های قبل/بعد
**driveDownload با validation**
...
## 🧪 دستورات اعتبارسنجی
- `npx vitest run src/test/aiClient.test.js src/test/aiProviders.test.js src/test/aiStore.test.js src/test/aiTasks.test.js src/test/appStateStore.test.js src/test/envExample.test.js src/test/example.test.js src/test/firebaseConfig.test.js src/test/inspectorBridge.regression.test.js src/test/packageLock.test.js src/test/quranPages.test.js src/test/routing.test.jsx src/test/setup.test.js src/test/stats.test.js src/test/telegram.test.js src/test/telegramCommands.test.js src/test/telegramStore.test.js src/App.test.jsx src/main.test.jsx src/test/AISettings.test.jsx src/test/TelegramSettings.test.jsx`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر minimal است و فقط یک خط اضافه می‌کند؛ هیچ ریسکی ندارد.
...
1. driveDownload پس از دریافت پاسخ، validateDrivePayload را صدا می‌زند _(verify: static)_
2. اگر payload یک primitive باشد (مثلاً عدد 12345)، خطای 'payload نامعتبر' پرتاب می‌کند _(verify: backend_test)_
3. اگر JSON قابل parse نباشد، خطای 'فایل قابل تجزیه نیست' پرتاب می‌کند _(verify: backend_test)_
4. تست‌های موجود در App.test.jsx پاس می‌شوند _(verify: backend_test)_
