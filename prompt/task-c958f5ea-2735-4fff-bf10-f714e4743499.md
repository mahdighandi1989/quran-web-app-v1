...
updated_at: '2026-06-12T19:12:15.643536+00:00'
target_files:
- server/ai-proxy.mjs
...
1. `server/ai-proxy.mjs` و `server/telegram-bot.mjs` شامل منطق محدودیت نرخ هستند.
2. پارامترهای محدودیت نرخ (مانند حداکثر درخواست‌ها، مدت زمان پنجره) از طریق متغیرهای محیطی قابل تنظیم هستند.
3. یک تست جدید (مثلاً در `tests/e2e/test_rate_limiting.py`) تأیید می‌کند که درخواست‌های بیش از حد مجاز رد می‌شوند.
