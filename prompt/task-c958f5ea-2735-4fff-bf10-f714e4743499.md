---
task_id: c958f5ea-2735-4fff-bf10-f714e4743499
title: عدم وجود محدودیت نرخ (Rate Limiting) صریح در AI proxy و Telegram webhook endpoints
type: security
priority: high
...
created_at: '2026-06-12T19:12:15.643534+00:00'
updated_at: '2026-06-12T19:29:38.804979+00:00'
target_files:
- server/ai-proxy.mjs
- server/telegram-bot.mjs
---

# عدم وجود محدودیت نرخ (Rate Limiting) صریح در AI proxy و Telegram webhook endpoints

## Raw Idea

سرور Node.js یک AI proxy endpoint (`/api/ai/proxy`) و یک Telegram webhook endpoint (`/webhook`) را در معرض دید قرار می‌دهد. نه لیست فایل‌های ارائه شده و نه `.env.example` هیچ مکانیسم محدودیت نرخ صریحی را برای این endpointها نشان نمی‌دهند. بدون محدودیت نرخ، این endpointها در برابر سوءاستفاده، حملات انکار سرویس (DoS) یا مصرف بیش از حد سهمیه API ارائه‌دهنده هوش مصنوعی آسیب‌پذیر هستند که منجر به هزینه‌های غیرمنتظره یا اختلال در سرویس می‌شود.
...
- `server/ai-proxy.mjs:نامشخص` — `API endpoint definition`
  ```
  // Add rate limiting middleware before the route handler
  ```
- `server/telegram-bot.mjs:نامشخص` — `Webhook endpoint definition`
  ```
  // Add rate limiting middleware before the route handler
  ```
...
Node.js server, API security, rate limiting.

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `.env.example` — برای متغیرهای پیکربندی جدید محدودیت نرخ.
- `server/package.json` — برای افزودن کتابخانه محدودیت نرخ در صورت نیاز.
- `docs/architecture/data-pipeline.md` — AI proxy بخش حیاتی پایپ‌لاین داده است.
- `server/telegram-bot.mjs` — این فایل `ai-proxy.mjs` را import می‌کند (caller)
- `server/ai.mjs` — `telegram-bot.mjs` این فایل را import می‌کند
- `server/ai-proxy.mjs` — `telegram-bot.mjs` این فایل را import می‌کند

## 🌐 نقشهٔ وابستگی‌ها
محدودیت نرخ یک نگرانی فراگیر برای هر API عمومی است. این از منابع سرور و سهمیه‌های API خارجی محافظت می‌کند.
...
- [ ] `server/ai-proxy.mjs` و `server/telegram-bot.mjs` شامل منطق محدودیت نرخ هستند.
- [ ] پارامترهای محدودیت نرخ (مانند حداکثر درخواست‌ها، مدت زمان پنجره) از طریق متغیرهای محیطی قابل تنظیم هستند.
- [ ] یک تست جدید (مثلاً در `tests/e2e/test_rate_limiting.py`) تأیید می‌کند که درخواست‌های بیش از حد مجاز رد می‌شوند.
- [ ] هیچ تستی fail نمی‌شود (`npm run test` / `pytest`)
- [ ] linter بدون warning عبور می‌کند
- [ ] type-check موفق است (`tsc --noEmit` / `mypy`)

## 🪜 مراحل اجرایی پیشنهادی
1. یک میان‌افزار محدودیت نرخ برای AI proxy و Telegram webhook endpoints پیاده‌سازی کنید. این می‌تواند شامل استفاده از یک کتابخانه مانند `express-rate-limit` (در صورت استفاده از Express) یا یک پیاده‌سازی سفارشی بر اساس آدرس IP یا شناسه کاربر (در صورت احراز هویت) باشد. محدودیت نرخ باید از طریق متغیرهای محیطی قابل تنظیم باشد.

## 💡 نمونه‌های قبل/بعد
**قبل: endpoint بدون محدودیت نرخ**
...
app.post('/api/ai/proxy', apiLimiter, async (req, res) => { /* ... */ });
```
...
## 🧪 دستورات اعتبارسنجی
- `grep -r "rate-limit" server/`
- `pytest tests/e2e/test_rate_limiting.py`

## ⚠️ ریسک‌ها و موارد احتیاط
محدودیت‌های نرخ بیش از حد تهاجمی می‌توانند کاربران قانونی را مسدود کنند. پیاده‌سازی نادرست می‌تواند دور زده شود.
...
1. `server/ai-proxy.mjs` و `server/telegram-bot.mjs` شامل منطق محدودیت نرخ هستند. _(verify: static)_
2. پارامترهای محدودیت نرخ (مانند حداکثر درخواست‌ها، مدت زمان پنجره) از طریق متغیرهای محیطی قابل تنظیم هستند. _(verify: static)_
3. یک تست جدید (مثلاً در `tests/e2e/test_rate_limiting.py`) تأیید می‌کند که درخواست‌های بیش از حد مجاز رد می‌شوند. _(verify: backend_test)_
