---
task_id: 3949f28e-5a5d-47f9-a14f-62c3806e30cc
title: عدم تطابق نام endpoint WebSocket در Inspector Bridge با backend پروژه
type: bug
priority: high
...
created_at: '2026-06-05T18:43:58.174326+00:00'
updated_at: '2026-06-06T04:56:59.805601+00:00'
target_files:
- index.html
- index.html
---

# عدم تطابق نام endpoint WebSocket در Inspector Bridge با backend پروژه

## Raw Idea

در index.html خط 207، متغیر WS_URL به صورت پیش‌فرض خالی است و تنها از طریق window.__INSPECTOR_WS_URL__ تنظیم می‌شود. اما در خط 208، PROJECT_ID به صورت hardcoded به 'gh_mahdighandi1989_quran_web_app_v1' تنظیم شده است. این PROJECT_ID در backend پروژه (server/) هیچ endpoint متناظری ندارد. همچنین در خط 243-248، پیام 'inspector-bridge-ready' با pageUrl: window.location.href ارسال می‌شود که در نسخه‌های قبلی (طبق task-61f7b3ba) باگ 'window.locatio' داشته است. این باعث می‌شود Inspector Bridge نتواند به درستی ثبت نام کند و دستورات Inspector را دریافت یا ارسال کند.
...
- `index.html:207-208` — `WS_URL / PROJECT_ID` — PROJECT_ID hardcoded و غیرقابل پیکربندی
  ```
  const WS_URL = (typeof window !== 'undefined' && window.__INSPECTOR_WS_URL__) || '';
  const PROJECT_ID = 'gh_mahdighandi1989_quran_web_app_v1';
  ```
- `index.html:243-248` — `inspector-bridge-ready message` — این بخش باید بررسی شود که window.location.href به درستی استفاده شده است
  ```
...
Vanilla JS در index.html، WebSocket API

## 🔗 فایل‌های مرتبط (Cross-references)
_(فایل‌هایی که با موقعیت‌های هدف در ارتباط هستند — import، caller، shared state)_

- `server/ai.mjs` (سطر 1) — backend پروژه که ممکن است endpoint WebSocket داشته باشد
- `server/telegram-bot.mjs` (سطر 1) — backend پروژه که ممکن است endpoint WebSocket داشته باشد

## 🌐 نقشهٔ وابستگی‌ها
این تغییر فقط روی index.html تأثیر می‌گذارد و backend پروژه را تحت تأثیر قرار نمی‌دهد.
...
- [ ] PROJECT_ID قابل پیکربندی از طریق window.__INSPECTOR_PROJECT_ID__ باشد
- [ ] پیام 'inspector-bridge-ready' حاوی pageUrl صحیح باشد
...
1. اصلاح PROJECT_ID به یک مقدار پویا یا حذف hardcoded value و جایگزینی با یک متغیر محیطی. همچنین اطمینان از اینکه window.location.href به درستی استفاده شده است.
...
_بعد:_
```
const PROJECT_ID = window.__INSPECTOR_PROJECT_ID__ || 'gh_mahdighandi1989_quran_web_app_v1';
```
...
## 🧪 دستورات اعتبارسنجی
- `باز کردن index.html در مرورگر و بررسی کنسول برای خطاهای WebSocket`
- `بررسی اینکه window.__INSPECTOR_PROJECT_ID__ قابل تنظیم است`

## ⚠️ ریسک‌ها و موارد احتیاط
تغییر کوچک، ریسک پایین
...
1. PROJECT_ID قابل پیکربندی از طریق window.__INSPECTOR_PROJECT_ID__ باشد _(verify: static)_
2. پیام 'inspector-bridge-ready' حاوی pageUrl صحیح باشد _(verify: static)_
3. هیچ خطای WebSocket در کنسول مرورگر ظاهر نشود _(verify: static)_
